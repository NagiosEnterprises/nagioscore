...
int run_scheduled_service_check(service * svc, int check_options, double latency)
{
	...
	log_debug_info(DEBUGL_CHECKS, 0,
				   "Attempting to run scheduled check of service '%s' on host '%s': check options=%d, latency=%lf\n",
	...
	result =
		run_async_service_check(svc, check_options, latency, TRUE, TRUE, &time_is_valid,
								&preferred_time);
		...
		if (svc->should_be_scheduled == TRUE) {
			...
			if (current_time >= preferred_time)
				preferred_time =
					current_time +
					((svc->check_interval <=
					  0) ? 300 : (svc->check_interval * interval_length));
			...
			/* this service could be rescheduled... */
				...
				log_debug_info(DEBUGL_CHECKS, 1, "Rescheduled next service check for %s",
...
int run_async_service_check(service * svc, int check_options, double latency,
							int scheduled_check, int reschedule_check, int *time_is_valid,
							time_t * preferred_time)
{
	...
	if (check_service_check_viability(svc, check_options, time_is_valid, preferred_time) ==
		ERROR)
		return ERROR;
	...
	neb_result =
		broker_service_check(NEBTYPE_SERVICECHECK_ASYNC_PRECHECK, NEBFLAG_NONE, NEBATTR_NONE,
							 svc, CHECK_TYPE_ACTIVE, start_time, end_time, svc->check_command,
							 svc->latency, 0.0, 0, FALSE, 0, NULL, NULL, NULL);

	if (neb_result == NEBERROR_CALLBACKCANCEL || neb_result == NEBERROR_CALLBACKOVERRIDE) {
		log_debug_info(DEBUGL_CHECKS, 0,
					   "Check of service '%s' on host '%s' (id=%u) was %s by a module\n",
	...
	get_raw_command_line_r(&mac, svc->check_command_ptr, svc->check_command, &raw_command,
						   macro_options);
	if (raw_command == NULL) {
	...
	/* send data to event broker */
	neb_result =
		broker_service_check(NEBTYPE_SERVICECHECK_INITIATE, NEBFLAG_NONE, NEBATTR_NONE, svc,
							 CHECK_TYPE_ACTIVE, start_time, end_time, svc->check_command,
							 svc->latency, 0.0, service_check_timeout, FALSE, 0,
							 processed_command, NULL, cr);
	...
	if (runchk_result == ERROR) {
		...
		update_check_stats((scheduled_check ==
							TRUE) ? ACTIVE_SCHEDULED_SERVICE_CHECK_STATS :
						   ACTIVE_ONDEMAND_SERVICE_CHECK_STATS, start_time.tv_sec);
	}
	...
	if (queued_check_result->check_type == CHECK_TYPE_ACTIVE) {
		if (queued_check_result->early_timeout == TRUE) {
			...
			asprintf(&temp_service->plugin_output,
					 "(Service check timed out after %.2lf seconds)",
					 temp_service->execution_time);
		...
		/* if there was some error running the command, just skip it (this shouldn't be happening) */
			...
			temp_service->plugin_output =
				(char *)strdup("(Service check did not exit properly)");
		...
		/* make sure the return code is within bounds */
			...
			logit(NSLOG_RUNTIME_WARNING, TRUE,
				  ...
				  queued_check_result->return_code, temp_service->description,
				  temp_service->host_name,
				  (queued_check_result->return_code ==
				   126 ? "Make sure the plugin you're trying to run is executable."
				   : (queued_check_result->return_code ==
					  127 ? " Make sure the plugin you're trying to run actually exists." :
					  "")));

			asprintf(&temp_plugin_output, "(Return code of %d is out of bounds%s : %s)",
					 queued_check_result->return_code,
					 (queued_check_result->return_code ==
					  126 ? " - plugin may not be executable" : (queued_check_result->
																 return_code ==
																 127 ?
									  ...
									  check_result * queued_check_result)
{
	...
	log_debug_info(DEBUGL_CHECKS, 0,
				   "** Handling check result for service '%s' on host '%s' from '%s'...\n",
				   temp_service->description, temp_service->host_name,
	...
	log_debug_info(DEBUGL_CHECKS, 1,
				   ...
				   (queued_check_result->check_type ==
					CHECK_TYPE_ACTIVE) ? "Active" : "Passive",
				   ...
				   queued_check_result->return_code, queued_check_result->output);
	...
	temp_service->execution_time =
		(double)((double)
				 (queued_check_result->finish_time.tv_sec -
				  queued_check_result->start_time.tv_sec) +
				 (double)((queued_check_result->finish_time.tv_usec -
						   queued_check_result->start_time.tv_usec) / 1000.0) / 1000.0);
	...
	temp_service->check_type =
		(queued_check_result->check_type ==
		 CHECK_TYPE_ACTIVE) ? CHECK_TYPE_ACTIVE : CHECK_TYPE_PASSIVE;
	...
	parse_check_output(queued_check_result->output, &temp_service->plugin_output,
					   &temp_service->long_plugin_output, &temp_service->perf_data, TRUE,
					   FALSE);
	...
	log_debug_info(DEBUGL_CHECKS, 2, "Short Output: %s\n",
				   (temp_service->plugin_output ==
					NULL) ? "NULL" : temp_service->plugin_output);
	log_debug_info(DEBUGL_CHECKS, 2, "Long Output:	%s\n",
				   (temp_service->long_plugin_output ==
					NULL) ? "NULL" : temp_service->long_plugin_output);
	...
	log_debug_info(DEBUGL_CHECKS, 2, "ST: %s  CA: %d  MA: %d  CS: %d  LS: %d  LHS: %d\n",
				   ...
				   temp_service->current_attempt, temp_service->max_attempts,
				   temp_service->current_state, temp_service->last_state,
	...
	if (state_change == TRUE || hard_state_change == TRUE) {
		...
		if (temp_service->acknowledgement_type == ACKNOWLEDGEMENT_NORMAL
			&& (state_change == TRUE || hard_state_change == FALSE)) {
	...
	if (temp_service->current_state == STATE_OK) {
		...
		if (temp_host->current_state != HOST_UP) {
			...
			if (first_host_check_initiated == TRUE)
				...
				if ((state_change == FALSE || state_changes_use_cached_state == TRUE)
					&& temp_host->has_been_checked == TRUE
					&& ((current_time - temp_host->last_check) <= cached_host_check_horizon)) {
	...
	/* hey, something's not working quite like it should... */
		...
		if (temp_host->current_state == HOST_UP) {
			...
			if (execute_host_checks
				&& (state_change == TRUE && state_changes_use_cached_state == FALSE)
		...
		if (temp_service->current_attempt < temp_service->max_attempts) {
			...
			if (execute_service_checks && enable_predictive_service_dependency_checks == TRUE
				...
				for (list = temp_service->exec_deps; list; list = list->next) {
					...
					if (temp_dependency->dependent_service_ptr == temp_service
						...
						schedule_service_check(master_service, current_time,
											   CHECK_OPTION_DEPENDENCY_CHECK);
					}
				}
	...
	if (reschedule_check == TRUE) {
		...
		if (next_valid_time > preferred_time) {
			 ...
			 */
			temp_service->next_check += ranged_urand(0, check_window(temp_service));
		}
...
#ifdef USE_EVENT_BROKER
	/* send data to event broker */
	broker_service_check(NEBTYPE_SERVICECHECK_PROCESSED, NEBFLAG_NONE, NEBATTR_NONE,
						 temp_service, temp_service->check_type,
						 queued_check_result->start_time, queued_check_result->finish_time,
						 NULL, temp_service->latency, temp_service->execution_time,
						 service_check_timeout, queued_check_result->early_timeout,
						 queued_check_result->return_code, NULL, NULL, queued_check_result);
#endif
...
void schedule_service_check(service * svc, time_t check_time, int options)
{
	...
	log_debug_info(DEBUGL_CHECKS, 0,
				   ...
				   svc->description, svc->host_name, ctime(&check_time));
...
int check_service_check_viability(service * svc, int check_options, int *time_is_valid,
								  time_t * new_time)
{
	int 		result = OK;
	...
	if (!(check_options & CHECK_OPTION_FORCE_EXECUTION)) {
		...
		if (check_time_against_period((unsigned long)current_time, svc->check_period_ptr) ==
			ERROR) {
			preferred_time = current_time;
			if (time_is_valid)
				*time_is_valid = FALSE;
	...
	if (check_service_parents(svc) == DEPENDENCIES_FAILED) {
		...
		log_debug_info(DEBUGL_CHECKS, 2,
					   "Execution parents for this service failed, so it will not be actively checked.\n");
	}
...
int check_service_parents(service * svc)
{
	...
	/* check all parents... */
	for (temp_servicesmember = svc->parents; temp_servicesmember;
		...
		if ((parent_service = temp_servicesmember->service_ptr) == NULL) {
			logit(NSLOG_RUNTIME_WARNING, TRUE,
				  "Warning: service '%s' on host '%s' is NULL ptr\n",
	...
	for (temp_service = service_list; temp_service != NULL; temp_service = temp_service->next) {
		...
		if (expected_time < current_time) {
			...
			logit(NSLOG_RUNTIME_WARNING, TRUE,
				  ...
				  temp_service->description, temp_service->host_name, temp_service->last_check,
...
void check_service_result_freshness(void)
{
	...
	for (temp_service = service_list; temp_service != NULL; temp_service = temp_service->next) {
		...
		if (temp_service->checks_enabled == FALSE
			&& temp_service->accept_passive_checks == FALSE)
			continue;
...
int is_service_result_fresh(service * temp_service, time_t current_time, int log_this)
{
	...
	if (temp_service->freshness_threshold == 0) {
		if (temp_service->state_type == HARD_STATE || temp_service->current_state == STATE_OK)
			freshness_threshold =
				(temp_service->check_interval * interval_length) + temp_service->latency +
				additional_freshness_latency;
		else
			freshness_threshold =
				(temp_service->retry_interval * interval_length) + temp_service->latency +
				additional_freshness_latency;
	} else
	 ...
	 * by Altinity
	 */
	else if (temp_service->checks_enabled == TRUE && event_start > temp_service->last_check
	...
	log_debug_info(DEBUGL_CHECKS, 2, "HBC: %d, PS: %lu, ES: %lu, LC: %lu, CT: %lu, ET: %lu\n",
				   temp_service->has_been_checked, (unsigned long)program_start,
				   (unsigned long)event_start, (unsigned long)temp_service->last_check,
				   (unsigned long)current_time, (unsigned long)expiration_time);

	/* the results for the last check of this service are stale */
	if (expiration_time < current_time) {

		get_time_breakdown((current_time - expiration_time), &days, &hours, &minutes,
						   &seconds);
		...
		if (log_this == TRUE)
			logit(NSLOG_RUNTIME_WARNING, TRUE,
				  ...
				  temp_service->description, temp_service->host_name, days, hours, minutes,
		...
		log_debug_info(DEBUGL_CHECKS, 1,
					   ...
					   temp_service->description, temp_service->host_name, days, hours,
					   minutes, seconds, tdays, thours, tminutes, tseconds);
...
void schedule_host_check(host * hst, time_t check_time, int options)
{
	...
	if (temp_event != NULL) {
		...
		log_debug_info(DEBUGL_CHECKS, 2, "Found another host check event for this host @ %s",
					   ctime(&temp_event->run_time));
...
void check_for_orphaned_hosts(void)
{
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		/* determine the time at which the check results should have come in (allow 10 minutes slack time) */
		expected_time =
			(time_t) (temp_host->next_check + temp_host->latency + host_check_timeout +
					  check_reaper_interval + 600);
...
int is_host_result_fresh(host * temp_host, time_t current_time, int log_this)
{
	...
	if (temp_host->freshness_threshold == 0) {
		...
		freshness_threshold =
			(interval * interval_length) + temp_host->latency + additional_freshness_latency;
	} else
	 ...
	 * Altinity
	 */
	else if (temp_host->checks_enabled == TRUE && event_start > temp_host->last_check
	...
	log_debug_info(DEBUGL_CHECKS, 2, "HBC: %d, PS: %lu, ES: %lu, LC: %lu, CT: %lu, ET: %lu\n",
				   temp_host->has_been_checked, (unsigned long)program_start,
				   (unsigned long)event_start, (unsigned long)temp_host->last_check,
				   (unsigned long)current_time, (unsigned long)expiration_time);
	...
	if (expiration_time < current_time) {
		...
		if (log_this == TRUE)
			logit(NSLOG_RUNTIME_WARNING, TRUE,
				  ...
				  temp_host->name, days, hours, minutes, seconds, tdays, thours, tminutes,
				  tseconds);
...
int run_scheduled_host_check(host * hst, int check_options, double latency)
{
	...
	log_debug_info(DEBUGL_CHECKS, 0,
				   "Attempting to run scheduled check of host '%s': check options=%d, latency=%lf\n",
	...
	/* attempt to run the check */
	result =
		run_async_host_check(hst, check_options, latency, TRUE, TRUE, &time_is_valid,
							 &preferred_time);
		...
		if (hst->should_be_scheduled == TRUE) {
			...
			if (current_time >= preferred_time)
				preferred_time =
					current_time +
					((hst->check_interval <=
					  0) ? 300 : (hst->check_interval * interval_length));
...
int run_async_host_check(host * hst, int check_options, double latency, int scheduled_check,
						 int reschedule_check, int *time_is_valid, time_t * preferred_time)
{
	nagios_macros mac;
	char	   *raw_command = NULL;
	char	   *processed_command = NULL;
	...
	if (!(check_options & CHECK_OPTION_FORCE_EXECUTION)) {
		...
		if (hst->last_check + cached_host_check_horizon > time(NULL)) {
			log_debug_info(DEBUGL_CHECKS, 0,
						   "Host '%s' was last checked within its cache horizon. Aborting check\n",
						   hst->name);
			return ERROR;
		}
	}
	...
	/* send data to event broker */
	neb_result =
		broker_host_check(NEBTYPE_HOSTCHECK_ASYNC_PRECHECK, NEBFLAG_NONE, NEBATTR_NONE, hst,
						  CHECK_TYPE_ACTIVE, hst->current_state, hst->state_type, start_time,
						  end_time, hst->check_command, hst->latency, 0.0, host_check_timeout,
	...
	/* get the raw command line */
	get_raw_command_line_r(&mac, hst->check_command_ptr, hst->check_command, &raw_command,
						   macro_options);
	if (raw_command == NULL) {
		clear_volatile_macros_r(&mac);
		log_debug_info(DEBUGL_CHECKS, 0,
	...
	neb_result =
		broker_host_check(NEBTYPE_HOSTCHECK_INITIATE, NEBFLAG_NONE, NEBATTR_NONE, hst,
						  CHECK_TYPE_ACTIVE, hst->current_state, hst->state_type, start_time,
						  end_time, hst->check_command, hst->latency, 0.0, host_check_timeout,
						  FALSE, 0, processed_command, NULL, NULL, NULL, NULL, cr);
	...
	if (runchk_result == ERROR) {
		...
		update_check_stats((scheduled_check ==
							TRUE) ? ACTIVE_SCHEDULED_HOST_CHECK_STATS :
						   ACTIVE_ONDEMAND_HOST_CHECK_STATS, start_time.tv_sec);
...
static int get_host_check_return_code(host * temp_host, check_result * queued_check_result)
{
	...
	if (queued_check_result->check_type == CHECK_TYPE_ACTIVE) {
		if (queued_check_result->early_timeout) {
			...
			asprintf(&temp_host->plugin_output, "(Host check timed out after %.2lf seconds)",
		...
		/* make sure the return code is within bounds */
			...
			logit(NSLOG_RUNTIME_WARNING, TRUE,
				  ...
				  queued_check_result->return_code, temp_host->name,
				  (queued_check_result->return_code == 126
				   || queued_check_result->return_code ==
				   127) ? " Make sure the plugin you're trying to run actually exists." : "");

			asprintf(&temp_plugin_output, "(Return code of %d is out of bounds%s : %s)",
					 queued_check_result->return_code, (queued_check_result->return_code == 126
														|| queued_check_result->return_code ==
														127) ? " - plugin may be missing" : "",
...
int handle_async_host_check_result(host * temp_host, check_result * queued_check_result)
{
	...
	log_debug_info(DEBUGL_CHECKS, 1,
				   "** Handling async check result for host '%s' from '%s'...\n",
	...
	/* parse check output to get: (1) short output, (2) long output, (3) perf data */
	parse_check_output(queued_check_result->output, &temp_host->plugin_output,
					   &temp_host->long_plugin_output, &temp_host->perf_data, TRUE, FALSE);
	...
	log_debug_info(DEBUGL_CHECKS, 2, "Long Output:	%s\n",
				   (temp_host->long_plugin_output ==
					NULL) ? "NULL" : temp_host->long_plugin_output);
	log_debug_info(DEBUGL_CHECKS, 2, "Perf Data:	%s\n",
	...
	process_host_check_result(temp_host, result, old_plugin_output, CHECK_OPTION_NONE,
...
#ifdef USE_EVENT_BROKER
	/* send data to event broker */
	broker_host_check(NEBTYPE_HOSTCHECK_PROCESSED, NEBFLAG_NONE, NEBATTR_NONE, temp_host,
					  temp_host->check_type, temp_host->current_state, temp_host->state_type,
					  start_time_hires, end_time_hires, temp_host->check_command,
					  temp_host->latency, temp_host->execution_time, host_check_timeout,
					  queued_check_result->early_timeout, queued_check_result->return_code,
					  NULL, temp_host->plugin_output, temp_host->long_plugin_output,
					  temp_host->perf_data, NULL, queued_check_result);
#endif
...
int process_host_check_result(host * hst, int new_state, char *old_plugin_output,
							  int check_options, int reschedule_check, int use_cached_result,
							  unsigned long check_timestamp_horizon)
{
	...
	log_debug_info(DEBUGL_CHECKS, 1,
				   ...
				   hst->name, hst->current_attempt, hst->max_attempts,
				   (hst->check_type == CHECK_TYPE_ACTIVE) ? "ACTIVE" : "PASSIVE",
				   (hst->state_type == HARD_STATE) ? "HARD" : "SOFT", hst->current_state,
				   new_state);
	...
	if (hst->current_state != HOST_UP) {
		...
		if (new_state == HOST_UP) {
			...
			reschedule_check = TRUE;
			next_check =
				(unsigned long)(current_time + (hst->check_interval * interval_length));
			...
			for (temp_hostsmember = hst->parent_hosts; temp_hostsmember != NULL;
				...
				if (parent_host->current_state != HOST_UP) {
					log_debug_info(DEBUGL_CHECKS, 1, "Check of parent host '%s' queued.\n",
								   parent_host->name);
					schedule_host_check(parent_host, current_time,
										CHECK_OPTION_DEPENDENCY_CHECK);
				}
			}
			...
			for (temp_hostsmember = hst->child_hosts; temp_hostsmember != NULL;
				 temp_hostsmember = temp_hostsmember->next) {
				...
				if (child_host->current_state != HOST_UP) {
					log_debug_info(DEBUGL_CHECKS, 1, "Check of child host '%s' queued.\n",
								   child_host->name);
					schedule_host_check(child_host, current_time,
										CHECK_OPTION_DEPENDENCY_CHECK);
				}
			}
				...
				if (hst->state_type == SOFT_STATE)
					next_check =
						(unsigned long)(current_time +
										(hst->retry_interval * interval_length));
	...
	/******* HOST WAS UP INITIALLY *******/
		...
		/***** HOST IS NOW DOWN/UNREACHABLE *****/
			...
			if (hst->check_type == CHECK_TYPE_ACTIVE || passive_host_checks_are_soft == TRUE)
				next_check =
					(unsigned long)(current_time + (hst->retry_interval * interval_length));
			...
			log_debug_info(DEBUGL_CHECKS, 1,
						   "Propagating checks to immediate parent hosts that are UP...\n");
			...
			log_debug_info(DEBUGL_CHECKS, 1,
						   "Propagating checks to immediate non-UNREACHABLE child hosts...\n");
			for (temp_hostsmember = hst->child_hosts; temp_hostsmember != NULL;
				 temp_hostsmember = temp_hostsmember->next) {
				child_host = temp_hostsmember->host_ptr;
			...
			if (enable_predictive_host_dependency_checks == TRUE
	...
	log_debug_info(DEBUGL_CHECKS, 1,
				   "Pre-handle_host_state() Host: %s, Attempt=%d/%d, Type=%s, Final State=%d (%s)\n",
				   hst->name, hst->current_attempt, hst->max_attempts,
				   (hst->state_type == HARD_STATE) ? "HARD" : "SOFT", hst->current_state,
				   host_state_name(hst->current_state));
	...
	log_debug_info(DEBUGL_CHECKS, 1,
				   "Post-handle_host_state() Host: %s, Attempt=%d/%d, Type=%s, Final State=%d (%s)\n",
				   hst->name, hst->current_attempt, hst->max_attempts,
				   (hst->state_type == HARD_STATE) ? "HARD" : "SOFT", hst->current_state,
				   host_state_name(hst->current_state));
	...
	if (hst->last_state == hst->current_state && should_stalk(hst)
	...
	if (reschedule_check == TRUE) {
		...
		if (hst->check_interval == 0
			&& (hst->state_type == HARD_STATE || hst->current_state == HOST_UP))
...
int check_host_check_viability(host * hst, int check_options, int *time_is_valid,
							   time_t * new_time)
{
	int 		result = OK;
	int 		perform_check = TRUE;
	...
	if (!(check_options & CHECK_OPTION_FORCE_EXECUTION)) {
		...
		if (check_time_against_period((unsigned long)current_time, hst->check_period_ptr) ==
			ERROR) {
...
int adjust_host_check_attempt(host * hst, int is_active)
{
	...
	log_debug_info(DEBUGL_CHECKS, 2,
				   ...
				   hst->name, hst->current_attempt, hst->max_attempts, hst->current_state,
				   hst->state_type);
	...
	/* check all parent hosts to see if we're DOWN or UNREACHABLE */
		...
		for (temp_hostsmember = hst->parent_hosts; temp_hostsmember != NULL;
			...
			log_debug_info(DEBUGL_CHECKS, 2, "	 Parent '%s' is %s\n", parent_host->name,
						   host_state_name(parent_host->current_state));
...
int handle_host_state(host * hst)
{
	...
	if (hst->last_state != hst->current_state
		|| (hst->current_state == HOST_UP && hst->state_type == SOFT_STATE))
		state_change = TRUE;
	...
	if (state_change == TRUE || hard_state_change == TRUE) {
		...
		if (hst->state_type == HARD_STATE
			|| (hst->state_type == SOFT_STATE && log_host_retries == TRUE))
			log_host_event(hst);
...
int parse_check_output(char *buf, char **short_output, char **long_output, char **perf_data,
					   int escape_newlines_please, int newlines_are_escaped)
{
	int 		current_line = 0;
	int 		eof = FALSE;
