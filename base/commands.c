...
"command file", "command file worker", 0, 0, NULL};
	...
	if (command_worker.pid) {
		...
		if (ret < 0) {
			logit(NSLOG_RUNTIME_ERROR, TRUE,
				  ...
				  command_worker.sd, nagios_iobs, iobroker_strerror(ret), errno,
				  strerror(errno));
			iocache_destroy(command_worker.ioc);
			goto err_close;
		}
...
int process_external_command1(char *cmd)
{
	...
	broker_external_command(NEBTYPE_EXTERNALCOMMAND_START, NEBFLAG_NONE, NEBATTR_NONE,
							command_type, entry_time, command_id, args, NULL);
#endif

	/* process the command */
	external_command_ret =
		(process_external_command2(command_type, entry_time, args) ==
		 OK) ? CMD_ERROR_OK : CMD_ERROR_FAILURE;
	if (external_command_ret != CMD_ERROR_OK) {
		logit(NSLOG_EXTERNAL_COMMAND | NSLOG_RUNTIME_WARNING, TRUE,
			  "Error: External command failed -> %s;%s\n", command_id, args);
	}

#ifdef USE_EVENT_BROKER
	/* send data to event broker */
	broker_external_command(NEBTYPE_EXTERNALCOMMAND_END, NEBFLAG_NONE, NEBATTR_NONE,
							command_type, entry_time, command_id, args, NULL);
#endif
...
int process_host_command(int cmd, time_t entry_time, char *args)
{
	...
	if ((author = my_strtok(NULL, ";")) != NULL) {
		if ((comment = my_strtok(NULL, ";")) != NULL) {
			...
			add_new_host_comment(USER_COMMENT, host_name, current_time, author, comment, FALSE,
...
int process_hostgroup_command(int cmd, time_t entry_time, char *args)
{
	...
	for (temp_member = temp_hostgroup->members; temp_member != NULL;
		...
		case CMD_ENABLE_HOSTGROUP_HOST_NOTIFICATIONS:
			enable_host_notifications(temp_host);
			if (comment)
				add_new_host_comment(USER_COMMENT, temp_host->name, current_time, author,
									 comment, FALSE, COMMENTSOURCE_EXTERNAL, FALSE, (time_t) 0,
									 NULL);
			break;
		...
		default:
			...
			for (temp_servicesmember = temp_host->services; temp_servicesmember != NULL;
				...
				case CMD_ENABLE_HOSTGROUP_SVC_NOTIFICATIONS:
					enable_service_notifications(temp_service);
					if (comment)
						add_new_service_comment(USER_COMMENT, temp_host->name,
												temp_service->description, current_time,
												author, comment, FALSE, COMMENTSOURCE_EXTERNAL,
												FALSE, (time_t) 0, NULL);
					break;
...
int process_service_command(int cmd, time_t entry_time, char *args)
{
	...
	if ((author = my_strtok(NULL, ";")) != NULL) {
		if ((comment = my_strtok(NULL, ";")) != NULL) {
			time_t		current_time = time(NULL);
			add_new_service_comment(USER_COMMENT, host_name, svc_description, current_time,
									author, comment, FALSE, COMMENTSOURCE_EXTERNAL, FALSE,
									(time_t) 0, NULL);
		}
	}
...
int process_servicegroup_command(int cmd, time_t entry_time, char *args)
{
	...
	case CMD_DISABLE_SERVICEGROUP_PASSIVE_SVC_CHECKS:
		...
		for (temp_member = temp_servicegroup->members; temp_member != NULL;
			...
			temp_service =
				find_service(temp_member->host_name, temp_member->service_description);
			...
			case CMD_ENABLE_SERVICEGROUP_SVC_NOTIFICATIONS:
				enable_service_notifications(temp_service);
				if (comment)
					add_new_service_comment(USER_COMMENT, temp_member->host_name,
											temp_service->description, current_time, author,
											comment, FALSE, COMMENTSOURCE_EXTERNAL, FALSE,
											(time_t) 0, NULL);
				break;
...
int cmd_add_comment(int cmd, time_t entry_time, char *args)
{
	...
	/* add the comment */
	result =
		add_new_comment((cmd == CMD_ADD_HOST_COMMENT) ? HOST_COMMENT : SERVICE_COMMENT,
						USER_COMMENT, host_name, svc_description, entry_time, user,
						comment_data, persistent, COMMENTSOURCE_EXTERNAL, FALSE, (time_t) 0,
						NULL);
	...
	if ((author = my_strtok(NULL, ";")) != NULL) {
		if ((comment = my_strtok(NULL, ";")) != NULL) {
			...
			if (cmd == CMD_DELAY_SVC_NOTIFICATION)
				...
				add_new_host_comment(USER_COMMENT, host_name, current_time, author, comment,
									 FALSE, COMMENTSOURCE_EXTERNAL, FALSE, (time_t) 0, NULL);
		}
	}
...
int cmd_schedule_check(int cmd, char *args)
{
	...
	if (cmd == CMD_SCHEDULE_HOST_CHECK || cmd == CMD_SCHEDULE_FORCED_HOST_CHECK
		|| cmd == CMD_SCHEDULE_HOST_SVC_CHECKS || cmd == CMD_SCHEDULE_FORCED_HOST_SVC_CHECKS) {
	...
	if (cmd == CMD_SCHEDULE_HOST_CHECK || cmd == CMD_SCHEDULE_FORCED_HOST_CHECK)
		schedule_host_check(temp_host, delay_time,
							(cmd ==
							 CMD_SCHEDULE_FORCED_HOST_CHECK) ? CHECK_OPTION_FORCE_EXECUTION :
							CHECK_OPTION_NONE);
	...
	/* schedule service checks */
		...
		for (temp_servicesmember = temp_host->services; temp_servicesmember != NULL;
			...
			schedule_service_check(temp_service, delay_time,
								   (cmd ==
									CMD_SCHEDULE_FORCED_HOST_SVC_CHECKS) ?
								   CHECK_OPTION_FORCE_EXECUTION : CHECK_OPTION_NONE);
		}
	} else
		schedule_service_check(temp_service, delay_time,
							   (cmd ==
								CMD_SCHEDULE_FORCED_SVC_CHECK) ? CHECK_OPTION_FORCE_EXECUTION :
							   CHECK_OPTION_NONE);
...
int cmd_schedule_host_service_checks(int cmd, char *args, int force)
{
	...
	for (temp_servicesmember = temp_host->services; temp_servicesmember != NULL;
		...
		schedule_service_check(temp_service, delay_time,
							   (force ==
								TRUE) ? CHECK_OPTION_FORCE_EXECUTION : CHECK_OPTION_NONE);
	}
...
int cmd_signal_process(int cmd, char *args)
{
	...
	if (!schedule_new_event
		((cmd == CMD_SHUTDOWN_PROCESS) ? EVENT_PROGRAM_SHUTDOWN : EVENT_PROGRAM_RESTART, TRUE,
		 scheduled_time, FALSE, 0, NULL, FALSE, NULL, NULL, 0))
		return ERROR;
...
int cmd_process_service_check_result(int cmd, time_t check_time, char *args)
{
	...
	result =
		process_passive_service_check(check_time, host_name, svc_description, return_code,
									  output);
...
int process_passive_service_check(time_t check_time, char *host_name, char *svc_description,
								  int return_code, char *output)
{
	check_result cr;
	host	   *temp_host = NULL;
	service    *temp_service = NULL;
	...
	cr.latency =
		(double)((double)(tv.tv_sec - check_time) + (double)(tv.tv_usec / 1000.0) / 1000.0);
	if (cr.latency < 0.0)
		cr.latency = 0.0;
...
int cmd_acknowledge_problem(int cmd, char *args)
{
	...
	/* acknowledge the service problem */
		...
		acknowledge_service_problem(temp_service, ack_author, ack_data, type, notify,
									persistent);
...
int cmd_schedule_downtime(int cmd, time_t entry_time, char *args)
{
	...
	case CMD_SCHEDULE_SVC_DOWNTIME:
		schedule_downtime(SERVICE_DOWNTIME, host_name, svc_description, entry_time, author,
	...
	case CMD_SCHEDULE_HOST_SVC_DOWNTIME:
		for (temp_servicesmember = temp_host->services; temp_servicesmember != NULL;
			...
			if ((temp_service = temp_servicesmember->service_ptr) == NULL)
				continue;
			schedule_downtime(SERVICE_DOWNTIME, host_name, temp_service->description,
	...
	case CMD_SCHEDULE_HOSTGROUP_HOST_DOWNTIME:
		for (temp_hgmember = temp_hostgroup->members; temp_hgmember != NULL;
			...
			schedule_downtime(HOST_DOWNTIME, temp_hgmember->host_name, NULL, entry_time,
	...
	case CMD_SCHEDULE_HOSTGROUP_SVC_DOWNTIME:
		for (temp_hgmember = temp_hostgroup->members; temp_hgmember != NULL;
			...
			for (temp_servicesmember = temp_host->services; temp_servicesmember != NULL;
				...
				if ((temp_service = temp_servicesmember->service_ptr) == NULL)
					continue;
				schedule_downtime(SERVICE_DOWNTIME, temp_service->host_name,
								  temp_service->description, entry_time, author, comment_data,
	...
	case CMD_SCHEDULE_SERVICEGROUP_HOST_DOWNTIME:
		...
		for (temp_sgmember = temp_servicegroup->members; temp_sgmember != NULL;
			...
			if (last_host == temp_host)
				continue;
			schedule_downtime(HOST_DOWNTIME, temp_sgmember->host_name, NULL, entry_time,
							  author, comment_data, start_time, end_time, fixed, triggered_by,
							  duration, &downtime_id);
			last_host = temp_host;
		}
	...
	case CMD_SCHEDULE_SERVICEGROUP_SVC_DOWNTIME:
		for (temp_sgmember = temp_servicegroup->members; temp_sgmember != NULL;
			 temp_sgmember = temp_sgmember->next)
			schedule_downtime(SERVICE_DOWNTIME, temp_sgmember->host_name,
							  temp_sgmember->service_description, entry_time, author,
	...
	case CMD_SCHEDULE_AND_PROPAGATE_HOST_DOWNTIME:
		...
		schedule_and_propagate_downtime(temp_host, entry_time, author, comment_data,
										start_time, end_time, fixed, 0, duration);
		break;
	...
	case CMD_SCHEDULE_AND_PROPAGATE_TRIGGERED_HOST_DOWNTIME:
		...
		schedule_and_propagate_downtime(temp_host, entry_time, author, comment_data,
										start_time, end_time, fixed, downtime_id, duration);
		break;
...
int cmd_delete_downtime_by_host_name(int cmd, char *args)
{
	...
	deleted =
		delete_downtime_by_hostname_service_description_start_time_comment(hostname,
																		   service_description,
																		   downtime_start_time,
																		   downtime_comment);
...
int cmd_delete_downtime_by_hostgroup_name(int cmd, char *args)
{
	...
	for (temp_member = temp_hostgroup->members; temp_member != NULL;
		...
		if (host_name != NULL && strcmp(temp_host->name, host_name) != 0)
			continue;
		deleted =
			+delete_downtime_by_hostname_service_description_start_time_comment(temp_host->
																				name,
																				service_description,
																				downtime_start_time,
																				downtime_comment);
...
int cmd_delete_downtime_by_start_time_comment(int cmd, char *args)
{
	...
	deleted =
		delete_downtime_by_hostname_service_description_start_time_comment(NULL, NULL,
																		   downtime_start_time,
																		   downtime_comment);
...
int cmd_change_object_int_var(int cmd, char *args)
{
	...
	case CMD_CHANGE_NORMAL_HOST_CHECK_INTERVAL:
		...
		if (old_dval == 0 && temp_host->checks_enabled == TRUE) {
			...
			time(&preferred_time);
			if (check_time_against_period(preferred_time, temp_host->check_period_ptr) ==
				ERROR) {
	...
	case CMD_CHANGE_MAX_HOST_CHECK_ATTEMPTS:
		...
		if (temp_host->state_type == HARD_STATE && temp_host->current_state != HOST_UP
	...
	case CMD_CHANGE_NORMAL_SVC_CHECK_INTERVAL:
		...
		if (old_dval == 0 && temp_service->checks_enabled == TRUE
			...
			/* schedule a check for right now (or as soon as possible) */
			time(&preferred_time);
			if (check_time_against_period(preferred_time, temp_service->check_period_ptr) ==
				ERROR) {
				get_next_valid_time(preferred_time, &next_valid_time,
	...
	case CMD_CHANGE_SVC_MODATTR:
		...
		broker_adaptive_service_data(NEBTYPE_ADAPTIVESERVICE_UPDATE, NEBFLAG_NONE,
									 NEBATTR_NONE, temp_service, cmd, attr,
									 temp_service->modified_attributes, NULL);
#endif
...
#ifdef USE_EVENT_BROKER
		/* send data to event broker */
		broker_adaptive_host_data(NEBTYPE_ADAPTIVEHOST_UPDATE, NEBFLAG_NONE, NEBATTR_NONE,
								  temp_host, cmd, attr, temp_host->modified_attributes, NULL);
#endif
...
#ifdef USE_EVENT_BROKER
		/* send data to event broker */
		broker_adaptive_contact_data(NEBTYPE_ADAPTIVECONTACT_UPDATE, NEBFLAG_NONE,
									 NEBATTR_NONE, temp_contact, cmd, attr,
									 temp_contact->modified_attributes, hattr,
									 temp_contact->modified_host_attributes, sattr,
									 temp_contact->modified_service_attributes, NULL);
#endif
...
int cmd_change_object_char_var(int cmd, char *args)
{
	...
	case CMD_CHANGE_GLOBAL_HOST_EVENT_HANDLER:
		...
		broker_adaptive_program_data(NEBTYPE_ADAPTIVEPROGRAM_UPDATE, NEBFLAG_NONE,
									 NEBATTR_NONE, cmd, attr, modified_host_process_attributes,
									 MODATTR_NONE, modified_service_process_attributes, NULL);
#endif
...
#ifdef USE_EVENT_BROKER
	/* send data to event broker */
	broker_adaptive_service_data(NEBTYPE_ADAPTIVESERVICE_UPDATE, NEBFLAG_NONE, NEBATTR_NONE,
								 svc, CMD_NONE, attr, svc->modified_attributes, NULL);
#endif
...
#ifdef USE_EVENT_BROKER
	/* send data to event broker */
	broker_adaptive_program_data(NEBTYPE_ADAPTIVEPROGRAM_UPDATE, NEBFLAG_NONE, NEBATTR_NONE,
								 CMD_NONE, attr, modified_host_process_attributes, attr,
								 modified_service_process_attributes, NULL);
#endif
...
#ifdef USE_EVENT_BROKER
	/* send data to event broker */
	broker_adaptive_host_data(NEBTYPE_ADAPTIVEHOST_UPDATE, NEBFLAG_NONE, NEBATTR_NONE, hst,
							  CMD_NONE, attr, hst->modified_attributes, NULL);
#endif
...
void enable_and_propagate_notifications(host * hst, int level, int affect_top_host,
										int affect_hosts, int affect_services)
{
	...
	for (temp_hostsmember = hst->child_hosts; temp_hostsmember != NULL;
		...
		/* recurse... */
		enable_and_propagate_notifications(child_host, level + 1, affect_top_host,
		...
		if (affect_services == TRUE) {
			for (temp_servicesmember = child_host->services; temp_servicesmember != NULL;
				 temp_servicesmember = temp_servicesmember->next) {
				if ((temp_service = temp_servicesmember->service_ptr) == NULL)
					continue;
				enable_service_notifications(temp_service);
...
void disable_and_propagate_notifications(host * hst, int level, int affect_top_host,
										 int affect_hosts, int affect_services)
{
	host	   *child_host = NULL;
	service    *temp_service = NULL;
	...
	for (temp_hostsmember = hst->child_hosts; temp_hostsmember != NULL;
		...
		/* recurse... */
		disable_and_propagate_notifications(child_host, level + 1, affect_top_host,
											affect_hosts, affect_services);
...
#ifdef USE_EVENT_BROKER
	/* send data to event broker */
	broker_adaptive_contact_data(NEBTYPE_ADAPTIVECONTACT_UPDATE, NEBFLAG_NONE, NEBATTR_NONE,
								 cntct, CMD_NONE, MODATTR_NONE, cntct->modified_attributes,
								 attr, cntct->modified_host_attributes, MODATTR_NONE,
								 cntct->modified_service_attributes, NULL);
#endif
...
void enable_contact_service_notifications(contact * cntct)
{
	...
	broker_adaptive_contact_data(NEBTYPE_ADAPTIVECONTACT_UPDATE, NEBFLAG_NONE, NEBATTR_NONE,
								 cntct, CMD_NONE, MODATTR_NONE, cntct->modified_attributes,
								 MODATTR_NONE, cntct->modified_host_attributes, attr,
								 cntct->modified_service_attributes, NULL);
#endif
...
void schedule_and_propagate_downtime(host * temp_host, time_t entry_time, char *author,
									 char *comment_data, time_t start_time, time_t end_time,
									 int fixed, unsigned long triggered_by,
									 unsigned long duration)
{
	...
	for (temp_hostsmember = temp_host->child_hosts; temp_hostsmember != NULL;
		...
		/* recurse... */
		schedule_and_propagate_downtime(child_host, entry_time, author, comment_data,
										start_time, end_time, fixed, triggered_by, duration);

		/* schedule downtime for this host */
		schedule_downtime(HOST_DOWNTIME, child_host->name, NULL, entry_time, author,
						  comment_data, start_time, end_time, fixed, triggered_by, duration,
						  NULL);
	}
...
void acknowledge_host_problem(host * hst, char *ack_author, char *ack_data, int type,
							  int notify, int persistent)
{
...
#ifdef USE_EVENT_BROKER
	/* send data to event broker */
	broker_acknowledgement_data(NEBTYPE_ACKNOWLEDGEMENT_ADD, NEBFLAG_NONE, NEBATTR_NONE,
								HOST_ACKNOWLEDGEMENT, (void *)hst, ack_author, ack_data, type,
								notify, persistent, NULL);
#endif
	...
	hst->acknowledgement_type =
		(type == ACKNOWLEDGEMENT_STICKY) ? ACKNOWLEDGEMENT_STICKY : ACKNOWLEDGEMENT_NORMAL;
	...
	time(&current_time);
	add_new_host_comment(ACKNOWLEDGEMENT_COMMENT, hst->name, current_time, ack_author,
						 ack_data, persistent, COMMENTSOURCE_INTERNAL, FALSE, (time_t) 0,
						 NULL);
...
void acknowledge_service_problem(service * svc, char *ack_author, char *ack_data, int type,
								 int notify, int persistent)
{
	time_t		current_time = 0L;
	...
	/* send data to event broker */
	broker_acknowledgement_data(NEBTYPE_ACKNOWLEDGEMENT_ADD, NEBFLAG_NONE, NEBATTR_NONE,
								SERVICE_ACKNOWLEDGEMENT, (void *)svc, ack_author, ack_data,
								type, notify, persistent, NULL);
#endif
	...
	/* add a comment for the acknowledgement */
	time(&current_time);
	add_new_service_comment(ACKNOWLEDGEMENT_COMMENT, svc->host_name, svc->description,
...
void start_executing_service_checks(void)
{
	...
	/* send data to event broker */
	broker_adaptive_program_data(NEBTYPE_ADAPTIVEPROGRAM_UPDATE, NEBFLAG_NONE, NEBATTR_NONE,
								 CMD_NONE, MODATTR_NONE, modified_host_process_attributes,
								 attr, modified_service_process_attributes, NULL);
#endif
...
void start_executing_host_checks(void)
{
	...
	/* send data to event broker */
	broker_adaptive_program_data(NEBTYPE_ADAPTIVEPROGRAM_UPDATE, NEBFLAG_NONE, NEBATTR_NONE,
								 CMD_NONE, attr, modified_host_process_attributes,
								 MODATTR_NONE, modified_service_process_attributes, NULL);
#endif
...
void clear_host_flapping_state(host * hst)
{
	...
	low_threshold =
		(hst->low_flap_threshold <= 0.0) ? low_host_flap_threshold : hst->low_flap_threshold;
	high_threshold =
		(hst->high_flap_threshold <=
		 0.0) ? high_host_flap_threshold : hst->high_flap_threshold;
	...
	if (hst->is_flapping) {
		...
		broker_flapping_data(NEBTYPE_FLAPPING_STOP, NEBFLAG_NONE, NEBATTR_FLAPPING_STOP_NORMAL,
							 HOST_FLAPPING, hst, hst->percent_state_change, high_threshold,
							 low_threshold, NULL);
#endif
...
void clear_service_flapping_state(service * svc)
{
	...
	low_threshold =
		(svc->low_flap_threshold <=
		 0.0) ? low_service_flap_threshold : svc->low_flap_threshold;
	high_threshold =
		(svc->high_flap_threshold <=
		 0.0) ? high_service_flap_threshold : svc->high_flap_threshold;
	...
	if (svc->is_flapping) {
		log_debug_info(DEBUGL_FLAPPING, 1,
					   "Service '%s' on host '%s' flapping state cleared.\n", svc->description,
					   svc->host_name);
		/* log a notice - this one is parsed by the history CGI */
		logit(NSLOG_INFO_MESSAGE, FALSE,
