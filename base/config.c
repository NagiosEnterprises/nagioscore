...
int read_main_config_file(char *main_config_file)
{
	...
	while (1) {
		...
		if (!strcmp(variable, "resource_file")) {
			...
			if ((tmpdir = opendir(check_result_path)) == NULL) {
				asprintf(&error_message, "Check result path '%s' is not a valid directory",
						 check_result_path);
				error = TRUE;
				break;
			}
		...
		/* skip external data directives */
			...
			if (service_skip_check_dependency_status < -1
				|| service_skip_check_dependency_status > 3) {
				asprintf(&error_message,
						 "Illegal value for service_skip_check_dependency_status");
				error = TRUE;
				break;
			}
			...
			if (service_skip_check_host_down_status < -1
				|| service_skip_check_host_down_status > 3) {
				asprintf(&error_message,
						 "Illegal value for service_skip_check_host_down_status");
				error = TRUE;
				break;
			}
		} else if (!strcmp(variable, "host_skip_check_dependency_status")) {
			host_skip_check_dependency_status = atoi(value);
			if (host_skip_check_dependency_status < -1
				|| host_skip_check_dependency_status > 3) {
				asprintf(&error_message,
						 "Illegal value for host_skip_check_dependency_status");
				error = TRUE;
				break;
			}
		}
	...
	/* handle errors */
	if (error == TRUE) {
		logit(NSLOG_CONFIG_ERROR, TRUE, "Error in configuration file '%s' - Line %d (%s)",
			  main_config_file, current_line,
...
int read_resource_file(char *resource_file)
{
	...
	while (1) {
		...
		if ((temp_ptr = my_strtok(input, "=")) == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  "Error: NULL variable - Line %d of resource file '%s'", current_line,
				  resource_file);
			error = TRUE;
			break;
		}
		...
		if ((temp_ptr = my_strtok(NULL, "\n")) == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  "Error: NULL variable value - Line %d of resource file '%s'", current_line,
				  resource_file);
			error = TRUE;
			break;
		}
...
int pre_flight_object_check(int *w, int *e)
{
	...
	buf1 = "temptraxe1";
	buf2 = "Probe 2";
	for (temp_se = get_first_serviceescalation_by_service(buf1, buf2, &ptr); temp_se != NULL;
		 temp_se = get_next_serviceescalation_by_service(buf1, buf2, &ptr)) {
		printf("FOUND ESCALATION FOR SVC '%s'/'%s': %d-%d/%.3f, PTR=%p\n", buf1, buf2,
			   temp_se->first_notification, temp_se->last_notification,
			   temp_se->notification_interval, ptr);
	}
	for (temp_he = get_first_hostescalation_by_host(buf1, &ptr); temp_he != NULL;
	...
	for (temp_service = service_list; temp_service != NULL; temp_service = temp_service->next) {
		...
		if (temp_service->event_handler != NULL) {
			...
			if (temp_service->event_handler_ptr == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE,
					  ...
					  temp_service->event_handler, temp_service->description,
		...
		if (temp_service->check_command_ptr == NULL) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE,
				  ...
				  temp_service->check_command, temp_service->description,
				  temp_service->host_name);
			errors++;
		}
		...
		for (sm = temp_service->parents; sm; sm = sm->next) {
			...
			if (sm->service_ptr == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE,
					  ...
					  sm->host_name, sm->service_description, temp_service->host_name,
		...
		if (temp_host->event_handler != NULL) {
			...
			if (temp_host->event_handler_ptr == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE,
					  ...
					  temp_host->event_handler, temp_host->name);
				errors++;
			}
		}
		...
		if (temp_host->check_command != NULL) {
			...
			if (temp_host->check_command_ptr == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE,
					  ...
					  temp_host->check_command, temp_host->name);
				errors++;
			}
		}
	...
	/*****************************************/
	for (temp_hostgroup = hostgroup_list, total_objects = 0; temp_hostgroup != NULL;
	...
	/*****************************************/
	for (temp_servicegroup = servicegroup_list, total_objects = 0; temp_servicegroup != NULL;
	...
	for (temp_contact = contact_list, total_objects = 0; temp_contact != NULL;
		...
		if (temp_contact->service_notification_commands == NULL) {
			...
			for (temp_commandsmember = temp_contact->service_notification_commands;
				...
				temp_commandsmember->command_ptr =
					find_bang_command(temp_commandsmember->command);
				if (temp_commandsmember->command_ptr == NULL) {
					logit(NSLOG_VERIFICATION_ERROR, TRUE,
						  ...
						  temp_commandsmember->command, temp_contact->name);
					errors++;
				}
			}
	...
	/*****************************************/
	for (temp_contactgroup = contactgroup_list, total_objects = 0; temp_contactgroup != NULL;
	...
	/* check all commands...				 */
	/*****************************************/
	for (temp_command = command_list, total_objects = 0; temp_command != NULL;
	...
	/*****************************************/
	for (temp_timeperiod = timeperiod_list, total_objects = 0; temp_timeperiod != NULL;
	...
	for (; olist; olist = olist->next) {
		...
		if (child_status == DFS_TEMP_CHECKED) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE,
				  ...
				  root->host_name, root->description, child->host_name, child->description);
			(*errors)++;
...
static int dfs_host_path(char *ary, host * root, int *errors)
{
	...
	for (child = root->child_hosts; child != NULL; child = child->next) {
		...
		if (child_status == DFS_TEMP_CHECKED) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE,
				  "Error: The hosts '%s' and '%s' form or lead into a circular parent/child chain!",
...
static int dfs_timeperiod_path(char *ary, timeperiod * root, int *errors)
{
	...
	for (exc = root->exclusions; exc; exc = exc->next) {
		...
		if (child_status == DFS_TEMP_CHECKED) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE,
				  "Error: The timeperiods '%s' and '%s' form or lead into a circular exclusion chain!",
...
int pre_flight_circular_check(int *w, int *e)
{
	...
	for (i = 0; i < num_objects.servicedependencies; i++) {
		...
		if (dep_type < 1 || dep_type > ARRAY_SIZE(ary))
			continue;
		dfs_servicedep_path(ary[dep_type - 1], temp_sd->dependent_service_ptr, dep_type,
							&errors);
	}
	if (verify_config)
