...
void init_timing_loop(void)
{
		...
		if (schedule_check == TRUE) {
			...
			scheduling_info.average_service_execution_time =
				(double)(((scheduling_info.average_service_execution_time *
						  ...
						  exec_time) / (double)scheduling_info.total_scheduled_services);
		} else {
	...
	for (temp_service = service_list;
		...
		for (interleave_block_index = 0;
			 ...
			 && temp_service != NULL; temp_service = temp_service->next) {
			...
			mult_factor =
				current_interleave_block + (interleave_block_index * total_interleave_blocks);

			log_debug_info(DEBUGL_EVENTS, 2, "CIB: %d, IBI: %d, TIB: %d, SIF: %d\n",
						   current_interleave_block, interleave_block_index,
						   total_interleave_blocks, scheduling_info.service_interleave_factor);
			 ...
			 */
			check_delay = mult_factor * scheduling_info.service_inter_check_delay;
			...
			is_valid_time =
				check_time_against_period(temp_service->next_check,
										  temp_service->check_period_ptr);
			if (is_valid_time == ERROR) {
				log_debug_info(DEBUGL_EVENTS, 2,
				...
				get_next_valid_time(temp_service->next_check, &next_valid_time,
									temp_service->check_period_ptr);
				temp_service->next_check = (time_t) (next_valid_time + check_delay);
			}
	...
	for (temp_service = service_list; temp_service != NULL; temp_service = temp_service->next) {
		...
		temp_service->next_check_event =
			schedule_new_event(EVENT_SERVICE_CHECK, FALSE, temp_service->next_check, FALSE, 0,
							   NULL, TRUE, (void *)temp_service, NULL,
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		is_valid_time =
			check_time_against_period(temp_host->next_check, temp_host->check_period_ptr);
		if (is_valid_time == ERROR) {
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		if (temp_host->should_be_scheduled == FALSE) {
			...
			/* passive checks are an exception if a forced check was scheduled before Nagios was restarted */
			if (!
				(temp_host->checks_enabled == FALSE && temp_host->next_check != (time_t) 0L
				 && (temp_host->check_options & CHECK_OPTION_FORCE_EXECUTION)))
				continue;
		}

		/* schedule a new host check event */
		temp_host->next_check_event =
			schedule_new_event(EVENT_HOST_CHECK, FALSE, temp_host->next_check, FALSE, 0, NULL,
							   TRUE, (void *)temp_host, NULL, temp_host->check_options);
	}
	...
	schedule_new_event(EVENT_CHECK_REAPER, TRUE, current_time + check_reaper_interval, TRUE,
	...
	if (check_orphaned_services == TRUE || check_orphaned_hosts == TRUE)
		schedule_new_event(EVENT_ORPHAN_CHECK, TRUE,
						   current_time + DEFAULT_ORPHAN_CHECK_INTERVAL, TRUE,
						   DEFAULT_ORPHAN_CHECK_INTERVAL, NULL, TRUE, NULL, NULL, 0);
	...
	if (check_service_freshness == TRUE)
		schedule_new_event(EVENT_SFRESHNESS_CHECK, TRUE,
						   ...
						   service_freshness_check_interval, NULL, TRUE, NULL, NULL, 0);
	...
	if (check_host_freshness == TRUE)
		schedule_new_event(EVENT_HFRESHNESS_CHECK, TRUE,
						   ...
						   host_freshness_check_interval, NULL, TRUE, NULL, NULL, 0);

	/* add a status save event */
	schedule_new_event(EVENT_STATUS_SAVE, TRUE, current_time + status_update_interval, TRUE,
					   status_update_interval, NULL, TRUE, NULL, NULL, 0);

	/* add a log rotation event if necessary */
	if (log_rotation_method != LOG_ROTATION_NONE)
		schedule_new_event(EVENT_LOG_ROTATION, TRUE, get_next_log_rotation_time(), TRUE, 0,
						   (void *)get_next_log_rotation_time, TRUE, NULL, NULL, 0);
	...
	if (retain_state_information == TRUE && retention_update_interval > 0)
		schedule_new_event(EVENT_RETENTION_SAVE, TRUE,
						   ...
						   (retention_update_interval * 60), NULL, TRUE, NULL, NULL, 0);
...
void display_scheduling_info(void)
{
	...
	printf("First scheduled check:			 %s",
		   (scheduling_info.total_scheduled_hosts ==
			0) ? "N/A\n" : ctime(&scheduling_info.first_host_check));
	printf("Last scheduled check:			 %s",
		   (scheduling_info.total_scheduled_hosts ==
	...
	minimum_concurrent_checks =
		ceil((((scheduling_info.total_scheduled_services /
				scheduling_info.average_service_check_interval)
			   +
			   (scheduling_info.total_scheduled_hosts /
				scheduling_info.average_host_check_interval))
	...
	if (((int)minimum_concurrent_checks > max_parallel_service_checks)
...
int event_execution_loop(void)
{
	...
	while (1) {
		...
		if (temp_event != last_event) {
			...
			log_debug_info(DEBUGL_EVENTS, 1, "Next Event Time: %s",
						   ctime(&temp_event->run_time));
			log_debug_info(DEBUGL_EVENTS, 1,
						   "Current/Max Service Checks: %d/%d (%.3lf%% saturation)\n",
						   currently_running_service_checks, max_parallel_service_checks,
						   ((float)currently_running_service_checks /
							(float)max_parallel_service_checks) * 100);
		}
		...
		log_debug_info(DEBUGL_SCHEDULING | DEBUGL_IPC, 1,
					   "## Polling %dms; sockets=%d; events=%u; iobs=%p\n", poll_time_ms,
					   iobroker_get_num_fds(nagios_iobs), squeue_size(nagios_squeue),
					   nagios_iobs);
		inputs = iobroker_poll(nagios_iobs, poll_time_ms);
		if (inputs < 0 && errno != EINTR) {
			logit(NSLOG_RUNTIME_ERROR, TRUE, "Error: Polling for input on %p failed: %s",
				  nagios_iobs, iobroker_strerror(inputs));
			break;
		}
...
int handle_timed_event(timed_event * event)
{
	...
	log_debug_info(DEBUGL_EVENTS, 0, "** Timed Event ** Type: EVENT_%s, Run Time: %s",
				   EVENT_TYPE_STR(event->event_type), ctime(&event->run_time));
	...
	case EVENT_ORPHAN_CHECK:
		...
		log_debug_info(DEBUGL_EVENTS, 0,
					   "** Orphaned Host and Service Check Event. Latency: %.3fs\n", latency);
	...
	case EVENT_SFRESHNESS_CHECK:
		...
		log_debug_info(DEBUGL_EVENTS, 0,
					   "** Service Result Freshness Check Event. Latency: %.3fs\n", latency);
	...
	case EVENT_HFRESHNESS_CHECK:
		...
		log_debug_info(DEBUGL_EVENTS, 0,
					   "** Host Result Freshness Check Event. Latency: %.3fs\n", latency);
...
void adjust_check_scheduling(void)
{
	...
	const double INTER_CHECK_RESCHEDULE_THRESHOLD =
		scheduling_info.service_inter_check_delay * 0.25;
	...
	if (!temp_pqueue) {
		logit(NSLOG_RUNTIME_ERROR, TRUE,
			  "Failed to allocate queue needed to adjust check scheduling.\n");
		return;
	}
	*temp_pqueue = *nagios_squeue;
	...
	if (!temp_pqueue->d) {
		logit(NSLOG_RUNTIME_ERROR, TRUE,
			  "Failed to allocate queue data needed to adjust check scheduling.\n");
		free(temp_pqueue);
		return;
	}
	...
	if (!events_to_reschedule) {
		logit(NSLOG_RUNTIME_ERROR, TRUE,
			  "Failed to allocate memory needed to adjust check scheduling.\n");
	...
	while ((sq_event = pqueue_pop(temp_pqueue))) {
		...
		if (last_check_tv.tv_sec > 0
			&& tv_delta_msec(&last_check_tv,
							 &sq_event->when) < INTER_CHECK_RESCHEDULE_THRESHOLD * 1000) {
...
*/ adjust_scheduling =
	...
	if (total_checks < 2 || !adjust_scheduling) {
		log_debug_info(DEBUGL_SCHEDULING, 0,
					   "No events need to be rescheduled (%d checks in %ds window).\n",
	...
	log_debug_info(DEBUGL_SCHEDULING, 0,
				   "Rescheduling events: %d checks in %ds window, ICD: %.3fs.\n", total_checks,
				   auto_rescheduling_window, inter_check_delay);
...
static void adjust_squeue_for_time_change(squeue_t ** q, int delta)
{
	...
	while ((event = squeue_pop(*q))) {
		...
		if (event->priority) {
			event->sq_event =
				squeue_add_usec(sq_new, event->run_time, event->priority - 1, event);
		} else {
...
void compensate_for_system_time_change(unsigned long last_time, unsigned long current_time)
{
	...
	for (temp_service = service_list; temp_service != NULL; temp_service = temp_service->next) {
		...
		adjust_timestamp_for_time_change(last_time, current_time, time_difference,
		...
		temp_service->next_notification =
			get_next_service_notification_time(temp_service, temp_service->last_notification);
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		temp_host->next_notification =
			get_next_host_notification_time(temp_host, temp_host->last_notification);
...
void adjust_timestamp_for_time_change(time_t last_time, time_t current_time,
									  unsigned long time_difference, time_t * ts)
{
