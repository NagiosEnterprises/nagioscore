...
void check_for_service_flapping(service * svc, int update, int allow_flapstart_notification)
{
	...
	curved_percent_change =
		(double)(((double)curved_changes * 100.0) / (double)(MAX_STATE_HISTORY_ENTRIES - 1));
							 ...
							 int allow_flapstart_notification)
{
	...
	if (update_history == TRUE && actual_check == FALSE
		&& (current_time - hst->last_state_history_update) < wait_threshold) {
					  ...
					  double low_threshold, int allow_flapstart_notification)
{
	...
	asprintf(&temp_buffer,
			 "Notifications for this service are being suppressed because it was detected as having been flapping between different states (%2.1f%% change >= %2.1f%% threshold).  When the service state stabilizes and the flapping stops, notifications will be re-enabled.",
			 percent_change, high_threshold);
	add_new_service_comment(FLAPPING_COMMENT, svc->host_name, svc->description, time(NULL),
							"(Nagios Process)", temp_buffer, 0, COMMENTSOURCE_INTERNAL, FALSE,
							(time_t) 0, &(svc->flapping_comment_id));
	my_free(temp_buffer);
	...
	broker_flapping_data(NEBTYPE_FLAPPING_START, NEBFLAG_NONE, NEBATTR_NONE, SERVICE_FLAPPING,
						 svc, percent_change, high_threshold, low_threshold, NULL);
#endif
...
void clear_service_flap(service * svc, double percent_change, double high_threshold,
						double low_threshold, int is_disabled)
{
	...
	if (is_disabled == 0) {
		...
		log_debug_info(DEBUGL_FLAPPING, 1,
					   "Disabled flap detection for service '%s' on host '%s'.\n",
					   svc->description, svc->host_name);
		/* log a notice - this one is parsed by the history CGI */
		logit(NSLOG_INFO_MESSAGE, FALSE,
			  "SERVICE FLAPPING ALERT: %s;%s;STOPPED; Disabled flap detection for service\n",
				   ...
				   double low_threshold, int allow_flapstart_notification)
{
	...
	asprintf(&temp_buffer,
			 "Notifications for this host are being suppressed because it was detected as having been flapping between different states (%2.1f%% change > %2.1f%% threshold).  When the host state stabilizes and the flapping stops, notifications will be re-enabled.",
			 percent_change, high_threshold);
	add_new_host_comment(FLAPPING_COMMENT, hst->name, time(NULL), "(Nagios Process)",
...
#ifdef USE_EVENT_BROKER
	/* send data to event broker */
	broker_flapping_data(NEBTYPE_FLAPPING_START, NEBFLAG_NONE, NEBATTR_NONE, HOST_FLAPPING,
						 hst, percent_change, high_threshold, low_threshold, NULL);
#endif
...
void clear_host_flap(host * hst, double percent_change, double high_threshold,
					 double low_threshold, int is_disabled)
{
	...
	if (is_disabled == 0) {
		...
		/* log a notice - this one is parsed by the history CGI */
		logit(NSLOG_INFO_MESSAGE, FALSE,
			  "HOST FLAPPING ALERT: %s;STOPPED; Disabled flap detection\n", hst->name);
	}
...
void handle_host_flap_detection_disabled(host * hst)
{
	...
	if (hst->is_flapping == TRUE) {
		...
		broker_flapping_data(NEBTYPE_FLAPPING_STOP, NEBFLAG_NONE,
							 NEBATTR_FLAPPING_STOP_DISABLED, HOST_FLAPPING, hst,
							 hst->percent_state_change, 0.0, 0.0, NULL);
#endif
...
void handle_service_flap_detection_disabled(service * svc)
{
	...
	if (svc->is_flapping == TRUE) {
		...
		/* send data to event broker */
		broker_flapping_data(NEBTYPE_FLAPPING_STOP, NEBFLAG_NONE,
							 NEBATTR_FLAPPING_STOP_DISABLED, SERVICE_FLAPPING, svc,
							 svc->percent_state_change, 0.0, 0.0, NULL);
#endif
