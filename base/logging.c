...
/* write something to the log file and syslog facility */
static void write_to_all_logs_with_timestamp(char *buffer, unsigned long data_type,
											 time_t * timestamp)
{
	/* write to syslog */
	write_to_syslog(buffer, data_type);
...
static FILE *open_log_file(void)
{
	...
	if ((fh =
		 open(log_file, O_RDWR | O_APPEND | O_CREAT | O_NOFOLLOW,
...
int log_service_event(service * svc)
{
	...
	asprintf(&temp_buffer, "SERVICE ALERT: %s;%s;%s;%s;%d;%s\n",
			 ...
			 svc->current_attempt, (svc->plugin_output == NULL) ? "" : svc->plugin_output);
...
int log_host_event(host * hst)
{
	...
	asprintf(&temp_buffer, "HOST ALERT: %s;%s;%s;%d;%s\n",
			 ...
			 hst->current_attempt, (hst->plugin_output == NULL) ? "" : hst->plugin_output);
...
int log_host_states(int type, time_t * timestamp)
{
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		asprintf(&temp_buffer, "%s HOST STATE: %s;%s;%s;%d;%s\n",
				 (type == INITIAL_STATES) ? "INITIAL" : "CURRENT", temp_host->name,
				 host_state_name(temp_host->current_state),
				 state_type_name(temp_host->state_type), temp_host->current_attempt,
...
int log_service_states(int type, time_t * timestamp)
{
	...
	for (temp_service = service_list; temp_service != NULL; temp_service = temp_service->next) {
		...
		asprintf(&temp_buffer, "%s SERVICE STATE: %s;%s;%s;%s;%d;%s\n",
				 ...
				 temp_service->current_attempt, temp_service->plugin_output);
...
int rotate_log_file(time_t rotation_time)
{
	...
	asprintf(&log_archive, "%s%snagios-%02d-%02d-%d-%02d.log", log_archive_path,
			 (log_archive_path[strlen(log_archive_path) - 1] == '/') ? "" : "/", t->tm_mon + 1,
	...
	if ((0 == archive_stat_result) || ((-1 == archive_stat_result) && (ENOENT != errno))) {
		return OK;
	}
...
int open_debug_log(void)
{
	...
	if ((fh =
		 open(debug_file, O_RDWR | O_APPEND | O_CREAT | O_NOFOLLOW, S_IRUSR | S_IWUSR)) == -1)
		return ERROR;
...
int log_debug_info(int level, int verbosity, const char *fmt, ...)
{
	...
	fprintf(debug_file_fp, "[%lu.%06lu] [%03d.%d] [pid=%lu] ", current_time.tv_sec,
			current_time.tv_usec, level, verbosity, (unsigned long)getpid());
