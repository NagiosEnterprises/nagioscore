...
static int nagios_core_worker(const char *path)
{
	...
	ret =
		nsock_printf_nul(sd, "@wproc register name=Core Worker %ld;pid=%ld", (long)getpid(),
						 (long)getpid());
	if (ret < 0) {
	...
	if (memcmp(response, "OK", 3)) {
		ret = read(sd, response + 3, sizeof(response) - 4);
		if (ret == (sizeof(response) - 4))
	...
	/* else start to monitor things... */
		...
		if (!nagios_binary_path) {
			logit(NSLOG_RUNTIME_ERROR, TRUE,
				  "Error: Unable to allocate memory for nagios_binary_path\n");
			exit(EXIT_FAILURE);
		}
		...
		do {
			...
			asprintf(&mac->x[MACRO_PROCESSSTARTTIME], "%llu",
...
#ifdef USE_EVENT_BROKER
				/* send program data to broker */
				broker_program_state(NEBTYPE_PROCESS_SHUTDOWN, NEBFLAG_PROCESS_INITIATED,
			...
			if (sigshutdown == TRUE)
				...
				broker_program_state(NEBTYPE_PROCESS_RESTART, NEBFLAG_USER_INITIATED,
									 NEBATTR_RESTART_NORMAL, NULL);
#endif
