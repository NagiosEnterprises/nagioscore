...
# define getopt(argc, argv, OPTSTR) getopt_long(argc, argv, OPTSTR, long_options, &option_index)
	...
	if (error == TRUE || display_help == TRUE) {
		...
		printf
			(" NUMACTHSTCHECKSxM	number of total active host checks occurring in last 1/5/15 minutes.\n");
		printf
			(" NUMOACTHSTCHECKSxM	number of on-demand active host checks occurring in last 1/5/15 minutes.\n");
		printf
		...
		printf
			(" NUMPSVHSTCHECKSxM	number of passive host checks occurring in last 1/5/15 minutes.\n");
		printf
			(" NUMACTSVCCHECKSxM	number of total active service checks occurring in last 1/5/15 minutes.\n");
		printf
			(" NUMOACTSVCCHECKSxM	number of on-demand active service checks occurring in last 1/5/15 minutes.\n");
		printf
			(" NUMCACHEDSVCCHECKSxM number of cached service checks occurring in last 1/5/15 minutes.\n");
		printf
...
static int display_mrtg_values(void)
{
	...
	for (temp_ptr = strtok(mrtg_variables, ","); temp_ptr != NULL;
		...
		/* active service check execution time */
			...
			printf("%d%s", (int)(average_active_service_execution_time * 1000),
				   mrtg_delimiter);
		...
		/* service states */
			...
			printf("%d%s", services_warning + services_unknown + services_critical,
				   mrtg_delimiter);
...
static int display_stats(void)
{
	...
	printf("Active Services Last 1/5/15/60 min: 	%d / %d / %d / %d\n",
		   ...
		   active_services_checked_last_15min, active_services_checked_last_1hour);
	...
	printf("Passive Services Last 1/5/15/60 min:	%d / %d / %d / %d\n",
		   ...
		   passive_services_checked_last_15min, passive_services_checked_last_1hour);
	...
	printf("   Scheduled:							%d / %d / %d\n",
		   active_scheduled_host_checks_last_1min, active_scheduled_host_checks_last_5min,
		   active_scheduled_host_checks_last_15min);
	printf("   On-demand:							%d / %d / %d\n",
		   active_ondemand_host_checks_last_1min, active_ondemand_host_checks_last_5min,
	...
	printf("   Cached:								%d / %d / %d\n",
		   active_cached_host_checks_last_1min, active_cached_host_checks_last_5min,
	...
	printf("   On-demand:							%d / %d / %d\n",
		   active_ondemand_service_checks_last_1min, active_ondemand_service_checks_last_5min,
		   active_ondemand_service_checks_last_15min);
	printf("   Cached:								%d / %d / %d\n",
		   active_cached_service_checks_last_1min, active_cached_service_checks_last_5min,
		...
		/* end of definition */
			...
			case STATUS_HOST_DATA:
				average_host_state_change =
					(((average_host_state_change * ((double)status_host_entries - 1.0)) +
					  state_change) / (double)status_host_entries);
				...
				if (check_type == CHECK_TYPE_ACTIVE) {
					...
					average_active_host_latency =
						(((average_active_host_latency * ((double)active_host_checks - 1.0)) +
						  latency) / (double)active_host_checks);
					...
					average_active_host_execution_time =
						(((average_active_host_execution_time *
						  ...
						  execution_time) / (double)active_host_checks);
					...
					average_active_host_state_change =
						(((average_active_host_state_change *
						   ((double)active_host_checks - 1.0)) +
						  state_change) / (double)active_host_checks);
					...
					average_passive_host_latency =
						(((average_passive_host_latency *
						  ...
						  latency) / (double)passive_host_checks);
					...
					average_passive_host_state_change =
						(((average_passive_host_state_change *
						   ((double)passive_host_checks - 1.0)) +
						  state_change) / (double)passive_host_checks);
			...
			case STATUS_SERVICE_DATA:
				average_service_state_change =
					(((average_service_state_change * ((double)status_service_entries - 1.0)) +
