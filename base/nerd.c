...
static int nerd_register_channel_callbacks(struct nerd_channel *chan)
{
	...
	for (i = 0; i < chan->num_callbacks; i++) {
		int 		result =
			neb_register_callback(chan->callbacks[i], &nerd_mod, 0, chan->handler);
		if (result != 0) {
...
int nerd_mkchan(const char *name, const char *description, int (*handler) (int, void *),
...
int nerd_init(void)
{
	...
	if (qh_register_handler
		("nerd", "Nagios Event Radio Dispatcher - Subscriber Service", 0,
		 nerd_qh_handler) < 0) {
	...
	chan_host_checks_id = nerd_mkchan("hostchecks",
									  "Host check results",
									  chan_host_checks,
									  nebcallback_flag(NEBCALLBACK_HOST_CHECK_DATA));
	chan_service_checks_id =
		nerd_mkchan("servicechecks", "Service check results", chan_service_checks,
					nebcallback_flag(NEBCALLBACK_SERVICE_CHECK_DATA));
	chan_opath_checks_id =
		nerd_mkchan("opathchecks", "Host and service checks in gource's log format",
					chan_opath_checks,
					nebcallback_flag(NEBCALLBACK_HOST_CHECK_DATA) |
					nebcallback_flag(NEBCALLBACK_SERVICE_CHECK_DATA));
