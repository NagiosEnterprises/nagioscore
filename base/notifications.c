...
int check_service_notification_viability(service *svc, int type, int options) {
	...
	if(type == NOTIFICATION_NORMAL && svc->first_notification_delay > 0 && svc->current_notification_number == 0 && svc->current_state != STATE_OK) {
		...
		if(current_time < (time_t)((svc->last_state_change == (time_t)0L) ? program_start : svc->last_state_change) + (time_t)(svc->first_notification_delay * interval_length)) {
...
int check_host_notification_viability(host *hst, int type, int options) {
	...
	/* check if delay of notifications is activated (ccztux) */
	if(type == NOTIFICATION_NORMAL && hst->first_notification_delay > 0 && hst->current_notification_number == 0 && hst->current_state != HOST_UP) {

		/* determine the time to use of the first problem point */
		if(current_time < (time_t)((hst->last_state_change == (time_t)0L) ? program_start : hst->last_state_change) + (time_t)(hst->first_notification_delay * interval_length)) {
