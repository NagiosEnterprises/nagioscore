...
static int qh_echo(int sd, char *buf, unsigned int len)
{
	...
	_unused_result(write(sd, buf, len));
	return 0;
}
	...
	if (!strcmp(buf, "list")) {
		for (qh = qhandlers; qh; qh = qh->next_qh) {
			nsock_printf(sd, "%-10s %s\n", qh->name,
						 qh->description ? qh->description : "(No description available)");
		}
		nsock_printf(sd, "%c", 0);
		return 0;
	}
	...
	if (!space && !strcmp(buf, "loadctl")) {
		nsock_printf_nul
			(sd, "jobs_max=%u;jobs_min=%u;"
			 ...
			 loadctl.nproc_limit, loadctl.nofile_limit, loadctl.options, loadctl.changes);
		return 0;
	}
