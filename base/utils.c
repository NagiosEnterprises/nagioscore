...
void init_main_cfg_vars(int first_time)
{
	...
	if (first_time) {
		...
		use_timezone = NULL;
		allow_empty_hostgroup_assignment = DEFAULT_ALLOW_EMPTY_HOSTGROUP_ASSIGNMENT;
		...
		host_perfdata_process_empty_results = DEFAULT_HOST_PERFDATA_PROCESS_EMPTY_RESULTS;
...
int my_system_r(nagios_macros * mac, char *cmd, int timeout, int *early_timeout,
				double *exectime, char **output, int max_output_length)
{
	pid_t		pid = 0;
	int 		status = 0;
	int 		result = 0;
	...
	*exectime = 0.0;
	...
	end_time.tv_sec = 0L;
	end_time.tv_usec = 0L;
	broker_system_command(NEBTYPE_SYSTEM_COMMAND_START, NEBFLAG_NONE, NEBATTR_NONE, start_time,
						  end_time, *exectime, timeout, *early_timeout, result, cmd, NULL,
						  NULL);
#endif
	...
	if (pid == -1) {
		logit(NSLOG_RUNTIME_WARNING, TRUE,
			  "Warning: fork() in my_system_r() failed for command \"%s\" - errno: %s\n", cmd,
			  strerror(errno));
	...
	/* parent waits for child to finish executing command */
		...
		*exectime =
			(double)((double)(end_time.tv_sec - start_time.tv_sec) +
		...
		broker_system_command(NEBTYPE_SYSTEM_COMMAND_END, NEBFLAG_NONE, NEBATTR_NONE,
							  start_time, end_time, *exectime, timeout, *early_timeout, result,
							  cmd, (output_dbuf.buf == NULL) ? NULL : output_dbuf.buf, NULL);
#endif
...
{
	return my_system_r(get_global_macros(), cmd, timeout, early_timeout, exectime, output,
					   max_output_length);
}


/* given a "raw" command, return the "expanded" or "whole" command line */
int get_raw_command_line_r(nagios_macros * mac, command * cmd_ptr, char *cmd,
						   char **full_command, int macro_options)
{
...
static timerange *_get_matching_timerange(time_t test_time, timeperiod * tperiod)
{
	...
	for (daterange_type = 0; daterange_type < DATERANGE_TYPES; daterange_type++) {
		...
		for (temp_daterange = tperiod->exceptions[daterange_type]; temp_daterange != NULL;
			...
			case DATERANGE_MONTH_DATE:
				start_time =
					calculate_time_from_day_of_month(test_time_year, temp_daterange->smon,
													 temp_daterange->smday);
				break;
			case DATERANGE_MONTH_DAY:
				start_time =
					calculate_time_from_day_of_month(test_time_year, test_time_mon,
													 temp_daterange->smday);
				break;
			case DATERANGE_MONTH_WEEK_DAY:
				start_time =
					calculate_time_from_weekday_of_month(test_time_year, temp_daterange->smon,
														 temp_daterange->swday,
														 temp_daterange->swday_offset);
				break;
			case DATERANGE_WEEK_DAY:
				start_time =
					calculate_time_from_weekday_of_month(test_time_year, test_time_mon,
														 temp_daterange->swday,
														 temp_daterange->swday_offset);
				break;
			default:
			...
			case DATERANGE_MONTH_DATE:
				year = test_time_year;
				end_time =
					calculate_time_from_day_of_month(year, temp_daterange->emon,
			...
			case DATERANGE_MONTH_WEEK_DAY:
				year = test_time_year;
				end_time =
					calculate_time_from_weekday_of_month(year, temp_daterange->emon,
			...
			if ((unsigned long)end_time == 0L) {
				...
				case DATERANGE_MONTH_DATE:
					...
					end_time =
						calculate_time_from_day_of_month(year, temp_daterange->emon, -1);
					break;
				case DATERANGE_MONTH_DAY:
					/* end date can't be helped, so skip it */
					if (temp_daterange->emday < 0)
					...
					/* else end date slipped past end of month, so use last day of month as end date */
					end_time =
						calculate_time_from_day_of_month(test_time_year, test_time_mon, -1);
					break;
				case DATERANGE_MONTH_WEEK_DAY:
					/* end date can't be helped, so skip it */
...
#ifdef TEST_TIMEPERIODS_A
			printf("NEW START:	  %llu = %s", (unsigned long long)start_time,
				   ctime(&start_time));
			 ...
			 * (fe. end-of-month) */
			if ((midnight + 84800UL >= start_time
				 && (midnight <= end_time || start_time > end_time)) || (midnight <= end_time
																		 && start_time >
																		 end_time)) {
#ifdef TEST_TIMEPERIODS_A
				printf("(MATCH)\n");
...
int check_time_against_period(time_t test_time, timeperiod * tperiod)
{
	...
	for (temp_timeperiodexclusion = tperiod->exclusions; temp_timeperiodexclusion != NULL;
		 temp_timeperiodexclusion = temp_timeperiodexclusion->next) {
		if (check_time_against_period(test_time, temp_timeperiodexclusion->timeperiod_ptr) ==
			OK) {
			return ERROR;
		}
	}

	for (temp_timerange = _get_matching_timerange(test_time, tperiod); temp_timerange != NULL;
...
static void _get_next_invalid_time(time_t pref_time, time_t * invalid_time,
								   timeperiod * tperiod)
{
	timeperiodexclusion *temp_timeperiodexclusion = NULL;
	int 		depth = 0;
	...
	while (earliest_time != last_earliest_time && depth < max_depth) {
		...
		for (; temp_timerange != NULL; temp_timerange = temp_timerange->next) {
			...
			printf("  INVALID RANGE START: %lu (%lu) = %s", temp_timerange->range_start,
				   (unsigned long)day_range_start, ctime(&day_range_start));
			printf("  INVALID RANGE END:   %lu (%lu) = %s", temp_timerange->range_end,
...
#ifdef TEST_TIMEPERIODS_B
				printf("	EARLIEST INVALID TIME: %llu = %s",
					   (unsigned long long)earliest_time, ctime(&earliest_time));
#endif
			}
		}
		...
		for (temp_timeperiodexclusion = tperiod->exclusions; temp_timeperiodexclusion != NULL;
			 temp_timeperiodexclusion = temp_timeperiodexclusion->next) {
			_get_next_valid_time(last_earliest_time, &potential_time,
								 temp_timeperiodexclusion->timeperiod_ptr);
			if (potential_time + 60 < earliest_time)
				earliest_time = potential_time + 60;
		}
	}
#ifdef TEST_TIMEPERIODS_B
	printf("	FINAL EARLIEST INVALID TIME: %llu = %s", (unsigned long long)earliest_time,
		   ctime(&earliest_time));
#endif
	...
	while (earliest_time != last_earliest_time && depth < max_depth) {
		...
		for (; temp_timerange != NULL; temp_timerange = temp_timerange->next) {
...
#ifdef TEST_TIMEPERIODS_B
			printf("  RANGE START: %lu (%lu) = %s", temp_timerange->range_start,
				   (unsigned long)day_range_start, ctime(&day_range_start));
			printf("  RANGE END:   %lu (%lu) = %s", temp_timerange->range_end,
				   (unsigned long)day_range_end, ctime(&day_range_end));
#endif
			...
			if (have_earliest_time == FALSE || potential_time < earliest_time) {
				earliest_time = potential_time;
#ifdef TEST_TIMEPERIODS_B
				printf("	EARLIEST TIME: %llu = %s", (unsigned long long)earliest_time,
					   ctime(&earliest_time));
#endif
			}
		...
		if (have_earliest_time == FALSE) {
			...
			for (temp_timeperiodexclusion = tperiod->exclusions;
				 temp_timeperiodexclusion != NULL;
				 temp_timeperiodexclusion = temp_timeperiodexclusion->next) {
				_get_next_invalid_time(earliest_time, &earliest_time,
									   temp_timeperiodexclusion->timeperiod_ptr);
#ifdef TEST_TIMEPERIODS_B
				printf("	FINAL EARLIEST TIME: %llu = %s", (unsigned long long)earliest_time,
					   ctime(&earliest_time));
#endif
			}
		}
...
"SEGV", "USR2", "PIPE", "ALRM", "TERM", "STKFLT", "CHLD", "CONT", "STOP", "TSTP", "TTIN", "TTOU",
"URG", "XCPU", "XFSZ", "VTALRM", "PROF", "WINCH", "IO", "PWR", "UNUSED", "ZERR", "DEBUG",
(char *)NULL };
	int 		x = 0;
...
void handle_sigxfsz(int sig)
{
	...
	if (SIGXFSZ == sig) {		/* Make sure we're handling the correct signal */
		...
		/* Try to figure out which file caused the signal and react
		   appropriately */
		for (x = 0, filep = files; x < (sizeof(files) / sizeof(files[0])); x++, filep++) {
...
static long long check_file_size(char *path, unsigned long fudge, struct rlimit rlim)
{
		...
		if (S_ISREG(status.st_mode)) {
			...
			if (status.st_size + fudge > rlim.rlim_cur) {
				logit(NSLOG_RUNTIME_ERROR, TRUE, "Size of file '%s' (%llu) "
					  ...
					  "ulimit(1).\n", path,
					  (unsigned long long)status.st_size, (unsigned long long)rlim.rlim_cur);
				return -1;
			} else {
...
int daemon_init(void)
{
	...
	if (lockfile < 0) {
		...
		logit(NSLOG_PROCESS_INFO | NSLOG_RUNTIME_ERROR, TRUE,
			  "Bailing out due to errors encountered while attempting to daemonize... (PID=%d)",
			  (int)getpid());
	...
	if (val > 0) {
		if ((val = sscanf(buf, "%d", &pidno)) < 1) {
			logit(NSLOG_RUNTIME_ERROR, TRUE, "Lockfile '%s' does not contain a valid PID (%s)",
				  lock_file, buf);
			cleanup();
			exit(ERROR);
		}
	...
	if (fcntl(lockfile, F_SETLK, &lock) < 0) {
		if (errno == EACCES || errno == EAGAIN) {
			...
			logit(NSLOG_RUNTIME_ERROR, TRUE, "Cannot lock lockfile '%s': %s. Bailing out...",
...
int drop_privileges(char *user, char *group)
{
	...
	if (gid != getegid()) {
		if (setgid(gid) == -1) {
			logit(NSLOG_RUNTIME_WARNING, TRUE, "Warning: Could not set effective GID=%d",
				  (int)gid);
			result = ERROR;
		}
	}
	...
	if (setuid(uid) == -1) {
		logit(NSLOG_RUNTIME_WARNING, TRUE, "Warning: Could not set effective UID=%d",
			  (int)uid);
		result = ERROR;
	}
	...
	if (result == OK && daemon_dumps_core == TRUE) {
		if (prctl(PR_SET_DUMPABLE, 1))
			logit(NSLOG_RUNTIME_WARNING, TRUE,
				  "Warning: Call to prctl(PR_SET_DUMPABLE, 1) failed with error %d", errno);
	}
#endif
...
int process_check_result_queue(char *dirname)
{
	...
	while ((dirfile = readdir(dirp)) != NULL) {
		...
		if (start + max_check_reaper_time < time(NULL)) {
			log_debug_info(DEBUGL_CHECKS, 0,
						   "Breaking out of check result reaper: max time (%ds) exceeded\n",
...
int process_check_result(check_result * cr)
{
	...
	if (cr->object_check_type == SERVICE_CHECK) {
		...
		log_debug_info(DEBUGL_CHECKS, 2,
					   "Processing check result for service '%s' on host '%s'\n",
	...
	logit(NSLOG_RUNTIME_ERROR, TRUE,
		  ...
		  cr->object_check_type, cr->host_name ? cr->host_name : "(null)",
...
int process_check_result_file(char *fname)
{
	...
	if ((thefile = mmap_fopen(fname)) == NULL) {
		...
		log_debug_info(DEBUGL_CHECKS, 1,
					   "Failed to open check result file for reading: '%s'\n", fname);
	...
	if (cr.host_name != NULL && cr.output != NULL) {
		...
		/* log a debug message */
		log_debug_info(DEBUGL_CHECKS, 1,
					   "Minimum amount of data not present; Skipped check result file: '%s'\n",
					   fname);
	}
	free_check_result(&cr);
...
int update_check_stats(int check_type, time_t check_time)
{
	...
	if ((((unsigned long)current_time -
		  (unsigned long)check_statistics[check_type].last_update) / 60) >
		CHECK_STATS_BUCKETS) {
		...
		printf("CLEARING ALL: TYPE[%d], CURRENT=%llu, LASTUPDATE=%llu\n", check_type,
			   ...
			   (unsigned long long)check_statistics[check_type].last_update);
#endif
	}
	...
	/* different current bucket number than last time */
		...
		for (x = check_statistics[check_type].current_bucket; x < (CHECK_STATS_BUCKETS * 2);
			 x++) {
...
int generate_check_stats(void)
{
	...
	for (check_type = 0; check_type < MAX_CHECK_STATS_TYPES; check_type++) {
		...
		/* different current bucket number than last time */
			...
			/* clear stats in buckets between last current bucket and new current bucket - stats haven't been updated in a while */
			for (x = check_statistics[check_type].current_bucket;
...
#ifdef DEBUG_CHECK_STATS
				printf
					("GEN CLEARING BUCKET %d, (NEW=%d, OLD=%d), CURRENT=%llu, LASTUPDATE=%llu\n",
					 this_bucket, new_current_bucket,
					 check_statistics[check_type].current_bucket,
					 (unsigned long long)current_time,
					 (unsigned long long)check_statistics[check_type].last_update);
			...
			printf("GEN NO CLEARING NEEDED: TYPE[%d], CURRENT=%llu, LASTUPDATE=%llu\n",
				   check_type, (unsigned long long)current_time,
				   (unsigned long long)check_statistics[check_type].last_update);
#endif
	...
	for (check_type = 0; check_type < MAX_CHECK_STATS_TYPES; check_type++) {
		...
		for (x = 0; x < CHECK_STATS_BUCKETS; x++) {
			...
			this_bucket =
				(check_statistics[check_type].current_bucket + CHECK_STATS_BUCKETS -
			...
			if (x == 0) {
				bucket_value =
					(int)(this_bucket_value + floor(last_bucket_value * last_bucket_weight));
			}
			...
			printf("X=%d, THIS[%d]=%d, LAST[%d]=%d, 1/5/15=%d,%d,%d  L=%d R=%d\n", x,
				   this_bucket, this_bucket_value, last_bucket, last_bucket_value,
				   check_statistics[check_type].minute_stats[0],
				   check_statistics[check_type].minute_stats[1],
				   check_statistics[check_type].minute_stats[2], left_value, right_value);
#endif
			/* record last update time */
			check_statistics[check_type].last_update = current_time;
		}
		...
		printf("TYPE[%d]   1/5/15 = %d, %d, %d (seconds=%d, this_weight=%f, last_weight=%f)\n",
			   check_type, check_statistics[check_type].minute_stats[0],
			   check_statistics[check_type].minute_stats[1],
			   check_statistics[check_type].minute_stats[2], seconds, this_bucket_weight,
			   last_bucket_weight);
#endif
	}
...
int check_for_nagios_updates(int force, int reschedule)
{
	...
	if (reschedule == TRUE) {
		...
		if (do_check == FALSE) {
			...
			next_check =
				next_check +
				(unsigned long)(((float)randnum / RAND_MAX) * UPDATE_CHECK_INTERVAL_WOBBLE);
		}
		...
		/* we tried to check for an update */
			...
			if (api_result == OK) {
				...
				next_check +=
					(unsigned long)(((float)randnum / RAND_MAX) *
									UPDATE_CHECK_INTERVAL_WOBBLE);
			}
			...
			/* query resulted in an error - retry at a shorter interval */
				...
				next_check +=
					(unsigned long)(((float)randnum / RAND_MAX) *
									UPDATE_CHECK_RETRY_INTERVAL_WOBBLE);
			}
		}
		...
		schedule_new_event(EVENT_CHECK_PROGRAM_UPDATE, TRUE, next_check, FALSE,
						   BASE_UPDATE_CHECK_INTERVAL, NULL, TRUE, NULL, NULL, 0);
	}
...
int query_update_api(void)
{
	...
	if (bare_update_check == FALSE) {
		...
		asprintf(&api_query2, "%s&version=%s%s", api_query, PROGRAM_VERSION,
				 (api_query_opts == NULL) ? "" : api_query_opts);
		my_free(api_query);
...
void cleanup(void)
{
	...
	if (test_scheduling == FALSE && verify_config == FALSE) {
		...
		neb_unload_all_modules(NEBMODULE_FORCE_UNLOAD,
							   (sigshutdown ==
								TRUE) ? NEBMODULE_NEB_SHUTDOWN : NEBMODULE_NEB_RESTART);
...
int reset_variables(void)
{
	...
	logging_options =
		NSLOG_RUNTIME_ERROR | NSLOG_RUNTIME_WARNING | NSLOG_VERIFICATION_ERROR |
		NSLOG_VERIFICATION_WARNING | NSLOG_CONFIG_ERROR | NSLOG_CONFIG_WARNING |
		NSLOG_PROCESS_INFO | NSLOG_HOST_NOTIFICATION | NSLOG_SERVICE_NOTIFICATION |
		NSLOG_EVENT_HANDLER | NSLOG_EXTERNAL_COMMAND | NSLOG_PASSIVE_CHECK | NSLOG_HOST_UP |
		NSLOG_HOST_DOWN | NSLOG_HOST_UNREACHABLE | NSLOG_SERVICE_OK | NSLOG_SERVICE_WARNING |
		NSLOG_SERVICE_UNKNOWN | NSLOG_SERVICE_CRITICAL | NSLOG_INFO_MESSAGE;
