...
option_help archive_json_help[] = {
	 ...
	 child_host_extras},
	{
	 ...
	 "services with the same description, depending on the value for " "service description.",
	 NULL},
	{
	 "hostgroup",
	 "Host Group",
...
int json_archive_alert_passes_selection(time_t, time_t, time_t, int, int,
										au_host *, char *, au_service *, char *, int,
										host *, int, host *, hostgroup *,
										servicegroup *, contact *, contactgroup *,
										unsigned, unsigned, unsigned, unsigned,
										unsigned);
json_object *json_archive_alert_selectors(unsigned, int, int, time_t, time_t, int, char *,
...
int json_archive_notification_passes_selection(time_t, time_t, time_t, int,
											   int, au_host *, char *, au_service *,
											   char *, int, host *, int, host *,
											   hostgroup *, servicegroup *,
											   au_contact *, char *, contactgroup *,
											   unsigned, unsigned, unsigned, char *,
											   char *);
json_object *json_archive_notification_selectors(unsigned, int, int, time_t, time_t, int,
...
int json_archive_statechange_passes_selection(time_t, time_t, int, int,
											  au_host *, char *, au_service *, char *,
											  unsigned, unsigned);
json_object *json_archive_statechange_selectors(unsigned, int, int, time_t, time_t, int,
												char *, char *, unsigned);
...
json_object *json_archive_single_host_availability(unsigned, time_t, time_t,
												   time_t, char *, timeperiod *, int, int, int,
												   int, unsigned, au_log *);
json_object *json_archive_single_service_availability(unsigned, time_t, time_t, time_t, char *,
													  char *, timeperiod *, int, int, int, int,
													  unsigned, au_log *);
json_object *json_archive_host_availability(unsigned, char *, au_availability *);
json_object *json_archive_service_availability(unsigned, char *, char *, au_availability *);
...
int main(void)
{
	...
	if (result != RESULT_SUCCESS) {
		json_object_append_object(json_root, "data", json_help(archive_json_help));
	...
	case ARCHIVE_QUERY_ALERTCOUNT:
		...
		json_object_append_object(json_root, "data",
								  json_archive_alertcount(cgi_data.format_options,
														  ...
														  cgi_data.state_types,
														  cgi_data.host_states,
														  cgi_data.service_states, log));
		break;
	case ARCHIVE_QUERY_ALERTLIST:
		...
		json_object_append_object(json_root, "data",
								  json_archive_alertlist(cgi_data.format_options,
	...
	case ARCHIVE_QUERY_AVAILABILITY:
		...
		json_object_append_object(json_root, "data",
								  json_archive_availability(cgi_data.format_options,
															query_time, cgi_data.start_time,
															cgi_data.end_time,
															cgi_data.object_type,
															cgi_data.host_name,
															cgi_data.service_description,
															...
															cgi_data.servicegroup,
															cgi_data.timeperiod,
															cgi_data.assume_initial_state,
															cgi_data.assume_state_retention,
															cgi_data.
															assume_states_during_nagios_downtime,
															cgi_data.
															assumed_initial_host_state,
															cgi_data.
															assumed_initial_service_state,
...
void free_cgi_data(archive_json_cgi_data * cgi_data)
{
	...
	if (NULL != cgi_data->notification_method)
		free(cgi_data->notification_method);
	if (NULL != cgi_data->timeperiod_name)
		free(cgi_data->timeperiod_name);
}


int process_cgivars(json_object * json_root, archive_json_cgi_data * cgi_data, time_t query_time)
{
	char	  **variables;
...
int validate_arguments(json_object * json_root, archive_json_cgi_data * cgi_data, time_t query_time)
{
	...
	if ((NULL != cgi_data->host_name) && (NULL != cgi_data->service_description)) {
...
int json_archive_alert_passes_selection(time_t timestamp, time_t start_time,
										...
										int match_object_types, au_host * current_host,
										char *match_host, au_service * current_service,
										char *match_service, int use_parent_host,
										...
										unsigned current_state_type,
										unsigned match_state_types, unsigned current_state,
										unsigned match_host_states,
										unsigned match_service_states)
{
	...
	case AU_OBJTYPE_HOST:
		...
		if ((1 == use_parent_host) && (NULL != current_host->hostp) &&
			FALSE == is_host_immediate_child_of_host(parent_host, current_host->hostp)) {
			return 0;
		}
		...
		if ((NULL != match_hostgroup) && (NULL != current_host->hostp) &&
			(FALSE == is_host_member_of_hostgroup(match_hostgroup, current_host->hostp))) {
			return 0;
		}
		...
		if ((NULL != match_contact) && (NULL != current_host->hostp) &&
			(FALSE == is_contact_for_host(current_host->hostp, match_contact))) {
			return 0;
		}
		...
		if ((1 == use_child_host) && (NULL != current_host->hostp)) {
			...
			if (NULL == child_host) {
				for (temp_host2 = host_list; temp_host2 != NULL; temp_host2 = temp_host2->next) {
					if (TRUE ==
						is_host_immediate_child_of_host(current_host->hostp, temp_host2)) {
						return 0;
					}
				}
			}
			/* Otherwise, skip this host if it does not have the specified host
			   as a child */
			else if (FALSE == is_host_immediate_child_of_host(current_host->hostp, child_host)) {
				return 0;
			}
		}
		break;
	case AU_OBJTYPE_SERVICE:
		...
		if ((NULL != current_service->servicep) && (FALSE ==
													is_authorized_for_service(current_service->
																			  servicep,
																			  &current_authdata)))
		{
			return 0;
		...
		if ((NULL != match_host) && strcmp(current_service->host_name, match_host)) {
			return 0;
		}

		/* If a specific service description was specified, skip this service
		   if it's description does not match the one specified */
		if ((NULL != match_service) && strcmp(current_service->description, match_service)) {
			return 0;
		}
		...
		if ((1 == use_parent_host) && (NULL != current_service->servicep)) {
			...
			if ((NULL != temp_host) && (FALSE ==
										is_host_immediate_child_of_host(parent_host,
																		temp_host))) {
				return 0;
			}
		}
		...
		if ((NULL != match_hostgroup) && (NULL != current_service->servicep)) {
			temp_host = find_host(current_service->servicep->host_name);
			if ((NULL != temp_host) && (FALSE ==
										is_host_member_of_hostgroup(match_hostgroup,
																	temp_host))) {
				return 0;
			}
		}
		...
		if ((NULL != match_servicegroup) && (NULL != current_service->servicep)) {
			...
			if ((NULL != temp_service) && (FALSE ==
										   ...
										   (match_servicegroup, temp_service))) {
				return 0;
			}
		}
		...
		if ((NULL != match_contact) && (NULL != current_service->servicep) &&
			(FALSE == is_contact_for_service(current_service->servicep, match_contact))) {
			return 0;
		}

		/* If the contact group was specified, skip this service if it does not
		   have the contact group specified */
		if ((NULL != match_contactgroup) && (NULL != current_service->servicep)) {
			found = 0;
			for (temp_contact_groupsmember =
...
json_object *json_archive_alert_selectors(unsigned format_options, int start,
										  int count, time_t start_time, time_t end_time,
										  int object_types, char *match_host,
										  char *match_service, int use_parent_host,
										  host * parent_host, int use_child_host,
										  host * child_host, hostgroup * match_hostgroup,
										  servicegroup * match_servicegroup,
										  ...
										  unsigned match_service_states)
{
	...
	if ((object_types & AU_OBJTYPE_HOST) && (match_host_states != AU_STATE_HOST_ALL)) {
		json_bitmask(json_selectors, format_options, "hoststates",
					 match_host_states, valid_host_states);
	}

	if ((object_types & AU_OBJTYPE_SERVICE) && (match_service_states != AU_STATE_SERVICE_ALL)) {
		json_bitmask(json_selectors, format_options, "servicestates",
					 match_service_states, valid_service_states);
	}

	if (NULL != match_host) {
		json_object_append_string(json_selectors, "hostname", &percent_escapes, match_host);
	}
...
json_object *json_archive_alertcount(unsigned format_options,
									 ...
									 char *match_host, char *match_service,
									 ...
									 unsigned match_service_states, au_log * log)
{
	...
	json_object_append_object(json_data, "selectors",
							  json_archive_alert_selectors(format_options, 0, 0, start_time,
														   end_time, object_types, match_host,
														   match_service, use_parent_host,
														   parent_host, use_child_host,
														   child_host, match_hostgroup,
														   match_servicegroup, match_contact,
														   match_contactgroup,
														   match_state_types,
														   match_host_states,
														   match_service_states));

	for (temp_node = log->entry_list->head; temp_node != NULL; temp_node = temp_node->next) {

		temp_entry = (au_log_entry *) temp_node->data;

		/* Skip all but alert type messages */
		if (AU_LOGTYPE_ALERT != temp_entry->entry_type)
			continue;
		...
		if (json_archive_alert_passes_selection(temp_entry->timestamp,
												...
												object_types, temp_host, match_host,
												temp_service, match_service, use_parent_host,
												parent_host, use_child_host, child_host,
												match_hostgroup, match_servicegroup,
												match_contact, match_contactgroup,
												temp_alert_log->state_type, match_state_types,
												temp_alert_log->state, match_host_states,
...
json_object *json_archive_alertlist(unsigned format_options, int start,
									int count, time_t start_time, time_t end_time,
									int object_types, char *match_host, char *match_service,
									int use_parent_host, host * parent_host,
									int use_child_host, host * child_host,
									hostgroup * match_hostgroup,
...
int json_archive_notification_passes_selection(time_t timestamp,
											   ...
											   int current_object_type, int match_object_types,
											   au_host * current_host, char *match_host,
											   au_service * current_service,
											   char *match_service, int use_parent_host,
											   host * parent_host, int use_child_host,
											   host * child_host, hostgroup * match_hostgroup,
											   servicegroup * match_servicegroup,
											   au_contact * current_contact,
											   char *match_contact,
											   contactgroup * match_contactgroup,
											   unsigned current_notification_type,
											   unsigned match_host_notification_types,
											   unsigned match_service_notification_types,
											   char *current_notification_method,
...
json_object *json_archive_notification_selectors(unsigned format_options,
												 int start, int count, time_t start_time,
												 time_t end_time, int match_object_types,
												 char *match_host, char *match_service,
												 int use_parent_host, host * parent_host,
												 int use_child_host, host * child_host,
												 hostgroup * match_hostgroup,
												 ...
												 char *match_notification_method)
{
	...
	if ((match_object_types & AU_OBJTYPE_HOST) &&
		...
		json_bitmask(json_selectors, format_options, "hostnotificationtypes",
					 match_host_notification_types, valid_host_notification_types);
	}
	...
	if ((match_object_types & AU_OBJTYPE_SERVICE) &&
		...
		json_bitmask(json_selectors, format_options, "servicenotificationtypes",
					 match_service_notification_types, valid_service_notification_types);
	}
	...
	if ((match_object_types & AU_OBJTYPE_SERVICE) && (NULL != match_servicegroup)) {
		json_object_append_string(json_selectors, "servicegroup",
								  &percent_escapes, match_servicegroup->group_name);
	}

	if (NULL != match_contact) {
		json_object_append_string(json_selectors, "contact", &percent_escapes, match_contact);
	}
...
json_object *json_archive_notificationcount(unsigned format_options,
											time_t start_time, time_t end_time,
											int match_object_types, char *match_host,
											char *match_service, int use_parent_host,
											host * parent_host, int use_child_host,
											host * child_host, hostgroup * match_hostgroup,
											servicegroup * match_servicegroup,
											...
											char *match_notification_method, au_log * log)
{
	...
	for (temp_node = log->entry_list->head; temp_node != NULL; temp_node = temp_node->next) {
		...
		if (AU_LOGTYPE_NOTIFICATION != temp_entry->entry_type)
			continue;
		...
		if (json_archive_notification_passes_selection(temp_entry->timestamp,
													   start_time, end_time,
													   temp_notification_log->obj_type,
													   match_object_types, temp_host,
													   match_host, temp_service, match_service,
													   use_parent_host, parent_host,
													   use_child_host, child_host,
													   match_hostgroup, match_servicegroup,
													   temp_notification_log->contact,
													   match_contact, match_contactgroup,
													   temp_notification_log->
													   notification_type,
													   match_host_notification_types,
													   match_service_notification_types,
													   temp_notification_log->method,
										   ...
										   char *match_notification_method, au_log * log)
{
	...
	for (temp_node = log->entry_list->head; temp_node != NULL; temp_node = temp_node->next) {
		...
		if (passes_start_and_count_limits(start, count, current, counted)) {
			...
			json_array_append_object(json_notificationlist, json_notification_details);
			counted++;
		}
		current++;
	}

	json_object_append_array(json_data, "notificationlist", json_notificationlist);
									   ...
									   au_log_notification * temp_notification)
{
	...
	json_enumeration(json_details, format_options, "object_type",
					 temp_notification->obj_type, svm_au_object_types);
...
int json_archive_statechange_passes_selection(time_t timestamp,
											  time_t end_time, int current_object_type,
											  int match_object_type, au_host * current_host,
											  char *match_host, au_service * current_service,
											  char *match_service, unsigned current_state_type,
											  unsigned match_state_types)
{
...
json_object *json_archive_statechange_selectors(unsigned format_options,
												int start, int count, time_t start_time,
												time_t end_time, int object_type,
...
json_object *json_archive_statechangelist(unsigned format_options,
										  int start, int count, time_t start_time,
										  time_t end_time, int object_type, char *host_name,
										  ...
										  unsigned state_types, au_log * log)
{
	...
	for (temp_node = log->entry_list->head; temp_node != NULL; temp_node = temp_node->next) {
		...
		if (json_archive_statechange_passes_selection(temp_entry->timestamp,
													  ...
													  object_type, temp_host, host_name,
													  temp_service, service_description,
													  temp_state_log->state_type,
													  state_types) == 0) {
			continue;
		}
		...
		if ((start_time > 0) && (temp_entry->timestamp < start_time)) {
			...
			case AU_OBJTYPE_HOST:
				start_log = au_create_alert_or_state_log(object_type,
														 temp_host, AU_STATETYPE_HARD,
														 initial_host_state,
														 "Initial Host Pseudo-State");
				break;
			case AU_OBJTYPE_SERVICE:
				start_log = au_create_alert_or_state_log(object_type,
														 temp_service, AU_STATETYPE_HARD,
			...
			if (start_log != NULL) {
				...
				json_array_append_object(json_statechangelist, json_statechange_details);
...
//						AU_LOGTYPE_STATE_INITIAL)) &&
	...
	case AU_OBJTYPE_HOST:
		...
		break;
	case AU_OBJTYPE_SERVICE:
		temp_service = au_find_service(log->hosts, host_name, service_description);
	...
	json_object_append_array(json_data, "statechangelist", json_statechangelist);
 ...
 */
int get_initial_nagios_state(au_linked_list * log_entries, time_t start_time, time_t end_time)
{
	...
	for (temp_node = log_entries->head; temp_node != NULL; temp_node = temp_node->next) {
 ...
 */
int get_initial_downtime_state(au_linked_list * log_entries, time_t start_time, time_t end_time)
{
 ...
 */
int get_initial_subject_state(au_linked_list * log_entries, time_t start_time, time_t end_time)
{
...
void print_availability(au_availability * availability, const char *padding)
{
	...
	printf("%sAll:: 		  up:		   %10lu\n", padding, availability->time_up);
	printf("%s				  down: 	   %10lu\n", padding, availability->time_down);
	printf("%s				  unreachable: %10lu\n", padding, availability->time_unreachable);
	printf("%s				  ok:		   %10lu\n", padding, availability->time_ok);
	printf("%s				  warning:	   %10lu\n", padding, availability->time_warning);
	printf("%s				  critical:    %10lu\n", padding, availability->time_critical);
	printf("%s				  unknown:	   %10lu\n", padding, availability->time_unknown);
	printf("%sScheduled::	  up:		   %10lu\n", padding, availability->scheduled_time_up);
	printf("%s				  down: 	   %10lu\n", padding,
		   availability->scheduled_time_down);
	printf("%s				  unreachable: %10lu\n", padding,
		   availability->scheduled_time_unreachable);
	printf("%s				  ok:		   %10lu\n", padding, availability->scheduled_time_ok);
	printf("%s				  warning:	   %10lu\n", padding,
...
void compute_window_availability(time_t start_time, time_t end_time,
								 int last_subject_state, int last_downtime_state,
								 int last_nagios_state, timeperiod * report_timeperiod,
								 ...
								 int assume_state_retention)
{
	...
	state_duration = calculate_window_duration(start_time, end_time, report_timeperiod);
	...
	for (temp_node = log_entries->head; temp_node != NULL; temp_node = temp_node->next) {
		...
		case AU_LOGTYPE_STATE_CURRENT:
			...
			printf("  Current alert state: \"%s\"\n",
				   svm_get_string_from_value(current_subject_state, valid_states));
#endif
			break;
		case AU_LOGTYPE_DOWNTIME:
			temp_downtime_log = (au_log_downtime *) current_log_entry->entry;
			...
			printf("  Current downtime state: \"%s\"\n",
				   svm_get_string_from_value(current_downtime_state, valid_states));
#endif
			break;
		case AU_LOGTYPE_NAGIOS:
			temp_nagios_log = (au_log_nagios *) current_log_entry->entry;
			...
			printf("  Current nagios state: \"%s\"\n",
				   svm_get_string_from_value(current_nagios_state, valid_states));
#endif
			break;
		default:
			continue;
		...
		compute_window_availability(last_time, current_time, last_subject_state,
									...
									availability, assume_state_during_nagios_downtime,
									object_type, assume_state_retention);
	...
	if (last_log_entry != NULL) {
		...
		if (last_log_entry->timestamp < end_time) {
			...
			compute_window_availability(last_time, current_time,
										last_subject_state, last_downtime_state,
										last_nagios_state, report_timeperiod, availability,
							   ...
							   int assume_state_retention)
{
	...
	if ((have_state_data(host->log_entries) == FALSE) &&
		(query_time > start_time) && (query_time <= end_time) && (host_status != NULL)) {
	...
	initial_host_state = get_initial_subject_state(host->log_entries, start_time, end_time);
#ifdef DEBUG
	printf("  Initial state from logs: %s\n",
		   svm_get_string_from_value(initial_host_state, valid_states));
#endif

	if ((AU_STATE_NO_DATA == initial_host_state) && (1 == assume_initial_state)) {
	...
	if ((have_real_data(host->log_entries) != TRUE) &&
		(AU_STATE_NO_DATA == initial_host_state))
		return;
								  ...
								  int assume_state_retention)
{
	...
	service_status = find_servicestatus(service->host_name, service->description);
	...
	if ((have_state_data(service->log_entries) == FALSE) &&
		(query_time > start_time) && (query_time <= end_time) && (service_status != NULL)) {
		...
		if (last_known_state != AU_STATE_NO_DATA) {
			...
			(void)au_add_alert_or_state_log(log, start_time,
											...
											service, AU_STATETYPE_HARD, last_known_state,
...
#endif
	if ((AU_STATE_NO_DATA == initial_service_state) && (1 == assume_initial_state)) {
	...
	if ((have_real_data(service->log_entries) != TRUE) &&
		(AU_STATE_NO_DATA == initial_service_state))
		return;
												 ...
												 unsigned state_types)
{
	...
	json_object_append_boolean(json_selectors, "assumeinitialstate", assume_initial_state);
	json_object_append_boolean(json_selectors, "assumestateretention", assume_state_retention);
...
json_object *json_archive_availability(unsigned format_options,
									   ...
									   int availability_object_type, char *host_name,
									   char *service_description,
									   hostgroup * hostgroup_selector,
									   servicegroup * servicegroup_selector,
									   timeperiod * report_timeperiod,
									   int assume_initial_state, int assume_state_retention,
									   int assume_state_during_nagios_downtime,
									   int assumed_initial_host_state,
									   int assumed_initial_service_state, unsigned state_types,
									   au_log * log)
{
	...
	case AU_OBJTYPE_HOST:
		if (NULL == host_name) {	/* compute for all hosts */
			...
			for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
				...
				json_host_object =
					json_archive_single_host_availability(format_options, query_time,
														  start_time, end_time,
				...
				if (NULL != json_host_object) {
					json_array_append_object(json_host_list, json_host_object);
				}
			}
			...
			if ((NULL != temp_host) &&
				...
				json_host_object =
					json_archive_single_host_availability(format_options,
														  ...
														  state_types, log);
				if (NULL != json_host_object) {
					json_object_append_object(json_data, "host", json_host_object);
				}
			}
		}
		break;
	case AU_OBJTYPE_SERVICE:
		if ((NULL == host_name) && (NULL == service_description)) {
			...
			for (temp_service = service_list; temp_service != NULL;
				...
				json_service_object =
					json_archive_single_service_availability(format_options, query_time,
															 start_time, end_time,
															 temp_service->host_name,
															 temp_service->description,
															 report_timeperiod,
				...
				if (NULL != json_service_object) {
					json_array_append_object(json_service_list, json_service_object);
				}
			}
			json_object_append_array(json_data, "services", json_service_list);
			...
			for (temp_service = service_list; temp_service != NULL;
				...
				if (!strcmp(temp_service->host_name, host_name) &&
					(TRUE == is_authorized_for_service(temp_service, &current_authdata))) {
					json_service_object =
						json_archive_single_service_availability(format_options, query_time,
																 start_time, end_time,
																 temp_service->host_name,
			...
			if ((NULL != temp_service) &&
				...
				json_service_object =
					json_archive_single_service_availability(format_options,
															 ...
															 state_types, log);
				if (NULL != json_service_object) {
					json_object_append_object(json_data, "service", json_service_object);
				}
			}
		}
		break;
	case AU_OBJTYPE_HOSTGROUP:
		if (NULL == hostgroup_selector) {
			...
			for (temp_hostgroup = hostgroup_list; temp_hostgroup != NULL;
				...
				for (temp_hostgroup_member = temp_hostgroup->members;
					...
					json_host_object =
						json_archive_single_host_availability(format_options, query_time,
															  start_time, end_time,
															  temp_hostgroup_member->host_name,
															  report_timeperiod,
															  assume_initial_state,
															  assume_state_retention,
															  assume_state_during_nagios_downtime,
				...
				json_object_append_array(json_hostgroup_object, "hosts", json_host_list);
				json_array_append_object(json_hostgroup_list, json_hostgroup_object);
			}
			json_object_append_array(json_data, "hostgroups", json_hostgroup_list);
		} else {
			...
			json_object_append_array(json_hostgroup_object, "hosts", json_host_list);
			json_object_append_object(json_data, "hostgroup", json_hostgroup_object);
		}
		break;
	case AU_OBJTYPE_SERVICEGROUP:
		if (NULL == servicegroup_selector) {
			...
			for (temp_servicegroup = servicegroup_list;
				...
				for (temp_servicegroup_member = temp_servicegroup->members;
					 ...
					 temp_servicegroup_member = temp_servicegroup_member->next) {
					if (FALSE ==
						is_authorized_for_service(temp_servicegroup_member->service_ptr,
												  &current_authdata)) {
						continue;
					}
					json_service_object =
				...
				json_object_append_array(json_servicegroup_object, "hosts", json_service_list);
				json_array_append_object(json_servicegroup_list, json_servicegroup_object);
			}
			json_object_append_array(json_data, "servicegroups", json_servicegroup_list);
		} else {
			...
			json_object_append_array(json_servicegroup_object, "services", json_service_list);
			json_object_append_object(json_data, "servicegroup", json_servicegroup_object);
		}
		break;
	default:
		break;
...
json_object *json_archive_single_host_availability(unsigned format_options,
												   time_t query_time, time_t start_time,
												   time_t end_time, char *host_name,
												   timeperiod * report_timeperiod,
												   int assume_initial_state,
												   int assume_state_retention,
												   int assume_state_during_nagios_downtime,
													  ...
													  unsigned state_types, au_log * log)
{
	...
	if ((service->availability = calloc(1, sizeof(au_availability))) != NULL) {
		...
		json_service_availability =
			json_archive_service_availability(format_options, service->host_name,
											...
											char *name, au_availability * availability)
{
	...
	if (name != NULL) {
		json_object_append_string(json_host_availability, "name", &percent_escapes, name);
	}

	json_object_append_duration(json_host_availability, "time_up", availability->time_up);
	json_object_append_duration(json_host_availability, "time_down", availability->time_down);
...
json_object *json_archive_service_availability(unsigned format_options,
											   char *host_name, char *description,
											   au_availability * availability)
{
	...
	json_object_append_duration(json_service_availability, "time_ok", availability->time_ok);
	...
	json_object_append_duration(json_service_availability,
								...
								availability->scheduled_time_warning);
	json_object_append_duration(json_service_availability, "scheduled_time_critical",
								availability->scheduled_time_critical);
	json_object_append_duration(json_service_availability, "scheduled_time_unknown",
								availability->scheduled_time_unknown);
	json_object_append_duration(json_service_availability, "time_indeterminate_nodata",
								availability->time_indeterminate_nodata);
	json_object_append_duration(json_service_availability, "time_indeterminate_notrunning",
								availability->time_indeterminate_notrunning);
