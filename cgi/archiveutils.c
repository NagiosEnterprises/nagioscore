...
int au_add_notification_log(au_log *, time_t, int, void *, au_contact *, int, char *, char *);
...
void *au_find_in_array(au_array *, void *, int (*cmp) (const void *, const void *));
					   ...
					   unsigned log_types, au_log * log, time_t * last_archive_data_update)
{
	...
	for (current_archive = newest_archive; current_archive <= oldest_archive;
		...
		if (stat(filename, &adstat) < 0) {
			...
			if (read_log_file(filename, obj_types, state_types, log_types, log) == 0) {
				  ...
				  unsigned log_types, au_log * log)
{
	...
	while (1) {
		...
		if (obj_types & AU_OBJTYPE_HOST) {
			...
			/* host initial states */
			else if ((log_types & AU_LOGTYPE_STATE) && strstr(input, "INITIAL HOST STATE:")) {
				if (parse_states_and_alerts(input, time_stamp,
											AU_LOGTYPE_STATE_INITIAL, AU_OBJTYPE_HOST,
											state_types, log) == 0) {
					retval = 0;
					break;
				}
			...
			/* host current states */
			else if ((log_types & AU_LOGTYPE_STATE) && strstr(input, "CURRENT HOST STATE:")) {
				if (parse_states_and_alerts(input, time_stamp,
											AU_LOGTYPE_STATE_CURRENT, AU_OBJTYPE_HOST,
											state_types, log) == 0) {
					retval = 0;
					break;
				}
			...
			/* scheduled downtime notices */
				...
				if (parse_downtime_alerts(input, time_stamp, AU_OBJTYPE_HOST, log) == 0) {
					retval = 0;
					break;
				}
			}
			...
			/* host notifications */
				...
				if (parse_notification_log(input, time_stamp, AU_OBJTYPE_HOST, log) == 0) {
					retval = 0;
					break;
				}
			}
		...
		if (obj_types & AU_OBJTYPE_SERVICE) {
			...
			if ((log_types & AU_LOGTYPE_ALERT) && strstr(input, "SERVICE ALERT:")) {
				if (parse_states_and_alerts(input, time_stamp, AU_LOGTYPE_ALERT,
			...
			else if ((log_types & AU_LOGTYPE_STATE) && strstr(input, "INITIAL SERVICE STATE:")) {
				if (parse_states_and_alerts(input, time_stamp,
			...
			else if ((log_types & AU_LOGTYPE_STATE) && strstr(input, "CURRENT SERVICE STATE:")) {
				if (parse_states_and_alerts(input, time_stamp,
			...
			/* scheduled service downtime notices */
				...
				if (parse_downtime_alerts(input, time_stamp, AU_OBJTYPE_SERVICE, log) == 0) {
					retval = 0;
					break;
				}
			}
			...
			/* service notifications */
				...
				if (parse_notification_log(input, time_stamp, AU_OBJTYPE_SERVICE, log) == 0) {
					retval = 0;
					break;
				}
			}
...
int au_cmp_log_entries(const void *a, const void *b)
{
	...
	if (lea->timestamp == leb->timestamp)
		return 0;
	else if (lea->timestamp < leb->timestamp)
		return -1;
	else
		return 1;
}

int au_add_nagios_log(au_log * log, time_t timestamp, int type, char *description)
{
	...
	if (au_list_add_node(global_host->log_entries, new_log_entry, au_cmp_log_entries) == 0) {
		return 0;
	}
...
void au_free_nagios_log(au_log_nagios * nagios_log)
{
	if (NULL == nagios_log)
		return;
	if (NULL != nagios_log->description)
		free(nagios_log->description);
	free(nagios_log);
}
							...
							unsigned obj_type, unsigned state_types, au_log * log)
{
	...
	case AU_OBJTYPE_SERVICE:
		...
		if (log->service_subjects->count > 0) {
			...
			if (temp_service_subject == NULL)
				return 1;
		}

		/* Find the au_service object associated with the service */
		temp_service = au_find_service(log->services, entry_host_name, entry_svc_description);
	...
	if (strstr(input, ";SOFT;")) {
		if (!(state_types & AU_STATETYPE_SOFT))
			return 1;
		state_type = AU_STATETYPE_SOFT;
	}
	if (strstr(input, ";HARD;")) {
		if (!(state_types & AU_STATETYPE_HARD))
			return 1;
	...
	case AU_OBJTYPE_HOST:
		...
		if (au_add_alert_or_state_log(log, timestamp, log_type, AU_OBJTYPE_HOST,
									  (void *)temp_host, state_type, state,
									  plugin_output) == 0) {
			return 0;
		}
		break;
	case AU_OBJTYPE_SERVICE:
		...
		if (au_add_alert_or_state_log(log, timestamp, log_type,
									  AU_OBJTYPE_SERVICE, (void *)temp_service, state_type,
									  state, plugin_output) == 0) {
			return 0;
		}
		break;
	}
...
void au_free_alert_log(au_log_alert * alert_log)
{
	if (NULL == alert_log)
		return;
	if (NULL != alert_log->plugin_output)
		free(alert_log->plugin_output);
	free(alert_log);
}

/* parse archive log downtime notifications */
int parse_downtime_alerts(char *input, time_t timestamp, unsigned obj_type, au_log * log)
{
...
{
	if (NULL == downtime_log)
		return;
	free(downtime_log);
}

int parse_notification_log(char *input, time_t timestamp, int obj_type, au_log * log)
{
	...
	if (NULL == temp_contact) {
		temp_contact = au_add_contact_and_sort(log->contacts, entry_contact_name);
	...
	case AU_OBJTYPE_SERVICE:
		if (au_add_notification_log(log, timestamp, AU_OBJTYPE_SERVICE,
									(void *)temp_service, temp_contact,
									notification_detail_type, method_name, temp_buffer) == 0) {
			return 0;
		}
		break;
	}
...
{
	if (NULL == notification_log)
		return;
	if (NULL != notification_log->method)
		free(notification_log->method);
	if (NULL != notification_log->message)
		free(notification_log->message);
	free(notification_log);
}

au_log_entry *au_add_log_entry(au_log * log, time_t timestamp, int entry_type, void *entry)
{
	...
	if (au_list_add_node(log->entry_list, (void *)new_log_entry, au_cmp_log_entries) == NULL) {
...
void au_free_host(au_host * host)
{
	...
	if (NULL != host->log_entries)
		au_free_list(host->log_entries, NULL);
	if (NULL != host->availability)
...
au_service *au_add_service(au_array * service_list, char *host_name, char *description)
{
	...
	snprintf(buf, sizeof(buf) - 1, "Service %s:%s log entries", host_name, description);
...
void au_free_service(au_service * service)
{
	if (NULL == service)
		return;
	if (NULL != service->host_name)
		free(service->host_name);
	if (NULL != service->description)
		free(service->description);
	/* Do not free the log entry data here because they are freed when the
	   master list's log entries are freed */
	if (NULL != service->log_entries)
		au_free_list(service->log_entries, NULL);
	if (NULL != service->availability)
		free(service->availability);
	free(service);
	return;
}

au_service *au_find_service(au_array * service_list, char *host_name, char *description)
{
	...
	found = (au_service *) au_find_in_array(service_list, (void *)&key, au_cmp_services);
	if (NULL == found)
		return NULL;
...
au_contact *au_find_contact(au_array * contact_list, char *name)
{
	...
	if (NULL == contact_list)
		return NULL;
	key.name = name;
	found = (au_contact *) au_find_in_array(contact_list, (void *)&key, au_cmp_contacts);
	if (NULL == found)
		return NULL;
