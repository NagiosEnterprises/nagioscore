...
int get_authentication_information(authdata * authinfo)
{
	...
	if ((thefile = mmap_fopen(get_cgi_config_location())) != NULL) {
		...
		while (1) {
			...
			if (!strcmp(authinfo->username, "")
				&& strstr(input, "default_user_name=") == input) {
				...
				if (strstr(input, "authorized_contactgroup_for_all_hosts=") == input) {
				...
				} else if (strstr(input, "authorized_contactgroup_for_system_information=") ==
						   input) {
					temp_ptr = strtok(input, "=");
					while ((temp_ptr = strtok(NULL, ","))) {
						temp_contactgroup = find_contactgroup(temp_ptr);
					...
					if (strstr(input, "authorized_contactgroup_for_configuration_information=")
						== input) {
					temp_ptr = strtok(input, "=");
					while ((temp_ptr = strtok(NULL, ","))) {
						temp_contactgroup = find_contactgroup(temp_ptr);
				...
				} else if (strstr(input, "authorized_contactgroup_for_all_host_commands=") ==
						   input) {
					temp_ptr = strtok(input, "=");
					while ((temp_ptr = strtok(NULL, ","))) {
						temp_contactgroup = find_contactgroup(temp_ptr);
				...
				} else if (strstr(input, "authorized_contactgroup_for_all_service_commands=")
						   == input) {
					temp_ptr = strtok(input, "=");
					while ((temp_ptr = strtok(NULL, ","))) {
						temp_contactgroup = find_contactgroup(temp_ptr);
				...
				} else if (strstr(input, "authorized_contactgroup_for_system_commands=") ==
						   input) {
					temp_ptr = strtok(input, "=");
					while ((temp_ptr = strtok(NULL, ","))) {
						temp_contactgroup = find_contactgroup(temp_ptr);
...
int is_authorized_for_hostgroup(hostgroup * hg, authdata * authinfo)
{
	...
	for (temp_hostsmember = hg->members; temp_hostsmember != NULL;
...
int is_authorized_for_servicegroup(servicegroup * sg, authdata * authinfo)
{
	 ...
	 */
	/* Reverted for 3.3.2 - must only be a member of one hostgroup */
	for (temp_servicesmember = sg->members; temp_servicesmember != NULL;
		 temp_servicesmember = temp_servicesmember->next) {
		temp_service =
			find_service(temp_servicesmember->host_name,
