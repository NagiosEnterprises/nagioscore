...
void reset_cgi_vars(void)
{
	...
	if (log_archive_path[strlen(log_archive_path) - 1] != '/'
		&& strlen(log_archive_path) < sizeof(log_archive_path) - 2)
...
int read_cgi_config_file(const char *filename, read_config_callback callback)
{
	...
	while (1) {
		...
		/* page limit added 2/1/2012 -MG */
			...
			if (physical_html_path[strlen(physical_html_path) - 1] != '/'
				&& (strlen(physical_html_path) < sizeof(physical_html_path) - 1))
				strcat(physical_html_path, "/");

			snprintf(physical_images_path, sizeof(physical_images_path), "%simages/",
					 physical_html_path);
			physical_images_path[sizeof(physical_images_path) - 1] = '\x0';

			snprintf(physical_ssi_path, sizeof(physical_images_path), "%sssi/",
					 physical_html_path);
			...
			snprintf(url_context_help_path, sizeof(url_context_help_path), "%scontexthelp/",
					 url_html_path);
			...
			snprintf(url_logo_images_path, sizeof(url_logo_images_path), "%slogos/",
					 url_images_path);
			url_logo_images_path[sizeof(url_logo_images_path) - 1] = '\x0';

			snprintf(url_stylesheets_path, sizeof(url_stylesheets_path), "%sstylesheets/",
					 url_html_path);
...
int read_main_config_file(const char *filename)
{
	...
	while (1) {
		...
		if (strstr(input, "interval_length=") == input) {
			...
			strncpy(log_archive_path, (temp_buffer == NULL) ? "" : temp_buffer,
					sizeof(log_archive_path));
			...
			temp_buffer = strtok(input, "=");
			temp_buffer = strtok(NULL, "\x0");
			strncpy(command_file, (temp_buffer == NULL) ? "" : temp_buffer,
					sizeof(command_file));
		...
		for (temp_servicesmember = temp_service->parents;
			...
			if ((parent_service = find_service(temp_servicesmember->host_name,
				...
				logit(NSLOG_CONFIG_ERROR, TRUE,
					  ...
					  temp_servicesmember->host_name, temp_servicesmember->service_description,
					  temp_service->host_name, temp_service->description);
			}
			/* add a reverse (child) link to make searches faster later on */
			if (add_child_link_to_service(parent_service, temp_service) == NULL) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
...
void cgi_init(void (*doc_header) (int), void (*doc_footer) (void), int object_options,
			  int status_options)
{
	int 		result;
...
void get_log_archive_to_use(int archive, char *buffer, int buffer_length)
{
	...
	snprintf(buffer, buffer_length, "%snagios-%02d-%02d-%d-%02d.log", log_archive_path,
...
void determine_log_rotation_times(int archive)
{
	...
	case LOG_ROTATION_HOURLY:
		...
		this_scheduled_log_rotation =
			(time_t) (this_scheduled_log_rotation - ((archive - 1) * 3600));
	...
	case LOG_ROTATION_DAILY:
		...
		this_scheduled_log_rotation = mktime(t);
		this_scheduled_log_rotation =
			(time_t) (this_scheduled_log_rotation - ((archive - 1) * 86400));
	...
	case LOG_ROTATION_WEEKLY:
		...
		this_scheduled_log_rotation = mktime(t);
		this_scheduled_log_rotation =
			(time_t) (this_scheduled_log_rotation - (86400 * t->tm_wday));
		this_scheduled_log_rotation =
			(time_t) (this_scheduled_log_rotation - ((archive - 1) * 604800));
...
void display_info_table(const char *title, int refresh, authdata * current_authdata)
{
	...
	printf
		("Nagios&reg; Core&trade; %s - <A HREF='https://www.nagios.org' TARGET='_new' CLASS='homepageURL'>www.nagios.org</A><BR>\n",
		 PROGRAM_VERSION);
...
void display_nav_table(char *url, int archive)
{
	...
	if (log_rotation_method != LOG_ROTATION_NONE) {
		...
		get_time_string(&last_scheduled_log_rotation, date_time, (int)sizeof(date_time),
						LONG_DATE_TIME);
		printf("%s", date_time);
		printf("<br>to<br>");
		if (archive == 0)
			...
			get_time_string(&this_scheduled_log_rotation, date_time, (int)sizeof(date_time),
							LONG_DATE_TIME);
			printf("%s", date_time);
		}
		printf("</td>\n");
...
void print_extra_hostgroup_url(char *group_name, char *url)
{
	...
	for (temp_buffer = my_strtok(input_buffer, "$"); temp_buffer != NULL;
		...
		if (in_macro == FALSE) {
			...
			if (strlen(output_buffer) + strlen(temp_buffer) < sizeof(output_buffer) - 1) {
				...
				if (!strcmp(temp_buffer, "HOSTGROUPNAME"))
					strncat(output_buffer, url_encode(temp_hostgroup->group_name),
							sizeof(output_buffer) - strlen(output_buffer) - 1);
			}
		...
		if (in_macro == FALSE) {
			...
			if (strlen(output_buffer) + strlen(temp_buffer) < sizeof(output_buffer) - 1) {
				...
				if (!strcmp(temp_buffer, "SERVICEGROUPNAME"))
					strncat(output_buffer, url_encode(temp_servicegroup->group_name),
							sizeof(output_buffer) - strlen(output_buffer) - 1);
			}
...
void include_ssi_files(const char *cgi_name, int type)
{
	...
	snprintf(common_ssi_file, sizeof(common_ssi_file) - 1, "%scommon-%s.ssi",
			 physical_ssi_path, (type == SSI_HEADER) ? "header" : "footer");
	...
	snprintf(cgi_ssi_file, sizeof(cgi_ssi_file) - 1, "%s%s-%s.ssi", physical_ssi_path,
			 (stripped_cgi_name == NULL) ? "" : stripped_cgi_name,
			 (type == SSI_HEADER) ? "header" : "footer");
...
void include_ssi_file(const char *filename)
{
	...
	/* an error occurred trying to stat() the file */
		...
		case ENAMETOOLONG:		/* File name too long. */
			printf("<br /> A stat call returned %d while looking for the file %s.<br />",
				   errno, filename);
			return;
...
void cgi_config_file_error(const char *config_file)
{
	...
	printf("<P>\n");
	printf
		("Make sure you read the documentation on installing and configuring Nagios thoroughly before continuing.  If all else fails, try sending a message to one of the mailing lists.  More information can be found at <a href='https://www.nagios.org'>https://www.nagios.org</a>.\n");
	printf("</P>\n");
...
void object_data_error(void)
{
	...
	printf("<P>\n");
	printf
		("Make sure you read the documentation on installing, configuring and running Nagios thoroughly before continuing.	If all else fails, try sending a message to one of the mailing lists.  More information can be found at <a href='https://www.nagios.org'>https://www.nagios.org</a>.\n");
	printf("</P>\n");
...
void display_splunk_generic_url(char *buf, int icon)
{
	...
	if (icon > 0)
		printf("<img src='%s%s' alt='Splunk It' title='Splunk It' border='0'>",
			   url_images_path,
			   (icon == 1) ? SPLUNK_SMALL_WHITE_ICON : SPLUNK_SMALL_BLACK_ICON);
	printf("</a>\n");
