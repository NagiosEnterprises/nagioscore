...
int process_cgivars(void) {
    ...
    for(x = 0; variables[x]; x++) {
        ...
        /* we found the string-to-expand argument */
            ...
            // If strtok() returns is NULL there are only spaces or nothing is there. 
            // Search for blank to avoid failing search.
            // TODO: If there is a search that requires spaces 
            //       between words this will need to be revisited.
            char *strip_tok = strtok(variables[x], " ");
            snprintf(to_expand, MAX_COMMAND_BUFFER, "%s", strip_tok != NULL ? strip_tok : "");
        }
