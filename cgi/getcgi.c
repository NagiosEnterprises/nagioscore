...
void sanitize_cgi_input(char **cgivars)
{
	...
	for (strptr = cgivars[i = 0]; strptr != NULL; strptr = cgivars[++i]) {
		...
		for (x = 0, y = 0; strptr[x] != '\x0'; x++) {
			...
			if (strptr[x] == ';' || strptr[x] == '|' || strptr[x] == '&' || strptr[x] == '<'
				|| strptr[x] == '>')
				keep = 0;
...
char	  **getcgivars(void)
{
	...
	if (!strcmp(request_method, "GET") || !strcmp(request_method, "HEAD")) {
		...
		if (strlen(content_type)
			&& strncasecmp(content_type, "application/x-www-form-urlencoded", 33)) {
...
accept_languages *parse_accept_languages(char *acceptlang)
{
	...
	while (langtok != NULL) {
		...
		if (NULL == langs->languages) {
			...
			if (NULL == (langs->languages = malloc(langs->count * sizeof(accept_language *)))) {
		...
		if (NULL == (langs->languages[langs->count - 1] = malloc(sizeof(accept_language)))) {
		...
		if (NULL != localitydelim) {
			...
			for (x = 0, stp = langs->languages[langs->count - 1]->locality;
				 x < (int)strlen(langs->languages[langs->count - 1]->locality); x++, stp++) {
				*stp = toupper(*stp);
			}
		...
		if (NULL == (langs->languages[langs->count - 1]->language = strdup(langtok))) {
