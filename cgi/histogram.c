...
int main(int argc, char **argv){
	...
	if(mode==CREATE_HTML && display_header==TRUE){
		...
		if(display_type!=DISPLAY_NO_HISTOGRAM && input_type==GET_INPUT_NONE){
			...
			printf("<IMG SRC='%s%s' BORDER=0 ALT='%s Event Histogram'>\n",url_images_path,TRENDS_ICON,(display_type==DISPLAY_HOST_HISTOGRAM)?"Host":"Service");
		...
		if(display_type!=DISPLAY_NO_HISTOGRAM && input_type==GET_INPUT_NONE){
			...
			printf("<option value=custom SELECTED>[ Current time range ]\n");
	...
	while(read_line(input_buffer,MAX_INPUT_BUFFER,fp)){
		...
		if(display_type==DISPLAY_HOST_HISTOGRAM){
			if(strstr(input_buffer,"HOST ALERT:")){
				...
				if(!(graph_statetypes & GRAPH_SOFT_STATETYPES) && strstr(input_buffer,";SOFT;"))
					continue;

				/* skip hard states if necessary */
				if(!(graph_statetypes & GRAPH_HARD_STATETYPES) && strstr(input_buffer,";HARD;"))
					continue;
