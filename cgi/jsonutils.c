...
const string_value_mapping svm_format_options[] = {
	...
	{"enumerate", JSON_FORMAT_ENUMERATE,
	 "Use textual representations of enumerated values rather than " "raw numeric values"},
	{"bitmask", JSON_FORMAT_BITMASK,
	 "Use textual representations of bitmask values rather than " "raw numeric values"},
...
void json_object_append_time(json_object * obj, char *key, unsigned long value)
{
	...
	json_object_append_string(obj, key, NULL, "%02u:%02u:%02u", hours, minutes, seconds);
}
...
void json_object_append_duration(json_object * obj, char *key, unsigned long value)
{
	json_object_member *mp;
					   ...
					   char *strftime_format, unsigned format_options)
{
	...
	padding++;
	for (x = 0, mpp = obj->members; x < obj->member_count; x++, mpp++) {
		json_member_print(*mpp, padding, whitespace, strftime_format, format_options);
		if (x != obj->member_count - 1)
			printf(",");
		if (whitespace)
			printf("\n");
	}
					   ...
					   char *strftime_format, unsigned format_options)
{
	...
	case JSON_TYPE_REAL:
		...
		break;
	case JSON_TYPE_TIME_T:
		json_time_t(padding, whitespace, mp->key, mp->value.time, strftime_format);
		break;
	case JSON_TYPE_STRING:
...
void indentf(int padding, int whitespace, char *format, ...)
{
	...
	if (whitespace > 0) {
		for (padvar = 0; padvar < padding; padvar++)
			printf("  ");
	}
	va_start(a_list, format);
...
json_object *json_result(time_t query_time, char *cgi, char *query,
						 int query_status, time_t last_data_update, authdata * authinfo,
						 int type, char *message, ...)
{
	...
	if (NULL != authinfo) {
		json_object_append_string(json_result, "user", &percent_escapes, authinfo->username);
	}
	if (NULL != query) {
		json_object_append_string(json_result, "query", &percent_escapes, query);
	...
	if (last_data_update != (time_t) - 1) {
		json_object_append_time_t(json_result, "last_data_update", last_data_update);
	}
...
json_object *json_help(option_help * help)
{
	...
	while (NULL != help->name) {
		...
		json_object_append_string(json_option, "label", &percent_escapes, (char *)help->label);
		json_object_append_string(json_option, "type", &percent_escapes, (char *)help->type);

		json_required = json_new_array();
		for (x = 0, stpp = (char **)help->required;
			 ((x < sizeof(help->required) /
			   sizeof(help->required[0])) && (NULL != *stpp)); x++, stpp++) {
			json_array_append_string(json_required, &percent_escapes, *stpp);
		}
		json_object_append_array(json_option, "required", json_required);

		json_optional = json_new_array();
		for (x = 0, stpp = (char **)help->optional;
			 ((x < sizeof(help->optional) /
			   sizeof(help->optional[0])) && (NULL != *stpp)); x++, stpp++) {
			json_array_append_string(json_optional, &percent_escapes, *stpp);
		}
		json_object_append_array(json_option, "optional", json_optional);
		...
		if (NULL != help->valid_values) {
			...
			for (svmp = (string_value_mapping *) help->valid_values;
				...
				if (NULL != svmp->description) {
					...
					json_object_append_object(json_validvalues, svmp->string, json_validvalue);
				} else {
					json_array_append_string(json_validvalues, &percent_escapes, svmp->string);
				}
			}
			json_object_append_object(json_option, "valid_values", json_validvalues);
		}
...
void json_boolean(int padding, int whitespace, char *key, int value)
{
	...
	if (NULL == key) {
		indentf(padding, whitespace, "%s", ((0 == value) ? "false" : "true"));
	} else {
		keybuf = json_escape_string(key, &string_escapes);
		indentf(padding, whitespace, "\"%s\":%s%s", keybuf,
				((whitespace > 0) ? " " : ""), ((0 == value) ? "false" : "true"));
	}
	if (NULL != keybuf)
		free(keybuf);
}
...
void json_unsigned(int padding, int whitespace, char *key, unsigned long long value)
{
...
void json_time(int padding, int whitespace, char *key, unsigned long value)
{
	...
	if (NULL == key) {
		indentf(padding, whitespace, "\"%02u:%02u:%02u\"", hours, minutes, seconds);
	} else {
		keybuf = json_escape_string(key, &string_escapes);
		indentf(padding, whitespace, "\"%s\":%s\"%02u:%02u:%02u\"", keybuf,
				((whitespace > 0) ? " " : ""), hours, minutes, seconds);
	}
	if (NULL != keybuf)
		free(keybuf);
}

void json_time_t(int padding, int whitespace, char *key, time_t value, char *format)
{
				   ...
				   int format_duration)
{
	...
	if (0 == format_duration) {
		...
		value -= minutes * 60;
		seconds = value;
		snprintf(buf, sizeof(buf) - 1, "%ud %uh %um %us", days, hours, minutes, seconds);
	}
					  ...
					  char *key, int value, const string_value_mapping * map)
{
	...
	if (format_options & JSON_FORMAT_ENUMERATE) {
		for (svmp = (string_value_mapping *) map; NULL != svmp->string; svmp++) {
			if (value == svmp->value) {
				json_object_append_string(json_parent, key, &percent_escapes, svmp->string);
				break;
			}
		}
		if (NULL == svmp->string) {
			json_object_append_string(json_parent, key, NULL, "Unknown value %d", svmp->value);
		}
	} else {
				  ...
				  int value, const string_value_mapping * map)
{
	...
	if (format_options & JSON_FORMAT_BITMASK) {
		...
		for (svmp = (string_value_mapping *) map; NULL != svmp->string; svmp++) {
			if (value & svmp->value) {
				json_array_append_string(json_bitmask_array, &percent_escapes, svmp->string);
			}
		}
		...
		if (NULL == svmp->string) {
			json_object_append_object(json_parent, "result",
									  json_result(query_time, cgi, query, query_status,
												  ...
												  "The %s option value '%s' is invalid.", key,
												  option));
	...
	if (NULL == (*var = strdup(value))) {
		json_object_append_object(json_parent, "result",
								  json_result(query_time, cgi, query, query_status,
											  ...
											  "Unable to allocate memory for %s option.",
											  key));
		...
		*var = 0;
		...
		json_object_append_object(json_parent, "result",
								  json_result(query_time, cgi, query, query_status,
											  (time_t) - 1, authinfo,
											  RESULT_OPTION_VALUE_INVALID,
											  "Value for %s option must be 'true' or 'false'.",
											  key));
		return RESULT_OPTION_VALUE_INVALID;
	}
...
char	   *json_escape_string(const char *src, const json_escape * escapes)
{
	...
	for (x = 0, escp = (json_escape_pair *) escapes->pairs; x < escapes->count; x++, escp++) {
		...
		while ((fromp = wcsstr(fromp, escp->from)) != NULL) {
			...
			if (from_len < to_len) {
				...
				memmove(fromp + to_len, fromp + from_len, wchars * sizeof(wchar_t));
				wcsncpy(fromp, escp->to, to_len);
