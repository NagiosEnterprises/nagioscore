...
option_help object_json_help[] = {
	 ...
	 svm_format_options},
	{
	 ...
	 "integer",
	 {NULL},
	 {"hostlist", "hostgrouplist", "servicelist", "servicegrouplist", "contactlist",
	  "contactgrouplist", "timeperiodlist", "commandlist", "servicedependencylist",
	  "serviceescalationlist", "hostdependencylist", "hostescalationlist", NULL},
	 NULL,
	 ...
	 child_host_extras},
	{
	 ...
	 {NULL},
	 {"hostlist", "hostgrouplist", "servicelist", "servicegrouplist", "contactlist",
	  "contactgrouplist", "timeperiodlist", "commandlist", NULL},
	 NULL,
	 ...
	 NULL},
	{
	 ...
	 {"hostgroup", NULL},
	 {"hostcount", "hostlist", "servicecount", "servicelist", "hostescalationcount",
	  "hostescalationlist", "serviceescalationcount", "serviceescalationlist", NULL},
	 NULL,
	 ...
	 child_service_extras},
	{
	 ...
	 {"contactgroup", NULL},
	 {"hostcount", "hostlist", "servicecount", "servicelist", "contactcount", "contactlist",
	  "serviceescalationcount", "serviceescalationlist", "hostescalationcount",
	  "hostescalationlist", NULL},
	 NULL,
	 ...
	 NULL},
	{
	 ...
	 {"contact", NULL},
	 {"hostcount", "hostlist", "servicecount", "servicelist", "serviceescalationcount",
	  "serviceescalationlist", "hostescalationcount", "hostescalationlist", NULL},
	 NULL,
	 ...
	 NULL},
	{
	 ...
	 "nagios:objectjson/hostlist",
	 {NULL},
	 {"hostdependencycount", "hostdependencylist", "servicedependencycount",
	  "servicedependencylist", NULL},
	 NULL,
...
int json_object_host_passes_selection(host *, int, host *, int, host *, hostgroup *,
									  contact *, contactgroup *, timeperiod *,
									  timeperiod *, command *, command *);
json_object *json_object_host_selectors(int, int, int, host *, int, host *, hostgroup *,
										contact *, contactgroup *, timeperiod *, timeperiod *,
										command *, command *);
...
int json_object_service_passes_service_selection(service *, servicegroup *,
												 ...
												 timeperiod *, command *, command *);
json_object *json_object_service_selectors(int, int, int, host *, int, host *, hostgroup *,
										   host *, servicegroup *, contact *, char *, char *,
...
int json_object_servicedependency_passes_selection(servicedependency *, host *,
												   hostgroup *, char *, servicegroup *,
												   host *, hostgroup *, char *,
												   servicegroup *);
json_object *json_object_servicedependency_selectors(int, int, host *, hostgroup *, char *,
													 servicegroup *, host *, hostgroup *,
													 char *, servicegroup *);

int json_object_serviceescalation_passes_selection(serviceescalation *, host *,
												   char *, hostgroup *, servicegroup *,
												   contact *, contactgroup *);
json_object *json_object_serviceescalation_selectors(int, int, host *, char *, hostgroup *,
...
int json_object_hostescalation_passes_selection(hostescalation *, host *, hostgroup *,
												contact *, contactgroup *);
json_object *json_object_hostescalation_selectors(int, int, host *, hostgroup *, contact *,
												  contactgroup *);
...
int main(void)
{
	...
	json_object_append_integer(json_root, "format_version", OUTPUT_FORMAT_VERSION);
	...
	if (result != RESULT_SUCCESS) {
		...
		json_object_print(json_root, 0, 1, cgi_data.strftime_format, cgi_data.format_options);
		document_footer();
		return result;
	}
	...
	case OBJECT_QUERY_HOSTCOUNT:
		...
		json_object_append_object(json_root, "data",
								  json_object_hostcount(cgi_data.use_parent_host,
														cgi_data.parent_host,
														cgi_data.use_child_host,
														cgi_data.child_host,
														cgi_data.hostgroup, cgi_data.contact,
	...
	case OBJECT_QUERY_HOSTLIST:
		...
		json_object_append_object(json_root, "data",
								  json_object_hostlist(cgi_data.format_options, cgi_data.start,
													   ...
													   cgi_data.use_parent_host,
													   cgi_data.parent_host,
													   cgi_data.use_child_host,
													   cgi_data.child_host, cgi_data.hostgroup,
													   cgi_data.contact, cgi_data.contactgroup,
													   cgi_data.check_timeperiod,
													   cgi_data.host_notification_timeperiod,
													   cgi_data.check_command,
													   cgi_data.event_handler));
	...
	case OBJECT_QUERY_SERVICELIST:
		json_object_append_object(json_root, "result",
								  json_result(query_time, THISCGI,
											  ...
											  RESULT_SUCCESS, ""));
		json_object_append_object(json_root, "data",
								  json_object_servicelist(cgi_data.format_options,
														  cgi_data.start, cgi_data.count,
														  cgi_data.details, cgi_data.host,
														  cgi_data.use_parent_host,
														  cgi_data.parent_host,
														  cgi_data.use_child_host,
														  cgi_data.child_host,
	...
	case OBJECT_QUERY_SERVICE:
		json_object_append_object(json_root, "result",
								  json_result(query_time, THISCGI,
											  ...
											  RESULT_SUCCESS, ""));
		json_object_append_object(json_root, "data",
								  json_object_service(cgi_data.format_options,
													  cgi_data.service));
		break;
	...
	case OBJECT_QUERY_CONTACTLIST:
		json_object_append_object(json_root, "result",
								  json_result(query_time, THISCGI,
											  ...
											  RESULT_SUCCESS, ""));
		json_object_append_object(json_root, "data",
								  json_object_contactlist(cgi_data.format_options,
														  cgi_data.start, cgi_data.count,
														  cgi_data.details,
														  cgi_data.contactgroup,
														  cgi_data.
	...
	case OBJECT_QUERY_CONTACT:
		json_object_append_object(json_root, "result",
								  json_result(query_time, THISCGI,
											  ...
											  RESULT_SUCCESS, ""));
		json_object_append_object(json_root, "data",
								  json_object_contact(cgi_data.format_options,
													  cgi_data.contact));
		break;
	...
	case OBJECT_QUERY_TIMEPERIODCOUNT:
		json_object_append_object(json_root, "result",
								  json_result(query_time, THISCGI,
											  ...
											  last_object_cache_update, &current_authdata,
											  RESULT_SUCCESS, ""));
		json_object_append_object(json_root, "data", json_object_timeperiodcount());
		break;
	...
	case OBJECT_QUERY_COMMANDLIST:
		json_object_append_object(json_root, "result",
								  json_result(query_time, THISCGI,
											  ...
											  RESULT_SUCCESS, ""));
		json_object_append_object(json_root, "data",
								  json_object_commandlist(cgi_data.format_options,
														  cgi_data.start, cgi_data.count,
														  cgi_data.details));
		break;
	case OBJECT_QUERY_COMMAND:
		json_object_append_object(json_root, "result",
								  json_result(query_time, THISCGI,
											  ...
											  RESULT_SUCCESS, ""));
		json_object_append_object(json_root, "data",
								  json_object_command(cgi_data.format_options,
													  cgi_data.command));
		break;
	case OBJECT_QUERY_SERVICEDEPENDENCYCOUNT:
		...
		json_object_append_object(json_root, "data",
								  json_object_servicedependencycount(cgi_data.master_host,
																	 cgi_data.master_hostgroup,
																	 cgi_data.
																	 master_service_description,
																	 cgi_data.
																	 master_servicegroup,
																	 cgi_data.dependent_host,
																	 cgi_data.
																	 dependent_hostgroup,
																	 cgi_data.
	...
	case OBJECT_QUERY_HELP:
		json_object_append_object(json_root, "result",
								  json_result(query_time, THISCGI,
											  ...
											  &current_authdata, RESULT_SUCCESS, ""));
		json_object_append_object(json_root, "data", json_help(object_json_help));
		break;
	default:
		json_object_append_object(json_root, "result",
...
{
	if (NULL != cgi_data->strftime_format)
		free(cgi_data->strftime_format);
	if (NULL != cgi_data->parent_host_name)
		free(cgi_data->parent_host_name);
	if (NULL != cgi_data->child_host_name)
		free(cgi_data->child_host_name);
	if (NULL != cgi_data->host_name)
		free(cgi_data->host_name);
	if (NULL != cgi_data->hostgroup_member_name)
		free(cgi_data->hostgroup_member_name);
	if (NULL != cgi_data->hostgroup_name)
		free(cgi_data->hostgroup_name);
	if (NULL != cgi_data->servicegroup_name)
		free(cgi_data->servicegroup_name);
	if (NULL != cgi_data->service_description)
		free(cgi_data->service_description);
	if (NULL != cgi_data->servicegroup_member_host_name)
		free(cgi_data->servicegroup_member_host_name);
	if (NULL != cgi_data->servicegroup_member_service_description)
		free(cgi_data->servicegroup_member_service_description);
	if (NULL != cgi_data->parent_service_name)
		free(cgi_data->parent_service_name);
	if (NULL != cgi_data->child_service_name)
		free(cgi_data->child_service_name);
	if (NULL != cgi_data->contactgroup_name)
		free(cgi_data->contactgroup_name);
	if (NULL != cgi_data->contact_name)
		free(cgi_data->contact_name);
	if (NULL != cgi_data->contactgroup_member_name)
		free(cgi_data->contactgroup_member_name);
	if (NULL != cgi_data->timeperiod_name)
		free(cgi_data->timeperiod_name);
	if (NULL != cgi_data->check_timeperiod_name)
		free(cgi_data->check_timeperiod_name);
	if (NULL != cgi_data->host_notification_timeperiod_name)
		free(cgi_data->host_notification_timeperiod_name);
	if (NULL != cgi_data->service_notification_timeperiod_name)
		free(cgi_data->service_notification_timeperiod_name);
	if (NULL != cgi_data->command_name)
		free(cgi_data->command_name);
	if (NULL != cgi_data->check_command_name)
		free(cgi_data->check_command_name);
	if (NULL != cgi_data->event_handler_name)
		free(cgi_data->event_handler_name);
	if (NULL != cgi_data->master_host_name)
		free(cgi_data->master_host_name);
	if (NULL != cgi_data->master_hostgroup_name)
		free(cgi_data->master_hostgroup_name);
	if (NULL != cgi_data->master_service_description)
		free(cgi_data->master_service_description);
	if (NULL != cgi_data->master_servicegroup_name)
		free(cgi_data->master_servicegroup_name);
	if (NULL != cgi_data->dependent_host_name)
		free(cgi_data->dependent_host_name);
	if (NULL != cgi_data->dependent_hostgroup_name)
		free(cgi_data->dependent_hostgroup_name);
	if (NULL != cgi_data->dependent_service_description)
		free(cgi_data->dependent_service_description);
	if (NULL != cgi_data->dependent_servicegroup_name)
		free(cgi_data->dependent_servicegroup_name);
}


int process_cgivars(json_object * json_root, object_json_cgi_data * cgi_data, time_t query_time)
{
	char	  **variables;
	...
	for (x = 0; variables[x] != NULL; x++) {
		...
		if (!strcmp(variables[x], "query")) {
			if ((result = parse_enumeration_cgivar(THISCGI,
												   svm_get_string_from_value(cgi_data->query,
																			 valid_queries),
												   get_query_status(query_status,
																	cgi_data->query),
												   json_root, query_time, authinfo,
												   variables[x], variables[x + 1],
												   valid_queries, &(cgi_data->query)))
				!= RESULT_SUCCESS) {
				break;
			}
			x++;
			...
			if (cgi_data->count == 0) {
				json_object_append_object(json_root, "result",
										  json_result(query_time, THISCGI,
													  svm_get_string_from_value(cgi_data->
																				query,
																				valid_queries),
													  get_query_status(query_status,
																	   cgi_data->query),
			...
			if ((result = parse_string_cgivar(THISCGI,
											  ...
											  variables[x + 1],
											  &(cgi_data->
												servicegroup_member_service_description)))
				!= RESULT_SUCCESS) {
				break;
			}
			x++;
			...
			if ((result = parse_string_cgivar(THISCGI,
											  ...
											  json_root, query_time, authinfo, variables[x],
											  variables[x + 1],
											  &(cgi_data->master_service_description)))
				!= RESULT_SUCCESS) {
				break;
			}
			x++;
...
int validate_arguments(json_object * json_root, object_json_cgi_data * cgi_data, time_t query_time)
{
	int 		result = RESULT_SUCCESS;
	host	   *temp_host = NULL;
	...
	default:
		...
		json_object_append_object(json_root, "result", json_result(query_time,
																   ...
																   (time_t) - 1, authinfo,
																   result,
																   "Missing validation for object type %u.",
																   cgi_data->query));
		break;
	}
	...
	if ((NULL != cgi_data->service_description) && (NULL != cgi_data->host_name)) {
		temp_service = find_service(cgi_data->host_name, cgi_data->service_description);
	...
	/* Validate the requested servicegroup member host name and servic
	   description */
	if (NULL != cgi_data->servicegroup_member_host_name ||
	...
	if (NULL != cgi_data->host_notification_timeperiod_name) {
		temp_timeperiod = find_timeperiod(cgi_data->host_notification_timeperiod_name);
		if (NULL == temp_timeperiod) {
			result = RESULT_OPTION_VALUE_INVALID;
			json_object_append_object(json_root, "result",
									  json_result(query_time, THISCGI,
												  ...
												  cgi_data->
												  host_notification_timeperiod_name));
		} else {
	...
	if (NULL != cgi_data->service_notification_timeperiod_name) {
		temp_timeperiod = find_timeperiod(cgi_data->service_notification_timeperiod_name);
		if (NULL == temp_timeperiod) {
			result = RESULT_OPTION_VALUE_INVALID;
			json_object_append_object(json_root, "result",
									  json_result(query_time, THISCGI,
	...
	if (NULL != cgi_data->master_servicegroup_name) {
		temp_servicegroup = find_servicegroup(cgi_data->master_servicegroup_name);
		if (NULL == temp_servicegroup) {
			result = RESULT_OPTION_VALUE_INVALID;
			json_object_append_object(json_root, "result",
									  json_result(query_time, THISCGI,
...
int json_object_host_passes_selection(host * temp_host, int use_parent_host,
									  host * parent_host, int use_child_host,
									  ...
									  command * check_command, command * event_handler)
{
	...
	if (NULL != temp_hostgroup &&
		(FALSE == is_host_member_of_hostgroup(temp_hostgroup, temp_host))) {
		return 0;
	}

	/* If the contact was specified, skip this host if it does not have
	   the contact specified */
	if (NULL != temp_contact && (FALSE == is_contact_for_host(temp_host, temp_contact))) {
		return 0;
	}
	...
	if (NULL != check_timeperiod && (check_timeperiod != temp_host->check_period_ptr)) {
		return 0;
	}
	...
	if (NULL != check_command && (check_command != temp_host->check_command_ptr)) {
		return 0;
	}

	/* If an event handler was specified, skip this host if it does not
	   have the event handler specified */
	if (NULL != event_handler && (event_handler != temp_host->event_handler_ptr)) {
		return 0;
	}
...
json_object *json_object_host_selectors(int start, int count,
										int use_parent_host, host * parent_host,
										int use_child_host, host * child_host,
								   ...
								   command * check_command, command * event_handler)
{
	...
	json_object_append_object(json_data, "selectors",
							  json_object_host_selectors(0, 0, use_parent_host, parent_host,
														 use_child_host, child_host,
														 temp_hostgroup, temp_contact,
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		if (json_object_host_passes_selection(temp_host, use_parent_host,
											  parent_host, use_child_host, child_host,
											  temp_hostgroup, temp_contact, temp_contactgroup,
...
json_object *json_object_hostlist(unsigned format_options, int start,
								  int count, int details, int use_parent_host,
								  ...
								  command * check_command, command * event_handler)
{
	...
	json_object_append_object(json_data, "selectors",
							  json_object_host_selectors(start, count, use_parent_host,
														 parent_host, use_child_host,
														 child_host, temp_hostgroup,
...
json_object *json_object_host(unsigned format_options, host * temp_host)
{
	...
	json_object_append_string(json_details, "name", &percent_escapes, temp_host->name);
...
void json_object_host_details(json_object * json_details, unsigned format_options, host * temp_host)
{
	...
	json_object_append_string(json_details, "alias", &percent_escapes, temp_host->alias);
	json_object_append_string(json_details, "address", &percent_escapes, temp_host->address);
	...
	json_object_append_real(json_details, "check_interval", temp_host->check_interval);
	json_object_append_real(json_details, "retry_interval", temp_host->retry_interval);
	json_object_append_integer(json_details, "max_attempts", temp_host->max_attempts);
	...
	for (temp_contact = contact_list; temp_contact != NULL; temp_contact = temp_contact->next) {
		if (TRUE == is_contact_for_host(temp_host, temp_contact)) {
			json_array_append_string(json_contacts, &percent_escapes, temp_contact->name);
		}
	}
#endif
...
# endif
#else
	json_object_append_boolean(json_details, "notify_on_down", temp_host->notify_on_down);
	json_object_append_boolean(json_details, "notify_on_unreachable",
	...
	json_object_append_real(json_details, "low_flap_threshold", temp_host->low_flap_threshold);
...
# if 0
	if (CORE3_COMPATIBLE) {
		...
		json_object_append_boolean(json_details "flap_detection_on_unreachable",
								   flag_isset(temp_host->flap_detection_options,
											  OPT_UNREACHABLE));
	} else {
# endif
...
# endif
#else
	json_object_append_boolean(json_details, "stalk_on_up", temp_host->stalk_on_up);
	json_object_append_boolean(json_details, "stalk_on_down", temp_host->stalk_on_down);
	json_object_append_boolean(json_details, "stalk_on_unreachable",
							   temp_host->stalk_on_unreachable);
#endif

	json_object_append_boolean(json_details, "check_freshness", temp_host->check_freshness);
	...
	json_object_append_boolean(json_details, "checks_enabled", temp_host->checks_enabled);
#ifdef JSON_NAGIOS_4X
# if 0
	if (CORE3_COMPATIBLE) {
...
# if 0
	if (CORE3_COMPATIBLE) {
		json_object_append_boolean(json_details, "obsess_over_host", temp_host->obsess);
	} else {
# endif
...
# endif
#else
	json_object_append_boolean(json_details, "obsess_over_host", temp_host->obsess_over_host);
#endif
#ifdef JSON_NAGIOS_4X
	json_object_append_integer(json_details, "hourly_value", temp_host->hourly_value);
#endif
	json_object_append_string(json_details, "notes", &percent_escapes, temp_host->notes);
	...
	json_object_append_boolean(json_details, "have_2d_coords", temp_host->have_2d_coords);
	json_object_append_integer(json_details, "x_2d", temp_host->x_2d);
	json_object_append_integer(json_details, "y_2d", temp_host->y_2d);
	json_object_append_boolean(json_details, "have_3d_coords", temp_host->have_3d_coords);
	...
	json_object_append_boolean(json_details, "should_be_drawn", temp_host->should_be_drawn);
	json_object_append_object(json_details, "custom_variables",
							  json_object_custom_variables(temp_host->custom_variables));
}

int json_object_hostgroup_passes_selection(hostgroup * temp_hostgroup, host * temp_hostgroup_member)
{

	/* Skip if user is not authorized for this hostgroup */
	if (FALSE == is_authorized_for_hostgroup(temp_hostgroup, &current_authdata)) {
		return 0;
	}
	...
	if (NULL != temp_hostgroup_member &&
		(FALSE == is_host_member_of_hostgroup(temp_hostgroup, temp_hostgroup_member))) {
		return 0;
	}
...
json_object *json_object_hostgroupcount(unsigned format_options, host * temp_hostgroup_member)
{
	...
	for (temp_hostgroup = hostgroup_list; temp_hostgroup != NULL;
		...
		if (json_object_hostgroup_passes_selection(temp_hostgroup, temp_hostgroup_member) == 0) {
			continue;
		}
									   ...
									   int count, int details, host * temp_hostgroup_member)
{
	...
	if (details > 0) {
		json_object_append_object(json_data, "hostgrouplist", json_hostgrouplist_object);
	} else {
		json_object_append_array(json_data, "hostgrouplist", json_hostgrouplist_array);
	}

	return json_data;
}

json_object *json_object_hostgroup(unsigned format_options, hostgroup * temp_hostgroup)
{
								   ...
								   unsigned format_options, hostgroup * temp_hostgroup)
{
	...
	json_object_append_string(json_details, "group_name", &percent_escapes,
							  temp_hostgroup->group_name);
	json_object_append_string(json_details, "alias", &percent_escapes, temp_hostgroup->alias);

	json_members = json_new_array();
	for (temp_member = temp_hostgroup->members; temp_member != NULL;
		 temp_member = temp_member->next) {
		json_array_append_string(json_members, &percent_escapes, temp_member->host_name);
	}
	json_object_append_array(json_details, "members", json_members);

	json_object_append_string(json_details, "notes", &percent_escapes, temp_hostgroup->notes);
	json_object_append_string(json_details, "notes_url", &percent_escapes,
...
int json_object_service_passes_host_selection(host * temp_host,
											  int use_parent_host, host * parent_host,
											  int use_child_host, host * child_host,
											  hostgroup * temp_hostgroup, host * match_host)
{
												 ...
												 command * event_handler)
{
	...
	if (NULL != temp_servicegroup &&
		(FALSE == is_service_member_of_servicegroup(temp_servicegroup, temp_service))) {
		return 0;
	}

	/* If the contact was specified, skip the services that do not have
	   the contact specified */
	if (NULL != temp_contact && (FALSE == is_contact_for_service(temp_service, temp_contact))) {
		return 0;
	}
	...
	/* If a contactgroup was specified, skip the services that do not have
	   the contactgroup specified */
	if (NULL != temp_contactgroup &&
		(FALSE == is_contactgroup_for_service(temp_service, temp_contactgroup))) {
		return 0;
	}
	...
	if (NULL != check_timeperiod && (check_timeperiod != temp_service->check_period_ptr)) {
		return 0;
	}
	...
	/* If a check command was specified, skip this service if it does not
	   have the check command specified */
	if (NULL != check_command && (check_command != temp_service->check_command_ptr)) {
		return 0;
	}

	/* If an event handler was specified, skip this service if it does not
	   have the event handler specified */
	if (NULL != event_handler && (event_handler != temp_service->event_handler_ptr)) {
		return 0;
	}
		...
		/* Otherwise, skip this service if it does not have the specified
		   service as a parent */
			...
			for (temp_servicesmember = temp_service->parents;
				 ...
				 temp_servicesmember = temp_servicesmember->next) {
				if (!strcmp(temp_servicesmember->service_description, parent_service_name)) {
					found = 1;
				}
			}
			if (0 == found) {
		...
		/* Otherwise, skip this service if it does not have the specified
		   service as a child */
			...
			for (temp_servicesmember = temp_service->children;
				 temp_servicesmember != NULL;
				 temp_servicesmember = temp_servicesmember->next) {
				if (!strcmp(temp_servicesmember->service_description, child_service_name)) {
					found = 1;
				}
			}
			if (0 == found) {
										   ...
										   command * check_command, command * event_handler)
{
	...
	if (NULL != match_host) {
		json_object_append_string(json_selectors, "host", &percent_escapes, match_host->name);
	}
	if (1 == use_parent_host) {
		json_object_append_string(json_selectors, "parenthost",
								  &percent_escapes,
...
json_object *json_object_servicelist(unsigned format_options, int start,
									 int count, int details, host * match_host,
									 int use_parent_host, host * parent_host,
									 int use_child_host, host * child_host,
									 hostgroup * temp_hostgroup,
									 servicegroup * temp_servicegroup, contact * temp_contact,
									 ...
									 command * check_command, command * event_handler)
{
	...
	json_object_append_object(json_data, "selectors",
							  json_object_service_selectors(start, count, use_parent_host,
															parent_host, use_child_host,
															child_host, temp_hostgroup,
															match_host, temp_servicegroup,
															temp_contact, service_description,
															parent_service_name,
															child_service_name,
															temp_contactgroup,
															check_timeperiod,
		...
		for (temp_service = service_list; temp_service != NULL;
			...
			if (passes_start_and_count_limits(start, count, current, counted)) {
				if (details > 0) {
					...
					asprintf(&buf, "%s", temp_service->description);
...
# endif
	...
	json_object_append_real(json_details, "check_interval", temp_service->check_interval);
	json_object_append_real(json_details, "retry_interval", temp_service->retry_interval);
	json_object_append_integer(json_details, "max_attempts", temp_service->max_attempts);
	json_object_append_boolean(json_details, "parallelize", temp_service->parallelize);
...
# endif
#else
	json_object_append_boolean(json_details, "stalk_on_ok", temp_service->stalk_on_ok);
	json_object_append_boolean(json_details, "stalk_on_warning",
	...
	json_object_append_boolean(json_details, "is_volatile", temp_service->is_volatile);
	json_object_append_string(json_details, "notification_period",
...
# if 0
	if (CORE3_COMPATIBLE) {
		...
		json_object_append_boolean(json_details, "flap_detection_on_warning",
								   flag_isset(temp_service->flap_detection_options,
											  OPT_WARNING));
...
# endif
	...
	json_object_append_boolean(json_details, "check_freshness", temp_service->check_freshness);
	json_object_append_integer(json_details, "freshness_threshold",
...
# endif
	...
	json_object_append_boolean(json_details, "checks_enabled", temp_service->checks_enabled);
	json_object_append_boolean(json_details, "retain_status_information",
...
# endif
...
#endif
#ifdef JSON_NAGIOS_4X
	json_object_append_integer(json_details, "hourly_value", temp_service->hourly_value);
#endif

#ifdef JSON_NAGIOS_4X
	json_parent_services = json_new_array();
	for (temp_servicesmember = temp_service->parents;
		 temp_servicesmember != NULL; temp_servicesmember = temp_servicesmember->next) {
	...
	json_object_append_string(json_details, "notes", &percent_escapes, temp_service->notes);
	json_object_append_string(json_details, "notes_url", &percent_escapes,
											  ...
											  service * temp_servicegroup_member)
{
	...
	/* Skip if user is not authorized for this hostgroup */
	if (FALSE == is_authorized_for_servicegroup(temp_servicegroup, &current_authdata)) {
		return 0;
	}
										  ...
										  service * temp_servicegroup_member)
{
	...
	if (details > 0) {
		json_object_append_object(json_data, "servicegrouplist", json_servicegrouplist_object);
	} else {
		json_object_append_array(json_data, "servicegrouplist", json_servicegrouplist_array);
	}
									  ...
									  servicegroup * temp_servicegroup)
{
	...
	json_object_append_string(json_details, "group_name",
							  &percent_escapes, temp_servicegroup->group_name);
	json_object_servicegroup_details(json_details, format_options, temp_servicegroup);
	json_object_append_object(json_servicegroup, "servicegroup", json_details);

	return json_servicegroup;
}

void json_object_servicegroup_details(json_object * json_details, unsigned format_options,
									  servicegroup * temp_servicegroup)
{
										 ...
										 timeperiod * service_notification_timeperiod)
{
	...
	if (NULL != temp_contactgroup &&
		(FALSE == is_contact_member_of_contactgroup(temp_contactgroup, temp_contact))) {
		return 0;
	}
	...
	if (NULL != service_notification_timeperiod &&
		(service_notification_timeperiod != temp_contact->service_notification_period_ptr)) {
		return 0;
	}
									 ...
									 timeperiod * service_notification_timeperiod)
{
	...
	if (details > 0) {
		json_object_append_object(json_data, "contactlist", json_contactlist_object);
	} else {
		json_object_append_array(json_data, "contactlist", json_contactlist_array);
	}
...
json_object *json_object_contact(unsigned format_options, contact * temp_contact)
{
	...
	json_object_append_string(json_details, "name", &percent_escapes, temp_contact->name);
								 ...
								 unsigned format_options, contact * temp_contact)
{
	...
	json_object_append_string(json_details, "name", &percent_escapes, temp_contact->name);
	json_object_append_string(json_details, "alias", &percent_escapes, temp_contact->alias);
	json_object_append_string(json_details, "email", &percent_escapes, temp_contact->email);
	json_object_append_string(json_details, "pager", &percent_escapes, temp_contact->pager);
	...
	for (temp_commandsmember = temp_contact->host_notification_commands;
		 temp_commandsmember != NULL; temp_commandsmember = temp_commandsmember->next) {
		json_array_append_string(json_host_commands, &percent_escapes,
								 temp_commandsmember->command);
	}
	json_object_append_array(json_details, "host_notification_commands", json_host_commands);
...
#ifdef JSON_NAGIOS_4X
	json_object_append_integer(json_details, "minimum_value", temp_contact->minimum_value);
#endif
	json_object_append_object(json_details, "custom_variables",
										  ...
										  contact * temp_contactgroup_member)
{
	...
	if (details > 0) {
		json_object_append_object(json_data, "contactgrouplist", json_contactgrouplist_object);
	} else {
		json_object_append_array(json_data, "contactgrouplist", json_contactgrouplist_array);
	}
									  ...
									  contactgroup * temp_contactgroup)
{
	...
	json_object_append_string(json_details, "group_name", &percent_escapes,
							  temp_contactgroup->group_name);
	json_object_contactgroup_details(json_details, format_options, temp_contactgroup);
	json_object_append_object(json_contactgroup, "contactgroup", json_details);

	return json_contactgroup;
}

void json_object_contactgroup_details(json_object * json_details, unsigned format_options,
									  contactgroup * temp_contactgroup)
{
	...
	for (temp_member = temp_contactgroup->members; temp_member != NULL;
		 temp_member = temp_member->next) {
		json_array_append_string(json_members, &percent_escapes, temp_member->contact_name);
	}
	json_object_append_array(json_details, "members", json_members);
}
...
json_object *json_object_timeperiodcount(void)
{
	...
	json_data = json_new_object();
	json_object_append_object(json_data, "selectors", json_object_timeperiod_selectors(0, 0));
										...
										int count, int details)
{
	...
	if (details > 0) {
		json_object_append_object(json_data, "timeperiodlist", json_timeperiodlist_object);
	} else {
		json_object_append_array(json_data, "timeperiodlist", json_timeperiodlist_array);
	}

	return json_data;
}

json_object *json_object_timeperiod(unsigned format_options, timeperiod * temp_timeperiod)
{

	json_object *json_timeperiod = json_new_object();
	json_object *json_details = json_new_object();

	json_object_append_string(json_details, "name", &percent_escapes, temp_timeperiod->name);
...
void json_object_timeperiod_details(json_object * json_details, unsigned format_options,
									timeperiod * temp_timeperiod)
{
	...
	json_object_append_string(json_details, "name", &percent_escapes, temp_timeperiod->name);
	json_object_append_string(json_details, "alias", &percent_escapes, temp_timeperiod->alias);
	...
	for (x = 0; x < sizeof(temp_timeperiod->days) / sizeof(temp_timeperiod->days[0]); x++) {
		if (NULL != temp_timeperiod->days[x]) {
			...
			json_object_timerange(json_timeranges, format_options, temp_timeperiod->days[x]);
			json_object_append_array(json_days, (char *)dayofweek[x], json_timeranges);
		}
	}
								   ...
								   daterange * temp_daterange, int daterange_type)
{
	...
	case DATERANGE_CALENDAR_DATE:
		json_object_append_string(json_daterange, "type", NULL, "DATERANGE_CALENDAR_DATE");
		...
		json_timeranges = json_new_array();
		json_object_timerange(json_timeranges, format_options, temp_daterange->times);
		json_object_append_array(json_daterange, "times", json_timeranges);
		break;
	case DATERANGE_MONTH_DATE:
		json_object_append_string(json_daterange, "type", NULL, "DATERANGE_MONTH_DATE");
		if (temp_daterange->smon == temp_daterange->emon &&
			...
			json_object_append_integer(json_daterange, "day", temp_daterange->smday);
		} else {
			json_object_append_string(json_daterange, "startmonth",
									  &percent_escapes, (char *)month[temp_daterange->smon]);
			json_object_append_integer(json_daterange, "startday", temp_daterange->smday);
			json_object_append_string(json_daterange, "endmonth",
									  &percent_escapes, (char *)month[temp_daterange->emon]);
			json_object_append_integer(json_daterange, "endday", temp_daterange->emday);
		}
		if (temp_daterange->skip_interval > 0) {
			json_object_append_integer(json_daterange, "skip_interval",
									   temp_daterange->skip_interval);
	...
	case DATERANGE_MONTH_DAY:
		json_object_append_string(json_daterange, "type", NULL, "DATERANGE_MONTH_DAY");
	...
	case DATERANGE_MONTH_WEEK_DAY:
		json_object_append_string(json_daterange, "type", NULL, "DATERANGE_MONTH_WEEK_DAY");
		if (temp_daterange->smon == temp_daterange->emon &&
			...
			json_object_append_integer(json_daterange, "startweekdayoffset",
									   temp_daterange->swday_offset);
			json_object_append_string(json_daterange, "endmonth", &percent_escapes,
									  (char *)month[temp_daterange->emon]);
			json_object_append_string(json_daterange, "endweekday", &percent_escapes,
	...
	case DATERANGE_WEEK_DAY:
		json_object_append_string(json_daterange, "type", NULL, "DATERANGE_WEEK_DAY");
						   ...
						   timerange * temp_timerange)
{
	...
	for (; temp_timerange != NULL; temp_timerange = temp_timerange->next) {
		...
		json_object_append_time(json_timerange, "range_start", temp_timerange->range_start);
		json_object_append_time(json_timerange, "range_end", temp_timerange->range_end);
...
json_object *json_object_commandcount(void)
{
	...
	json_data = json_new_object();
	json_object_append_object(json_data, "selectors", json_object_command_selectors(0, 0));
									 ...
									 int count, int details)
{
	...
	if (details > 0) {
		json_object_append_object(json_data, "commandlist", json_commandlist_object);
	} else {
		json_object_append_array(json_data, "commandlist", json_commandlist_array);
	}
...
json_object *json_object_command(unsigned format_options, command * temp_command)
{
	...
	json_object_append_string(json_details, "name", &percent_escapes, temp_command->name);
...
int json_object_servicedependency_passes_selection(servicedependency * temp_servicedependency,
												   host * master_host,
												   ...
												   servicegroup * dependent_servicegroup)
{
	...
	if (NULL != master_host && strcmp(temp_servicedependency->host_name, master_host->name)) {
		return 0;
	}
	...
	if (NULL != master_hostgroup) {
		...
		if ((NULL != temp_host) && (FALSE ==
									is_host_member_of_hostgroup(master_hostgroup,
																temp_host))) {
			return 0;
		}
	}
	...
	if (NULL != master_service_description &&
		strcmp(temp_servicedependency->service_description, master_service_description)) {
		return 0;
	}
	...
	if (NULL != dependent_host &&
		strcmp(temp_servicedependency->dependent_host_name, dependent_host->name)) {
		return 0;
	}
	...
	if (NULL != dependent_hostgroup) {
		...
		if ((NULL != temp_host) && (FALSE ==
									is_host_member_of_hostgroup(dependent_hostgroup,
																temp_host))) {
			return 0;
		}
	}
	...
	if (NULL != dependent_servicegroup) {
		...
		if ((NULL != temp_service) && (FALSE ==
									   ...
									   (dependent_servicegroup, temp_service))) {
			return 0;
		}
	}
...
json_object *json_object_servicedependencylist(unsigned format_options,
											   int start, int count, host * master_host,
											   hostgroup * master_hostgroup,
											   char *master_service_description,
											   servicegroup * master_servicegroup,
											   host * dependent_host,
											   ...
											   servicegroup * dependent_servicegroup)
{
	...
	json_object_append_array(json_data, "servicedependencylist", json_servicedependencylist);

	return json_data;
}

void json_object_servicedependency_details(json_object * json_details, unsigned format_options,
										   servicedependency * temp_servicedependency)
{
...
# endif
#else
	json_object_append_boolean(json_details, "fail_on_ok", temp_servicedependency->fail_on_ok);
	json_object_append_boolean(json_details, "fail_on_warning",
...
int json_object_serviceescalation_passes_selection(serviceescalation * temp_serviceescalation,
												   ...
												   servicegroup * match_servicegroup,
												   ...
												   contactgroup * match_contactgroup)
{
	...
	if (NULL != match_host && strcmp(temp_serviceescalation->host_name, match_host->name)) {
		return 0;
	}
	...
	if (NULL != match_hostgroup) {
		...
		for (temp_hostsmember = match_hostgroup->members;
			 temp_hostsmember != NULL; temp_hostsmember = temp_hostsmember->next) {
			if (!strcmp(temp_hostsmember->host_name, temp_serviceescalation->host_name)) {
				found = 1;
				break;
			}
		}
	...
	if (NULL != match_contactgroup && (FALSE ==
									   is_contactgroup_for_service_escalation
									   (temp_serviceescalation, match_contactgroup))) {
		return 0;
	}
												...
												contactgroup * match_contactgroup)
{
	...
	for (temp_serviceescalation = serviceescalation_list;
		...
		if (json_object_serviceescalation_passes_selection(temp_serviceescalation,
														   ...
														   match_hostgroup, match_servicegroup,
														   match_contact,
...
json_object *json_object_serviceescalationlist(unsigned format_options,
											   int start, int count, host * match_host,
											   char *service_description,
											   hostgroup * match_hostgroup,
											   servicegroup * match_servicegroup,
											   contact * match_contact,
											   contactgroup * match_contactgroup)
{
	...
	json_object_append_array(json_data, "serviceescalationlist", json_serviceescalationlist);

	return json_data;
}

void json_object_serviceescalation_details(json_object * json_details, unsigned format_options,
										   serviceescalation * temp_serviceescalation)
{
...
int json_object_hostdependency_passes_selection(hostdependency * temp_hostdependency,
												host * master_host,
												hostgroup * master_hostgroup,
												host * dependent_host,
												hostgroup * dependent_hostgroup)
{
	...
	if (NULL != master_host && strcmp(temp_hostdependency->host_name, master_host->name)) {
		return 0;
	}
	...
	if (NULL != dependent_host &&
		strcmp(temp_hostdependency->dependent_host_name, dependent_host->name)) {
		return 0;
	}
											 ...
											 hostgroup * dependent_hostgroup)
{
	...
	for (temp_hostdependency = hostdependency_list;
		 temp_hostdependency != NULL; temp_hostdependency = temp_hostdependency->next) {
#endif
											...
											hostgroup * dependent_hostgroup)
{
	...
	for (temp_hostdependency = hostdependency_list;
		...
		if (passes_start_and_count_limits(start, count, current, counted)) {
			...
			json_array_append_object(json_hostdependencylist, json_hostdependency_details);
		}
		counted++;
		current++;
	}

	json_object_append_array(json_data, "hostdependencylist", json_hostdependencylist);

	return json_data;
}

void json_object_hostdependency_details(json_object * json_details, unsigned format_options,
										hostdependency * temp_hostdependency)
{
...
# endif
#else
	json_object_append_boolean(json_details, "fail_on_up", temp_hostdependency->fail_on_up);
	json_object_append_boolean(json_details, "fail_on_down",
												...
												contactgroup * match_contactgroup)
{
	...
	if (NULL != match_host && strcmp(temp_hostescalation->host_name, match_host->name)) {
		return 0;
	}
	...
	if (NULL != match_hostgroup) {
		...
		for (temp_hostsmember = match_hostgroup->members;
			 temp_hostsmember != NULL; temp_hostsmember = temp_hostsmember->next) {
			if (!strcmp(temp_hostsmember->host_name, temp_hostescalation->host_name)) {
				found = 1;
				break;
			}
		}
	...
	/* If a contact was specified, skip this host escalation if it does
	   not have the contact specified */
	if (NULL != match_contact &&
		(FALSE == is_contact_for_host_escalation(temp_hostescalation, match_contact))) {
		return 0;
	}
											 ...
											 contactgroup * match_contactgroup)
{
	...
	for (temp_hostescalation = hostescalation_list;
		 temp_hostescalation != NULL; temp_hostescalation = temp_hostescalation->next) {
#endif
		if (json_object_hostescalation_passes_selection(temp_hostescalation,
														match_host, match_hostgroup,
														match_contact,
														match_contactgroup) == 0) {
			continue;
		}
											...
											contactgroup * match_contactgroup)
{
	...
	for (temp_hostescalation = hostescalation_list;
		...
		if (passes_start_and_count_limits(start, count, current, counted)) {
			...
			json_array_append_object(json_hostescalationlist, json_hostescalation_details);
		}
		counted++;
		current++;
	}

	json_object_append_array(json_data, "hostescalationlist", json_hostescalationlist);

	return json_data;
}

void json_object_hostescalation_details(json_object * json_details, unsigned format_options,
										hostescalation * temp_hostescalation)
{
