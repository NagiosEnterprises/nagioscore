...
int main(void)
{
	...
	if (display_header == TRUE) {
		...
		display_info_table((log_rotation_method == LOG_ROTATION_NONE
							|| log_archive == 0) ? "Current Event Log" : "Archived Event Log",
						   FALSE, &current_authdata);
		printf("</td>\n");

		/* middle column of top table - log file navigation options */
		printf("<td align=center valign=top width=33%%>\n");
		snprintf(temp_buffer, sizeof(temp_buffer) - 1, "%s?%s", SHOWLOG_CGI,
				 (use_lifo == FALSE) ? "oldestfirst&" : "");
		...
		printf("<tr>");
		printf
			("<td align=left valign=bottom CLASS='optBoxItem'><input type='submit' value='Update'></td>\n");
		printf("</tr>\n");
...
int display_log(void)
{
	...
	if (is_authorized_for_system_information(&current_authdata) == FALSE) {
		printf("<HR>\n");
		printf
			("<DIV CLASS='errorMessage'>It appears as though you do not have permission to view the log file...</DIV><br><br>\n");
		printf
			("<DIV CLASS='errorDescription'>If you believe this is an error, check the HTTP server authentication requirements for accessing this CGI<br>and check the authorization options in your CGI configuration file.</DIV>\n");
		printf("<HR>\n");
		return ERROR;
	}
	...
	if (use_lifo == TRUE) {
		...
		if (error != LIFO_OK) {
			if (error == LIFO_ERROR_MEMORY) {
				printf
					("<P><DIV CLASS='warningMessage'>Not enough memory to reverse log file - displaying log in natural order...</DIV></P>");
				error = FALSE;
			} else
				error = TRUE;
			use_lifo = FALSE;
	...
	if (use_lifo == FALSE) {
		...
		if ((thefile = mmap_fopen(log_file_to_use)) == NULL) {
			printf("<HR>\n");
			printf
				("<P><DIV CLASS='errorMessage'>Error: Could not open log file '%s' for reading!</DIV></P>",
				 log_file_to_use);
			printf("<HR>\n");
			error = TRUE;
		}
		...
		while (1) {
			...
			if (strstr(input, " starting...")) {
				...
				strcpy(image_alt, HOST_UNREACHABLE_ICON_ALT);
					   ...
					   && (strstr(input, ";RECOVERY;") || strstr(input, ";UP;"))) {
			...
			} else if (strstr(input, "SERVICE ALERT:")
					   && (strstr(input, ";RECOVERY;") || strstr(input, ";OK;"))) {
			...
			if (display_frills == TRUE)
				printf("<img align='left' src='%s%s' alt='%s' title='%s'>", url_images_path,
					   image, image_alt, image_alt);
			printf("[%s] %s", date_time,
				   (temp_buffer == NULL) ? "" : html_encode(temp_buffer, FALSE));
