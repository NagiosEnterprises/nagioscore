...
option_help status_json_help[] = {
	 ...
	 NULL},
	{
	 ...
	 "nagios:objectjson/hostlist",
	 {"host", "service", NULL},
	 {"servicecount", "servicelist", "commentcount", "commentlist", "downtimecount",
	  "downtimelist", NULL},
	 NULL,
...
int json_status_host_passes_selection(host *, int, host *, int, host *,
									  hostgroup *, contact *, hoststatus *, int,
									  time_t, time_t, contactgroup *, timeperiod *,
									  timeperiod *, command *, command *);
json_object *json_status_host_display_selectors(unsigned, int, int, int, host *, int, host *,
												hostgroup *, int, contact *, int, time_t,
												time_t, contactgroup *, timeperiod *,
												timeperiod *, command *, command *);
...
int json_status_service_passes_service_selection(service *, servicegroup *,
												 contact *, servicestatus *, int,
												 time_t, time_t, char *, char *,
												 char *, contactgroup *, timeperiod *,
												 timeperiod *, command *, command *);
json_object *json_status_service_selectors(unsigned, int, int, int, host *, int, host *,
										   hostgroup *, host *, servicegroup *, int, int,
										   contact *, int, time_t, time_t, char *, char *,
...
int json_status_comment_passes_selection(nagios_comment *, int, time_t, time_t,
										 ...
										 char *, char *);
json_object *json_status_comment_selectors(unsigned, int, int, int, time_t, time_t, unsigned,
...
int json_status_downtime_passes_selection(scheduled_downtime *, int, time_t,
										  ...
										  unsigned, char *, char *);
json_object *json_status_downtime_selectors(unsigned, int, int, int, time_t, time_t, unsigned,
											unsigned, unsigned, int, unsigned, char *, char *);
	...
	case STATUS_QUERY_HOSTCOUNT:
		...
		json_object_append_object(json_root, "data",
								  json_status_hostcount(cgi_data.format_options,
														...
														cgi_data.child_host,
														cgi_data.hostgroup,
														cgi_data.host_statuses,
														cgi_data.contact,
														cgi_data.host_time_field,
	...
	case STATUS_QUERY_SERVICECOUNT:
		json_object_append_object(json_root, "result",
								  json_result(query_time, THISCGI,
											  ...
											  RESULT_SUCCESS, ""));
		json_object_append_object(json_root, "data",
								  json_status_servicecount(cgi_data.format_options,
														   cgi_data.host,
														   cgi_data.use_parent_host,
														   cgi_data.parent_host,
														   cgi_data.use_child_host,
														   ...
														   cgi_data.host_statuses,
														   cgi_data.service_statuses,
														   cgi_data.contact,
														   cgi_data.service_time_field,
														   cgi_data.start_time,
														   cgi_data.end_time,
	...
	case STATUS_QUERY_SERVICELIST:
		json_object_append_object(json_root, "result",
								  json_result(query_time, THISCGI,
											  ...
											  RESULT_SUCCESS, ""));
		json_object_append_object(json_root, "data",
								  json_status_servicelist(cgi_data.format_options,
	...
	case STATUS_QUERY_CONTACTCOUNT:
		json_object_append_object(json_root, "result",
								  json_result(query_time, THISCGI,
											  ...
											  last_status_data_update, &current_authdata,
											  RESULT_SUCCESS, ""));
		json_status_contactcount(1, cgi_data.format_options, cgi_data.start, cgi_data.count,
								 cgi_data.details, cgi_data.contactgroup);
		break;
	case STATUS_QUERY_CONTACTLIST:
		json_object_append_object(json_root, "result",
								  json_result(query_time, THISCGI,
											  ...
											  last_status_data_update, &current_authdata,
											  RESULT_SUCCESS, ""));
		json_status_contactlist(1, cgi_data.format_options, cgi_data.start, cgi_data.count,
								cgi_data.details, cgi_data.contactgroup);
		break;
	...
	case STATUS_QUERY_COMMENTCOUNT:
		...
		json_object_append_object(json_root, "data",
								  json_status_commentcount(cgi_data.format_options,
														   cgi_data.comment_time_field,
														   cgi_data.start_time,
														   cgi_data.end_time,
														   cgi_data.comment_types,
	...
	case STATUS_QUERY_COMMENTLIST:
		json_object_append_object(json_root, "result",
								  json_result(query_time, THISCGI,
											  ...
											  RESULT_SUCCESS, ""));
		json_object_append_object(json_root, "data",
								  json_status_commentlist(cgi_data.format_options,
														  cgi_data.start, cgi_data.count,
														  cgi_data.details,
														  cgi_data.comment_time_field,
														  cgi_data.start_time,
	...
	case STATUS_QUERY_COMMENT:
		json_object_append_object(json_root, "result",
								  json_result(query_time, THISCGI,
											  ...
											  RESULT_SUCCESS, ""));
		json_object_append_object(json_root, "data",
								  json_status_comment(cgi_data.format_options,
													  cgi_data.comment));
		break;
	case STATUS_QUERY_DOWNTIMECOUNT:
		...
		json_object_append_object(json_root, "data",
								  json_status_downtimecount(cgi_data.format_options,
															cgi_data.downtime_time_field,
															cgi_data.start_time,
															cgi_data.end_time,
															cgi_data.downtime_object_types,
															cgi_data.downtime_types,
															cgi_data.triggered,
	...
	case STATUS_QUERY_DOWNTIME:
		json_object_append_object(json_root, "result",
								  json_result(query_time, THISCGI,
											  ...
											  RESULT_SUCCESS, ""));
		json_object_append_object(json_root, "data",
								  json_status_downtime(cgi_data.format_options,
													   cgi_data.downtime));
		break;
...
int process_cgivars(json_object * json_root, status_json_cgi_data * cgi_data, time_t query_time)
{
	char	  **variables;
...
int validate_arguments(json_object * json_root, status_json_cgi_data * cgi_data, time_t query_time)
{
	int 		result = RESULT_SUCCESS;
	host	   *temp_host = NULL;
...
int json_status_host_passes_selection(host * temp_host, int use_parent_host,
									  ...
									  contact * temp_contact, hoststatus * temp_hoststatus,
									  ...
									  command * check_command, command * event_handler)
{
	...
	case STATUS_TIME_LAST_STATE_CHANGE:
		if ((start_time > 0) && (temp_hoststatus->last_state_change < start_time)) {
			return 0;
		}
	...
	case STATUS_TIME_LAST_HARD_STATE_CHANGE:
		if ((start_time > 0) && (temp_hoststatus->last_hard_state_change < start_time)) {
			return 0;
		}
		if ((end_time > 0) && (temp_hoststatus->last_hard_state_change > end_time)) {
			return 0;
		}
		break;
	...
	case STATUS_TIME_LAST_TIME_UNREACHABLE:
		if ((start_time > 0) && (temp_hoststatus->last_time_unreachable < start_time)) {
			return 0;
		}
		if ((end_time > 0) && (temp_hoststatus->last_time_unreachable > end_time)) {
			return 0;
		}
		break;
	case STATUS_TIME_LAST_NOTIFICATION:
		if ((start_time > 0) && (temp_hoststatus->last_notification < start_time)) {
			return 0;
		}
	...
	case STATUS_TIME_NEXT_NOTIFICATION:
		if ((start_time > 0) && (temp_hoststatus->next_notification < start_time)) {
			return 0;
		}
...
json_object *json_status_host_selectors(unsigned format_options, int start,
										int count, int use_parent_host, host * parent_host,
										int use_child_host, host * child_host,
										hostgroup * temp_hostgroup, int host_statuses,
										contact * temp_contact, int time_field,
								   ...
								   command * check_command, command * event_handler)
{
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		if (json_status_host_passes_selection(temp_host, use_parent_host,
											  ...
											  temp_hostgroup, temp_contact, temp_hoststatus,
											  time_field, start_time, end_time,
											  temp_contactgroup, check_timeperiod,
...
json_object *json_status_hostlist(unsigned format_options, int start, int count,
								  int details, int use_parent_host, host * parent_host,
								  int use_child_host, host * child_host,
								  hostgroup * temp_hostgroup, int host_statuses,
								  contact * temp_contact, int time_field, time_t start_time,
								  time_t end_time, contactgroup * temp_contactgroup,
								  ...
								  command * check_command, command * event_handler)
{
	...
	json_object_append_object(json_data, "selectors",
							  json_status_host_selectors(format_options, start, count,
														 ...
														 temp_hostgroup, host_statuses,
														 temp_contact, time_field, start_time,
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		if (passes_start_and_count_limits(start, count, current, counted)) {
			if (details > 0) {
				...
				json_object_append_object(json_hostlist, temp_host->name, json_host_details);
			} else {
							  ...
							  hoststatus * temp_hoststatus)
{
	...
	json_status_host_details(json_details, format_options, temp_host, temp_hoststatus);
							  ...
							  host * temp_host, hoststatus * temp_hoststatus)
{
	...
	json_enumeration(json_details, format_options, "status",
					 temp_hoststatus->status, svm_host_statuses);
	json_object_append_time_t(json_details, "last_update", temp_hoststatus->last_update);
	...
	json_object_append_integer(json_details, "max_attempts", temp_hoststatus->max_attempts);
	json_object_append_time_t(json_details, "last_check", temp_hoststatus->last_check);
	json_object_append_time_t(json_details, "next_check", temp_hoststatus->next_check);
	...
	json_object_append_time_t(json_details, "last_time_up", temp_hoststatus->last_time_up);
	json_object_append_time_t(json_details, "last_time_down", temp_hoststatus->last_time_down);
	...
	json_object_append_boolean(json_details, "is_flapping", temp_hoststatus->is_flapping);
	...
	json_object_append_real(json_details, "execution_time", temp_hoststatus->execution_time);
...
int json_status_service_passes_service_selection(service * temp_service,
												 ...
												 int time_field, time_t start_time,
												 ...
												 command * event_handler)
{
	...
	case STATUS_TIME_NEXT_CHECK:
		...
		break;
	case STATUS_TIME_LAST_STATE_CHANGE:
		if ((start_time > 0) && (temp_servicestatus->last_state_change < start_time)) {
			return 0;
		}
		...
		break;
	case STATUS_TIME_LAST_HARD_STATE_CHANGE:
		if ((start_time > 0) && (temp_servicestatus->last_hard_state_change < start_time)) {
			return 0;
		}
		if ((end_time > 0) && (temp_servicestatus->last_hard_state_change > end_time)) {
			return 0;
		}
		break;
	...
	case STATUS_TIME_LAST_TIME_WARNING:
		if ((start_time > 0) && (temp_servicestatus->last_time_warning < start_time)) {
			return 0;
		}
	...
	case STATUS_TIME_LAST_TIME_CRITICAL:
		if ((start_time > 0) && (temp_servicestatus->last_time_critical < start_time)) {
			return 0;
		}
		if ((end_time > 0) && (temp_servicestatus->last_time_critical > end_time)) {
			return 0;
		}
		break;
	case STATUS_TIME_LAST_TIME_UNKNOWN:
		if ((start_time > 0) && (temp_servicestatus->last_time_unknown < start_time)) {
			return 0;
		}
		...
		break;
	case STATUS_TIME_LAST_NOTIFICATION:
		if ((start_time > 0) && (temp_servicestatus->last_notification < start_time)) {
			return 0;
		}
		...
		break;
	case STATUS_TIME_NEXT_NOTIFICATION:
		if ((start_time > 0) && (temp_servicestatus->next_notification < start_time)) {
			return 0;
		}
	...
	if (NULL != service_description && strcmp(temp_service->description, service_description)) {
		return 0;
	}
...
json_object *json_status_service_selectors(unsigned format_options,
										   int start, int count, int use_parent_host,
										   host * parent_host, int use_child_host,
										   host * child_host, hostgroup * temp_hostgroup,
										   host * match_host, servicegroup * temp_servicegroup,
										   int host_statuses, int service_statuses,
										   contact * temp_contact, int time_field,
										   time_t start_time, time_t end_time,
...
json_object *json_status_servicecount(unsigned format_options, host * match_host,
									  ...
									  int service_statuses, contact * temp_contact,
									  int time_field, time_t start_time, time_t end_time,
									  ...
									  command * check_command, command * event_handler)
{
	...
	json_data = json_new_object();
	json_object_append_object(json_data, "selectors",
							  json_status_service_selectors(format_options, 0, 0,
															use_parent_host, parent_host,
															use_child_host, child_host,
															temp_hostgroup, match_host,
															temp_servicegroup, host_statuses,
															service_statuses, temp_contact,
															time_field, start_time, end_time,
															service_description,
															parent_service_name,
															child_service_name,
									 ...
									 command * check_command, command * event_handler)
{
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
			...
			if (passes_start_and_count_limits(start, count, current, counted)) {
				if (details > 0) {
					...
					json_object_append_object(json_servicelist,
											  temp_service->description, json_service_details);
				} else {
		...
		if (service_count > 0) {
			json_object_append_object(json_hostlist, temp_host->name, json_servicelist);
		}
	}
								 ...
								 servicestatus * temp_servicestatus)
{
	...
	json_object_append_integer(json_details, "max_attempts", temp_servicestatus->max_attempts);
	json_object_append_integer(json_details, "current_attempt",
							   temp_servicestatus->current_attempt);
	json_enumeration(json_details, format_options, "status",
					 temp_servicestatus->status, svm_service_statuses);
	json_object_append_time_t(json_details, "last_update", temp_servicestatus->last_update);
	json_object_append_boolean(json_details, "has_been_checked",
							   temp_servicestatus->has_been_checked);
	json_object_append_boolean(json_details, "should_be_scheduled",
							   temp_servicestatus->should_be_scheduled);
	json_object_append_time_t(json_details, "last_check", temp_servicestatus->last_check);
	json_bitmask(json_details, format_options, "check_options",
	...
	json_object_append_time_t(json_details, "last_time_ok", temp_servicestatus->last_time_ok);
	...
	json_object_append_time_t(json_details, "next_check", temp_servicestatus->next_check);
	json_object_append_boolean(json_details, "no_more_notifications",
	...
	json_enumeration(json_details, format_options, "acknowledgement_type",
					 temp_servicestatus->acknowledgement_type, svm_acknowledgement_types);
	json_object_append_integer(json_details, "current_notification_number",
	...
	json_object_append_boolean(json_details, "is_flapping", temp_servicestatus->is_flapping);
	json_object_append_real(json_details, "percent_state_change",
...
#ifdef JSON_NAGIOS_4X
	json_object_append_boolean(json_details, "obsess", temp_servicestatus->obsess);
#else
										 ...
										 char *host_name, char *service_description)
{
	...
	if (NULL != host_name) {
		if ((NULL == temp_comment->host_name) || strcmp(temp_comment->host_name, host_name)) {
			return 0;
		}
	}
	...
	if (NULL != service_description) {
		if ((NULL == temp_comment->service_description) ||
			strcmp(temp_comment->service_description, service_description)) {
			return 0;
		}
	}
...
json_object *json_status_comment_selectors(unsigned format_options, int start,
										   int count, int time_field, time_t start_time,
										   time_t end_time, unsigned comment_types,
										   unsigned entry_types, unsigned persistence,
										   unsigned expiring, char *host_name,
										   char *service_description)
{
	...
	if (NULL != host_name) {
		json_object_append_string(json_selectors, "hostname", &percent_escapes, host_name);
	}
	if (NULL != service_description) {
		json_object_append_string(json_selectors, "servicedescription",
								  &percent_escapes, service_description);
									  ...
									  char *service_description)
{
	...
	json_object_append_object(json_data, "selectors",
							  json_status_comment_selectors(format_options, 0, 0, time_field,
															start_time, end_time,
															comment_types, entry_types,
															persistence, expiring, host_name,
															service_description));

	for (temp_comment = comment_list; temp_comment != NULL; temp_comment = temp_comment->next) {
		if (json_status_comment_passes_selection(temp_comment, time_field,
												 start_time, end_time, comment_types,
												 entry_types, persistence, expiring, host_name,
									 ...
									 char *service_description)
{
		...
		if (passes_start_and_count_limits(start, count, current, counted)) {
			if (details > 0) {
				...
				json_status_comment_details(json_comment_details, format_options,
											temp_comment);
				json_object_append_object(json_commentlist_object, buf, json_comment_details);
			} else {
				json_array_append_integer(json_commentlist_array, temp_comment->comment_id);
			}
			counted++;
		}
		current++;
	...
	if (details > 0) {
		json_object_append_object(json_data, "commentlist", json_commentlist_object);
	} else {
		json_object_append_array(json_data, "commentlist", json_commentlist_array);
	}

	return json_data;
}

json_object *json_status_comment(unsigned format_options, nagios_comment * temp_comment)
{

	json_object *json_comment = json_new_object();
	json_object *json_details = json_new_object();

	json_object_append_integer(json_details, "comment_id", temp_comment->comment_id);
...
void json_status_comment_details(json_object * json_details, unsigned format_options,
								 nagios_comment * temp_comment)
{
	...
	json_object_append_boolean(json_details, "persistent", temp_comment->persistent);
	json_object_append_time_t(json_details, "entry_time", temp_comment->entry_time);
	json_object_append_boolean(json_details, "expires", temp_comment->expires);
	json_object_append_time_t(json_details, "expire_time", temp_comment->expire_time);
	json_object_append_string(json_details, "host_name", &percent_escapes,
	...
	json_object_append_string(json_details, "author", &percent_escapes, temp_comment->author);
...
int json_status_downtime_passes_selection(scheduled_downtime * temp_downtime,
										  int time_field, time_t start_time, time_t end_time,
										  unsigned object_types, unsigned downtime_types,
										  unsigned triggered, int triggered_by,
										  unsigned in_effect, char *host_name,
										  char *service_description)
{
	...
	case STATUS_TIME_FLEX_DOWNTIME_START:
		if ((start_time > 0) && (temp_downtime->flex_downtime_start < start_time)) {
			return 0;
		}
	...
	if (NULL != host_name) {
		if ((NULL == temp_downtime->host_name) || strcmp(temp_downtime->host_name, host_name)) {
			return 0;
		}
	}
	...
	if (NULL != service_description) {
		if ((NULL == temp_downtime->service_description) ||
			strcmp(temp_downtime->service_description, service_description)) {
			return 0;
		}
	}
...
json_object *json_status_downtime_selectors(unsigned format_options, int start,
											int count, int time_field, time_t start_time,
											time_t end_time, unsigned object_types,
									   ...
									   char *service_description)
{
	...
	for (temp_downtime = scheduled_downtime_list; temp_downtime != NULL;
		...
		if (!json_status_downtime_passes_selection(temp_downtime, time_field,
												   start_time, end_time, object_types,
												   downtime_types, triggered, triggered_by,
												   in_effect, host_name,
												   service_description)) {
			continue;
		}
...
json_object *json_status_downtimelist(unsigned format_options, int start,
									  int count, int details, int time_field,
									  time_t start_time, time_t end_time,
									  unsigned object_types, unsigned downtime_types,
									  ...
									  char *host_name, char *service_description)
{
	...
	for (temp_downtime = scheduled_downtime_list; temp_downtime != NULL;
		...
		if (passes_start_and_count_limits(start, count, current, counted)) {
			if (details > 0) {
				...
				json_array_append_integer(json_downtimelist_array, temp_downtime->downtime_id);
			}
			counted++;
		}
		current++;
	...
	if (details > 0) {
		json_object_append_object(json_data, "downtimelist", json_downtimelist_object);
	} else {
		json_object_append_array(json_data, "downtimelist", json_downtimelist_array);
	}

	return json_data;
}

json_object *json_status_downtime(unsigned format_options, scheduled_downtime * temp_downtime)
{

	json_object *json_downtime = json_new_object();
	json_object *json_details = json_new_object();

	json_object_append_integer(json_details, "downtime_id", temp_downtime->downtime_id);
...
void json_status_downtime_details(json_object * json_details, unsigned format_options,
								  scheduled_downtime * temp_downtime)
{
	...
	json_object_append_time_t(json_details, "entry_time", temp_downtime->entry_time);
	json_object_append_time_t(json_details, "start_time", temp_downtime->start_time);
	...
	json_object_append_integer(json_details, "triggered_by", (int)temp_downtime->triggered_by);
	json_object_append_integer(json_details, "duration", (int)temp_downtime->duration);
	json_object_append_boolean(json_details, "is_in_effect", temp_downtime->is_in_effect);
	json_object_append_boolean(json_details, "start_notification_sent",
							   temp_downtime->start_notification_sent);
	json_object_append_string(json_details, "author", &percent_escapes, temp_downtime->author);
...
json_object *json_status_program(unsigned format_options)
{
...
#ifndef JSON_NAGIOS_4X
	json_object_append_time_t(json_status, "last_command_check", last_command_check);
#endif
	json_object_append_time_t(json_status, "last_log_rotation", last_log_rotation);
	json_object_append_boolean(json_status, "enable_notifications", enable_notifications);
	json_object_append_boolean(json_status, "execute_service_checks", execute_service_checks);
	json_object_append_boolean(json_status, "accept_passive_service_checks",
							   accept_passive_service_checks);
	json_object_append_boolean(json_status, "execute_host_checks", execute_host_checks);
	json_object_append_boolean(json_status, "accept_passive_host_checks",
							   accept_passive_host_checks);
	json_object_append_boolean(json_status, "enable_event_handlers", enable_event_handlers);
	json_object_append_boolean(json_status, "obsess_over_services", obsess_over_services);
	json_object_append_boolean(json_status, "obsess_over_hosts", obsess_over_hosts);
	json_object_append_boolean(json_status, "check_service_freshness",
							   check_service_freshness);
	json_object_append_boolean(json_status, "check_host_freshness", check_host_freshness);
	json_object_append_boolean(json_status, "enable_flap_detection", enable_flap_detection);
#ifndef JSON_NAGIOS_4X
	...
	json_object_append_string(json_status, "active_scheduled_host_check_stats",
							  NULL, "%d,%d,%d",
							  check_statistics[ACTIVE_SCHEDULED_HOST_CHECK_STATS].
							  minute_stats[0],
							  check_statistics[ACTIVE_SCHEDULED_HOST_CHECK_STATS].
							  minute_stats[1],
							  check_statistics[ACTIVE_SCHEDULED_HOST_CHECK_STATS].
							  minute_stats[2]);
	json_object_append_string(json_status, "active_ondemand_host_check_stats", NULL,
							  "%d,%d,%d",
							  check_statistics[ACTIVE_ONDEMAND_HOST_CHECK_STATS].
							  minute_stats[0],
							  check_statistics[ACTIVE_ONDEMAND_HOST_CHECK_STATS].
							  minute_stats[1],
							  check_statistics[ACTIVE_ONDEMAND_HOST_CHECK_STATS].
							  minute_stats[2]);
	json_object_append_string(json_status, "passive_host_check_stats", NULL, "%d,%d,%d",
	...
	json_object_append_string(json_status, "active_scheduled_service_check_stats", NULL,
							  "%d,%d,%d",
							  ...
							  check_statistics[ACTIVE_SCHEDULED_SERVICE_CHECK_STATS].
							  minute_stats[2]);
	json_object_append_string(json_status, "active_ondemand_service_check_stats", NULL,
							  "%d,%d,%d",
							  check_statistics[ACTIVE_ONDEMAND_SERVICE_CHECK_STATS].
							  minute_stats[0],
							  check_statistics[ACTIVE_ONDEMAND_SERVICE_CHECK_STATS].
							  minute_stats[1],
							  check_statistics[ACTIVE_ONDEMAND_SERVICE_CHECK_STATS].
							  minute_stats[2]);
	json_object_append_string(json_status, "passive_service_check_stats", NULL, "%d,%d,%d",
	...
	json_object_append_string(json_status, "cached_host_check_stats", NULL, "%d,%d,%d",
							  ...
							  check_statistics[ACTIVE_CACHED_HOST_CHECK_STATS].
							  minute_stats[2]);
	json_object_append_string(json_status, "cached_service_check_stats", NULL, "%d,%d,%d",
							  check_statistics[ACTIVE_CACHED_SERVICE_CHECK_STATS].
							  minute_stats[0],
							  check_statistics[ACTIVE_CACHED_SERVICE_CHECK_STATS].
							  minute_stats[1],
							  check_statistics[ACTIVE_CACHED_SERVICE_CHECK_STATS].
							  minute_stats[2]);
	json_object_append_string(json_status, "external_command_stats", NULL, "%d,%d,%d",
	...
	json_object_append_string(json_status, "parallel_host_check_stats", NULL, "%d,%d,%d",
	...
	json_object_append_string(json_status, "serial_host_check_stats", NULL, "%d,%d,%d",
...
json_object *json_status_performance(void)
{
	...
	json_object_append_real(json_temp, "average",
							(double)((double)total_service_execution_time /
	...
	json_object_append_real(json_temp, "average",
							(double)((double)total_service_latency /
									 (double)total_active_service_checks));
	...
	json_object_append_integer(json_checks, "15min", passive_service_checks_15min);
	json_object_append_integer(json_checks, "1hour", passive_service_checks_1hour);
	json_object_append_integer(json_checks, "start", passive_service_checks_start);
	...
	json_object_append_object(json_programstatus, "service_checks", json_service_checks);
	...
	json_object_append_real(json_temp, "average",
							(double)((double)total_host_execution_time /
	...
	json_object_append_real(json_temp, "average",
							(double)((double)total_host_latency /
									 (double)total_active_host_checks));
	json_object_append_object(json_metrics, "check_latency", json_temp);
	...
	json_object_append_real(json_temp, "average",
							(double)((double)total_host_percent_change_a /
									 (double)total_active_host_checks));
	json_object_append_object(json_metrics, "percent_state_change", json_temp);
	...
	json_object_append_real(json_temp, "average",
							(double)((double)total_host_percent_change_b /
									 (double)total_active_host_checks));
	json_object_append_object(json_metrics, "percent_state_change", json_temp);
	...
	json_object_append_object(json_programstatus, "host_checks", json_host_checks);
	...
	json_object_append_object(json_check_statistics, "active_ondemand_host_checks", json_temp);

	json_temp = json_new_object();
	json_object_append_integer(json_temp, "1min", program_stats[PARALLEL_HOST_CHECK_STATS][0]);
	json_object_append_integer(json_temp, "5min", program_stats[PARALLEL_HOST_CHECK_STATS][1]);
	json_object_append_integer(json_temp, "15min",
							   program_stats[PARALLEL_HOST_CHECK_STATS][2]);
	json_object_append_object(json_check_statistics, "parallel_host_checks", json_temp);

	json_temp = json_new_object();
	json_object_append_integer(json_temp, "1min", program_stats[SERIAL_HOST_CHECK_STATS][0]);
	json_object_append_integer(json_temp, "5min", program_stats[SERIAL_HOST_CHECK_STATS][1]);
	json_object_append_integer(json_temp, "15min", program_stats[SERIAL_HOST_CHECK_STATS][2]);
	json_object_append_object(json_check_statistics, "serial_host_checks", json_temp);
	...
	json_object_append_object(json_check_statistics, "cached_host_checks", json_temp);

	json_temp = json_new_object();
	json_object_append_integer(json_temp, "1min", program_stats[PASSIVE_HOST_CHECK_STATS][0]);
	json_object_append_integer(json_temp, "5min", program_stats[PASSIVE_HOST_CHECK_STATS][1]);
	json_object_append_integer(json_temp, "15min", program_stats[PASSIVE_HOST_CHECK_STATS][2]);
	json_object_append_object(json_check_statistics, "passive_host_checks", json_temp);
	...
	json_object_append_object(json_check_statistics, "cached_service_checks", json_temp);
	...
	json_object_append_object(json_check_statistics, "passive_service_checks", json_temp);

	json_temp = json_new_object();
	json_object_append_integer(json_temp, "1min", program_stats[EXTERNAL_COMMAND_STATS][0]);
	json_object_append_integer(json_temp, "5min", program_stats[EXTERNAL_COMMAND_STATS][1]);
	json_object_append_integer(json_temp, "15min", program_stats[EXTERNAL_COMMAND_STATS][2]);
	json_object_append_object(json_check_statistics, "external_commands", json_temp);

	json_object_append_object(json_programstatus, "check_statistics", json_check_statistics);
	...
	json_object_append_object(json_programstatus, "buffer_usage", json_buffer_usage);
