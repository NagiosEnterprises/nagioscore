...
void display_page_header(void)
{
	...
	if (display_header == TRUE) {
		...
		if (show_all_hosts == TRUE)
			...
			snprintf(temp_buffer, sizeof(temp_buffer) - 1, "Network Map For Host <I>%s</I>",
					 host_name);
...
#ifdef LEGACY_GRAPHICAL_CGIS
			printf
				("<a href='%s?host=all&max_width=%d&max_height=%d'>View Status Map For All Hosts</a><BR>",
				 STATUSMAP_CGI, max_image_width, max_image_height);
#else
			printf
				("<a href='%s?host=all&max_width=%d&max_height=%d'>View Status Map For All Hosts</a><BR>",
				 LEGACY_STATUSMAP_CGI, max_image_width, max_image_height);
#endif
		...
		printf
			("[ Raw Image Size: %d x %d pixels | Scaling Factor: %1.2lf | Scaled Image Size: %d x %d pixels ]",
			 canvas_width, canvas_height, scaling_factor, (int)(canvas_width * scaling_factor),
			 (int)(canvas_height * scaling_factor));
		printf("</font></div></p>\n");

		printf("<p><div align=center><font size=-1>\n");
		printf("[ Canvas_x: %d | Canvas_y: %d | Canvas_width: %d | Canvas_height: %d ]",
		...
		if (user_supplied_canvas == FALSE && strcmp(host_name, "all")
			...
			if (proximity_width <= 0)
				...
				current_zoom_granularity =
					(total_image_width - proximity_width) / zoom_width_granularity;
			...
			for (zoom = 0; zoom <= 10; zoom++) {
...
#ifdef LEGACY_GRAPHICAL_CGIS
				printf
					("<td valign=center><a href='%s?host=%s&layout=%d&max_width=%d&max_height=%d&proximity_width=%d&proximity_height=%d%s%s",
					 STATUSMAP_CGI, url_encode(host_name), layout_method, max_image_width,
					 max_image_height, zoom_width, zoom_height,
					 (display_header == TRUE) ? "" : "&noheader",
					 (display_popups == FALSE) ? "&nopopups" : "");
#else
				printf
					("<td valign=center><a href='%s?host=%s&layout=%d&max_width=%d&max_height=%d&proximity_width=%d&proximity_height=%d%s%s",
					 LEGACY_STATUSMAP_CGI, url_encode(host_name), layout_method,
					 max_image_width, max_image_height, zoom_width, zoom_height,
					 (display_header == TRUE) ? "" : "&noheader",
					 (display_popups == FALSE) ? "&nopopups" : "");
#endif
				...
				printf("<img src='%s%s' border=0 alt='%d' title='%d'></a></td>\n",
					   url_images_path,
					   (current_zoom_granularity == zoom) ? ZOOM2_ICON : ZOOM1_ICON, zoom,
					   zoom);
			}
		...
		printf("<option value=%d %s>User-supplied coords\n", LAYOUT_USER_SUPPLIED,
			   (layout_method == LAYOUT_USER_SUPPLIED) ? "selected" : "");
#endif
		printf("<option value=%d %s>Depth layers\n", LAYOUT_SUBLAYERS,
			   (layout_method == LAYOUT_SUBLAYERS) ? "selected" : "");
		printf("<option value=%d %s>Collapsed tree\n", LAYOUT_COLLAPSED_TREE,
			   (layout_method == LAYOUT_COLLAPSED_TREE) ? "selected" : "");
		printf("<option value=%d %s>Balanced tree\n", LAYOUT_BALANCED_TREE,
			   (layout_method == LAYOUT_BALANCED_TREE) ? "selected" : "");
		...
		printf("<option value=%d %s>Circular (Marked Up)\n", LAYOUT_CIRCULAR_MARKUP,
			   (layout_method == LAYOUT_CIRCULAR_MARKUP) ? "selected" : "");
		printf("<option value=%d %s>Circular (Balloon)\n", LAYOUT_CIRCULAR_BALLOON,
			   (layout_method == LAYOUT_CIRCULAR_BALLOON) ? "selected" : "");
		printf("</select>\n");
		printf("</td>\n");
		...
		for (temp_hostgroup = hostgroup_list; temp_hostgroup != NULL;
			...
			printf("<option value='%s' %s>%s\n", escape_string(temp_hostgroup->group_name),
				   (found == 1) ? "SELECTED" : "", temp_hostgroup->alias);
		}
		printf("</select>\n");
...
void display_map(void)
{
	...
	if (create_type == CREATE_HTML) {
		printf("<P><DIV ALIGN=center>\n");
#ifdef LEGACY_GRAPHICAL_CGIS
		printf("<img src='%s?host=%s&createimage&time=%llu", STATUSMAP_CGI,
			   url_encode(host_name), (unsigned long long)time(NULL));
#else
		printf("<img src='%s?host=%s&createimage&time=%llu", LEGACY_STATUSMAP_CGI,
			   url_encode(host_name), (unsigned long long)time(NULL));
#endif
		printf
			("&canvas_x=%d&canvas_y=%d&canvas_width=%d&canvas_height=%d&max_width=%d&max_height=%d&layout=%d%s%s%s",
			 canvas_x, canvas_y, canvas_width, canvas_height, max_image_width,
			 max_image_height, layout_method, (use_links == FALSE) ? "&nolinks" : "",
			 (use_text == FALSE) ? "&notext" : "",
			 (use_highlights == FALSE) ? "&nohighlights" : "");
		print_layer_url(TRUE);
...
void calculate_host_coords(void)
{
	...
	if (layout_method == LAYOUT_SUBLAYERS) {
		...
		center_x =
			(((DEFAULT_NODE_WIDTH * max_layer_width) +
			  (DEFAULT_NODE_HSPACING * (max_layer_width - 1))) / 2) + offset_x;
		...
		for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
			...
			if (is_host_immediate_parent_of_host(this_host, temp_host) == TRUE) {
				...
				temp_host->x_2d =
					center_x -
					(((parent_hosts * DEFAULT_NODE_WIDTH) +
					  ((parent_hosts - 1) * DEFAULT_NODE_HSPACING)) / 2) +
					(current_parent_host * (DEFAULT_NODE_WIDTH + DEFAULT_NODE_HSPACING)) +
			...
			/* this is an immediate child of the "main" host we're drawing */
				...
				temp_host->x_2d =
					center_x -
					(((child_hosts * DEFAULT_NODE_WIDTH) +
					  ((child_hosts - 1) * DEFAULT_NODE_HSPACING)) / 2) +
					(current_child_host * (DEFAULT_NODE_WIDTH + DEFAULT_NODE_HSPACING)) +
					(DEFAULT_NODE_WIDTH / 2);
				if (this_host == NULL)
					temp_host->y_2d = (DEFAULT_NODE_HEIGHT + DEFAULT_NODE_VSPACING) + offset_y;
				else
					temp_host->y_2d =
						((DEFAULT_NODE_HEIGHT + DEFAULT_NODE_VSPACING) * 2) + offset_y;
				current_child_host++;
		...
		for (current_layer = 1;; current_layer++) {
			...
			for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
				...
				if (host_child_depth_separation(this_host, temp_host) == current_layer) {
					...
					temp_host->x_2d =
						center_x -
						(((layer_members * DEFAULT_NODE_WIDTH) +
						  ((layer_members - 1) * DEFAULT_NODE_HSPACING)) / 2) +
						(current_layer_member * (DEFAULT_NODE_WIDTH + DEFAULT_NODE_HSPACING)) +
						(DEFAULT_NODE_WIDTH / 2);
					if (this_host == NULL)
						temp_host->y_2d =
							((DEFAULT_NODE_HEIGHT + DEFAULT_NODE_VSPACING) * current_layer) +
							offset_y;
					else
						temp_host->y_2d =
							((DEFAULT_NODE_HEIGHT + DEFAULT_NODE_VSPACING) * (current_layer +
	...
	/***** "BALANCED" TREE MODE *****/
		...
		/* calculate center x coord */
		center_x =
			(((DEFAULT_NODE_WIDTH * max_drawing_width) +
			  (DEFAULT_NODE_HSPACING * (max_drawing_width - 1))) / 2) + offset_x;
		...
		calculate_balanced_tree_coords(this_host, center_x,
									   DEFAULT_NODE_HEIGHT + DEFAULT_NODE_VSPACING + offset_y);

	}


	/***** CIRCULAR LAYOUT MODE *****/
	else if (layout_method == LAYOUT_CIRCULAR || layout_method == LAYOUT_CIRCULAR_MARKUP
			 || layout_method == LAYOUT_CIRCULAR_BALLOON) {
...
void find_eligible_hosts(void)
{
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		else if (temp_host->x_2d < (canvas_x - DEFAULT_NODE_WIDTH)
				 || temp_host->x_2d > (canvas_x + canvas_width))
			temp_host->should_be_drawn = FALSE;

		/* make sure y coordinates fall within canvas bounds */
		else if (temp_host->y_2d < (canvas_y - DEFAULT_NODE_HEIGHT)
				 || temp_host->y_2d > (canvas_y + canvas_height))
			temp_host->should_be_drawn = FALSE;
...
void load_background_image(void)
{
	...
	snprintf(temp_buffer, sizeof(temp_buffer) - 1, "%s%s", physical_images_path,
...
void draw_background_image(void)
{
	...
	/* bail out if we shouldn't be drawing a background image */
	if (create_type == CREATE_HTML || layout_method != LAYOUT_USER_SUPPLIED
	...
	gdImageCopy(map_image, background_image, 0, 0, canvas_x, canvas_y, canvas_width,
				canvas_height);
...
void draw_host_links(void)
{
	...
	for (this_host = host_list; this_host != NULL; this_host = this_host->next) {
		...
		if (this_host->parent_hosts == NULL && layout_method != LAYOUT_USER_SUPPLIED
			&& draw_nagios_icon == TRUE) {

			x = this_host->x_2d + (DEFAULT_NODE_WIDTH / 2) - canvas_x;
			y = this_host->y_2d + (DEFAULT_NODE_WIDTH / 2) - canvas_y;

			draw_line(x, y, nagios_icon_x + (DEFAULT_NODE_WIDTH / 2) - canvas_x,
					  nagios_icon_y + (DEFAULT_NODE_WIDTH / 2) - canvas_y, color_black);
		}

		/* this is a child of the main host we're drawing in auto-layout mode... */
		if (layout_method != LAYOUT_USER_SUPPLIED && draw_child_links == TRUE
			&& number_of_immediate_child_hosts(this_host) > 0
			&& is_host_immediate_child_of_host(main_host, this_host) == TRUE) {
			...
			if (this_hoststatus != NULL) {
				if (this_hoststatus->status == SD_HOST_DOWN
			...
			draw_dashed_line(x, y, x, y + DEFAULT_NODE_HEIGHT + DEFAULT_NODE_VSPACING,
							 status_color);

			/* draw arrow tips */
			draw_line(x, y + DEFAULT_NODE_HEIGHT + DEFAULT_NODE_VSPACING, x - 5,
					  y + DEFAULT_NODE_HEIGHT + DEFAULT_NODE_VSPACING - 5, color_black);
			draw_line(x, y + DEFAULT_NODE_HEIGHT + DEFAULT_NODE_VSPACING, x + 5,
					  y + DEFAULT_NODE_HEIGHT + DEFAULT_NODE_VSPACING - 5, color_black);
		}

		/* this is a parent of the main host we're drawing in auto-layout mode... */
		if (layout_method != LAYOUT_USER_SUPPLIED && draw_parent_links == TRUE
			&& is_host_immediate_child_of_host(this_host, main_host) == TRUE) {
		...
		for (temp_hostsmember = this_host->parent_hosts; temp_hostsmember != NULL;
			...
			if ((child_in_layer_list == TRUE && parent_in_layer_list == FALSE)
				|| (child_in_layer_list == FALSE && parent_in_layer_list == TRUE))
				dotted_line = TRUE;
			/* both hosts should not be drawn, so use a dotted line */
			else if ((child_in_layer_list == FALSE && parent_in_layer_list == FALSE
					  && exclude_layers == FALSE) || (child_in_layer_list == TRUE
													  && parent_in_layer_list == TRUE
													  && exclude_layers == TRUE))
				dotted_line = TRUE;
			...
			if (parent_hoststatus != NULL) {
				if (parent_hoststatus->status == SD_HOST_DOWN
					|| parent_hoststatus->status == SD_HOST_UNREACHABLE)
					status_color = color_red;
				else
					status_color = color_black;
			} else
			...
			/* draw the link */
			if (dotted_line == TRUE)
				draw_dotted_line((this_host->x_2d + (DEFAULT_NODE_WIDTH / 2)) - canvas_x,
								 (this_host->y_2d + (DEFAULT_NODE_WIDTH) / 2) - canvas_y,
								 (parent_host->x_2d + (DEFAULT_NODE_WIDTH / 2)) - canvas_x,
								 (parent_host->y_2d + (DEFAULT_NODE_WIDTH / 2)) - canvas_y,
								 status_color);
			else
				draw_line((this_host->x_2d + (DEFAULT_NODE_WIDTH / 2)) - canvas_x,
						  (this_host->y_2d + (DEFAULT_NODE_WIDTH) / 2) - canvas_y,
						  (parent_host->x_2d + (DEFAULT_NODE_WIDTH / 2)) - canvas_x,
						  (parent_host->y_2d + (DEFAULT_NODE_WIDTH / 2)) - canvas_y,
						  status_color);
...
void draw_hosts(void)
{
	...
	if (layout_method != LAYOUT_USER_SUPPLIED && draw_nagios_icon == TRUE) {
		...
		snprintf(image_input_file, sizeof(image_input_file) - 1, "%s%s",
				 physical_logo_images_path, NAGIOS_GD2_ICON);
		...
		if (create_type == CREATE_IMAGE)
			draw_text("Nagios Process", x1 + (DEFAULT_NODE_WIDTH / 2),
					  y1 + DEFAULT_NODE_HEIGHT, color_black);
	}
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		if (create_type == CREATE_IMAGE) {
			...
			if (layout_method == LAYOUT_CIRCULAR_BALLOON) {
				...
				if (average_host_services == 0)
					...
					host_services_ratio =
						(double)((double)host_services / (double)average_host_services);
				...
				gdImageArc(map_image, x1 + (DEFAULT_NODE_WIDTH / 2),
						   y1 + (DEFAULT_NODE_WIDTH / 2), outer_radius, outer_radius, 0, 360,
						   color_blue);
				...
				if (translated_x > 0 && translated_y > 0 && translated_x < canvas_width
					&& translated_y < canvas_height)
					gdImageFillToBorder(map_image, translated_x, translated_y, color_blue,
										time_color);

				/* border of node should reflect current state */
				for (current_radius = outer_radius; current_radius >= inner_radius;
					 current_radius--)
					gdImageArc(map_image, x1 + (DEFAULT_NODE_WIDTH / 2),
							   y1 + (DEFAULT_NODE_WIDTH / 2), current_radius, current_radius,
							   0, 360, status_color);

				/* draw circles around the selected host (if there is one) */
				if (!strcmp(host_name, temp_host->name) && use_highlights == TRUE) {
					for (current_radius = DEFAULT_NODE_WIDTH * 2; current_radius > 0;
						 current_radius -= 10)
						gdImageArc(map_image, x1 + (DEFAULT_NODE_WIDTH / 2),
								   y1 + (DEFAULT_NODE_WIDTH / 2), current_radius,
								   current_radius, 0, 360, status_color);
				}
			}
			...
			/* normal method is to use icons for hosts... */
				...
				if (temp_host != NULL && use_highlights == TRUE) {
					if (temp_host->parent_hosts == NULL) {
						gdImageArc(map_image, x1 + (DEFAULT_NODE_WIDTH / 2),
								   y1 + (DEFAULT_NODE_WIDTH / 2), (DEFAULT_NODE_WIDTH * 2),
								   (DEFAULT_NODE_WIDTH * 2), 0, 360, status_color);
						draw_line(x1 - (DEFAULT_NODE_WIDTH / 2), y1 + (DEFAULT_NODE_WIDTH / 2),
								  x1 + (DEFAULT_NODE_WIDTH * 3 / 2),
								  y1 + (DEFAULT_NODE_WIDTH / 2), status_color);
						draw_line(x1 + (DEFAULT_NODE_WIDTH / 2), y1 - (DEFAULT_NODE_WIDTH / 2),
								  x1 + (DEFAULT_NODE_WIDTH / 2),
								  y1 + (DEFAULT_NODE_WIDTH * 3 / 2), status_color);
					}
				}
				...
				if (has_image == TRUE) {
					...
					/* get the name of the image file to open for the logo */
					snprintf(image_input_file, sizeof(image_input_file) - 1, "%s%s",
							 physical_logo_images_path, temp_host->statusmap_image);
					image_input_file[sizeof(image_input_file) - 1] = '\x0';
					...
					/* copy the logo image to the canvas image... */
					if (logo_image != NULL) {
						gdImageCopy(map_image, logo_image, x1, y1, 0, 0, logo_image->sx,
									logo_image->sy);
						gdImageDestroy(logo_image);
					} else
						has_image = FALSE;
				...
				if (has_image == FALSE) {
					...
					if (unknown_logo_image != NULL)
						gdImageCopy(map_image, unknown_logo_image, x1, y1, 0, 0,
									unknown_logo_image->sx, unknown_logo_image->sy);

					else {

						/* last ditch effort - draw a host bounding box */
						draw_line(x1, y1, x1, y1 + DEFAULT_NODE_WIDTH, color_black);
						draw_line(x1, y1 + DEFAULT_NODE_WIDTH, x2, y1 + DEFAULT_NODE_WIDTH,
								  color_black);
						draw_line(x2, y1 + DEFAULT_NODE_WIDTH, x2, y1, color_black);
						draw_line(x2, y1, x1, y1, color_black);
					}
			...
			draw_host_text(temp_host->name, x1 + (DEFAULT_NODE_WIDTH / 2),
						   y1 + DEFAULT_NODE_HEIGHT);
		}
		...
		/* we're creating HTML image map... */
			...
			/* coordinates */
			printf("coords='%d,%d,%d,%d' ", (int)(x1 * scaling_factor),
				   (int)(y1 * scaling_factor),
				   (int)((x1 + DEFAULT_NODE_WIDTH) * scaling_factor),
				   (int)((y1 + DEFAULT_NODE_HEIGHT) * scaling_factor));
			...
			if (!strcmp(host_name, temp_host->name))
...
#ifdef LEGACY_GRAPHICAL_CGIS
				printf
					("href='%s?host=%s&layout=%d&max_width=%d&max_height=%d&proximity_width=%d&proximity_height=%d%s%s%s%s%s",
					 STATUSMAP_CGI, url_encode(temp_host->name), layout_method,
					 max_image_width, max_image_height, proximity_width, proximity_height,
					 (display_header == TRUE) ? "" : "&noheader",
					 (use_links == FALSE) ? "&nolinks" : "",
					 (use_text == FALSE) ? "&notext" : "",
					 (use_highlights == FALSE) ? "&nohighlights" : "",
					 (display_popups == FALSE) ? "&nopopups" : "");
#else
				printf
					("href='%s?host=%s&layout=%d&max_width=%d&max_height=%d&proximity_width=%d&proximity_height=%d%s%s%s%s%s",
					 LEGACY_STATUSMAP_CGI, url_encode(temp_host->name), layout_method,
					 max_image_width, max_image_height, proximity_width, proximity_height,
					 (display_header == TRUE) ? "" : "&noheader",
					 (use_links == FALSE) ? "&nolinks" : "",
					 (use_text == FALSE) ? "&notext" : "",
...
void draw_text(char *buffer, int x, int y, int text_color)
{
	...
	if (layout_method != LAYOUT_CIRCULAR_MARKUP)
		gdImageFilledRectangle(map_image, x - (string_width / 2) - 2, y - (2 * string_height),
							   x + (string_width / 2) + 2, y - string_height, color_white);
	gdImageString(map_image, gdFontSmall, x - (string_width / 2), y - (2 * string_height),
				  (unsigned char *)buffer, text_color);
...
void write_host_popup_text(host * hst)
{
	...
	printf("<td class=\\\"popupText\\\"><i>%s</i></td></tr>",
		   (hst->icon_image_alt == NULL) ? "" : html_encode(hst->icon_image_alt, TRUE));

	printf
		("<tr><td class=\\\"popupText\\\">Name:</td><td class=\\\"popupText\\\"><b>%s</b></td></tr>",
		 escape_string(hst->name));
	printf
		("<tr><td class=\\\"popupText\\\">Alias:</td><td class=\\\"popupText\\\"><b>%s</b></td></tr>",
		 escape_string(hst->alias));
	printf
		("<tr><td class=\\\"popupText\\\">Address:</td><td class=\\\"popupText\\\"><b>%s</b></td></tr>",
	...
	printf("</b></td></tr>");
	printf
		("<tr><td class=\\\"popupText\\\">Status Information:</td><td class=\\\"popupText\\\"><b>%s</b></td></tr>",
	...
	snprintf(state_duration, sizeof(state_duration) - 1, "%2dd %2dh %2dm %2ds%s", days, hours,
			 minutes, seconds, (temp_status->last_state_change == (time_t) 0) ? "+" : "");
	state_duration[sizeof(state_duration) - 1] = '\x0';
	printf
		("<tr><td class=\\\"popupText\\\">State Duration:</td><td class=\\\"popupText\\\"><b>%s</b></td></tr>",
		 state_duration);

	get_time_string(&temp_status->last_check, date_time, (int)sizeof(date_time),
					SHORT_DATE_TIME);
	printf
		("<tr><td class=\\\"popupText\\\">Last Status Check:</td><td class=\\\"popupText\\\"><b>%s</b></td></tr>",
		 (temp_status->last_check == (time_t) 0) ? "N/A" : date_time);
	get_time_string(&temp_status->last_state_change, date_time, (int)sizeof(date_time),
					SHORT_DATE_TIME);
	printf
		("<tr><td class=\\\"popupText\\\">Last State Change:</td><td class=\\\"popupText\\\"><b>%s</b></td></tr>",
		 (temp_status->last_state_change == (time_t) 0) ? "N/A" : date_time);

	printf
		("<tr><td class=\\\"popupText\\\">Parent Host(s):</td><td class=\\\"popupText\\\"><b>");
	if (hst->parent_hosts == NULL)
		printf("None (This is a root host)");
	else {
		for (temp_hostsmember = hst->parent_hosts; temp_hostsmember != NULL;
			 temp_hostsmember = temp_hostsmember->next)
			printf("%s%s", (temp_hostsmember == hst->parent_hosts) ? "" : ", ",
				   html_encode(temp_hostsmember->host_name, TRUE));
	}
	printf("</b></td></tr>");

	printf
		("<tr><td class=\\\"popupText\\\">Immediate Child Hosts:</td><td class=\\\"popupText\\\"><b>");
...
int initialize_graphics(void)
{
	...
	snprintf(physical_logo_images_path, sizeof(physical_logo_images_path) - 1, "%slogos/",
	...
	snprintf(image_input_file, sizeof(image_input_file) - 1, "%s%s", physical_logo_images_path,
			 UNKNOWN_GD2_ICON);
	image_input_file[sizeof(image_input_file) - 1] = '\x0';
...
void write_popup_code(void)
{
	...
	printf("<!--\n");
	printf
		("// JavaScript popup based on code originally found at http://www.helpmaster.com/htmlhelp/javascript/popjbpopup.htm\n");
	...
	printf
		("table += \"<tr><td bgcolor='%s' class='popupText'>\" + text + \"</td></tr></table>\";\n",
		 background_color);
	...
	printf
		("else if (document.all||document.documentElement) document.popup.style.visibility = \"hidden\";\n");
		...
		if (is_host_immediate_child_of_host(parent, temp_host) == TRUE) {
			...
			if (this_depth >= 0
				&& (have_min_depth == FALSE
					|| (have_min_depth == TRUE && (this_depth < min_depth)))) {
				have_min_depth = TRUE;
...
void calculate_balanced_tree_coords(host * parent, int x, int y)
{
	...
	start_drawing_x =
		x -
		(((DEFAULT_NODE_WIDTH * parent_drawing_width) +
		  (DEFAULT_NODE_HSPACING * (parent_drawing_width - 1))) / 2);
		...
		if (is_host_immediate_child_of_host(parent, temp_host) == TRUE) {
			...
			temp_host->x_2d =
				...
				(((DEFAULT_NODE_WIDTH * this_drawing_width) +
				  (DEFAULT_NODE_HSPACING * (this_drawing_width - 1))) / 2);
			temp_host->y_2d = y + DEFAULT_NODE_HEIGHT + DEFAULT_NODE_VSPACING;
			temp_host->have_2d_coords = TRUE;
			temp_host->should_be_drawn = TRUE;

			current_drawing_x +=
				(this_drawing_width * DEFAULT_NODE_WIDTH) +
				((this_drawing_width - 1) * DEFAULT_NODE_HSPACING) + DEFAULT_NODE_HSPACING;
...
void calculate_circular_layer_coords(host * parent, double start_angle, double useable_angle,
									 int layer, int radius)
{
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		if (is_host_immediate_child_of_host(parent, temp_host) == TRUE) {
			...
			available_angle =
				useable_angle * ((double)this_drawing_width / (double)parent_drawing_width);
			...
			/* recurse into child host ... */
			calculate_circular_layer_coords(temp_host,
											current_drawing_angle +
											((available_angle - clipped_available_angle) / 2),
											clipped_available_angle, layer + 1,
											radius + CIRCULAR_DRAWING_RADIUS);
...
void draw_circular_layer_markup(host * parent, double start_angle, double useable_angle,
								int layer, int radius)
{
	int 		parent_drawing_width = 0;
	int 		this_drawing_width = 0;
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		if (is_host_immediate_child_of_host(parent, temp_host) == TRUE) {
			...
			x_coord[0] =
				-(sin(-current_drawing_angle * (M_PI / 180.0)) *
				  (radius - (CIRCULAR_DRAWING_RADIUS / 2)));
			y_coord[0] =
				-(sin((90 + current_drawing_angle) * (M_PI / 180.0)) *
				  (radius - (CIRCULAR_DRAWING_RADIUS / 2)));
			x_coord[1] =
				-(sin(-current_drawing_angle * (M_PI / 180.0)) *
				  (radius + (CIRCULAR_DRAWING_RADIUS / 2)));
			...
			x_coord[2] =
				-(sin((-(current_drawing_angle + available_angle)) * (M_PI / 180.0)) *
				  (radius - (CIRCULAR_DRAWING_RADIUS / 2)));
			y_coord[2] =
				-(sin((90 + current_drawing_angle + available_angle) * (M_PI / 180.0)) *
				  (radius - (CIRCULAR_DRAWING_RADIUS / 2)));
			x_coord[3] =
				-(sin((-(current_drawing_angle + available_angle)) * (M_PI / 180.0)) *
				  (radius + (CIRCULAR_DRAWING_RADIUS / 2)));
			...
			if (immediate_children > 1 || layer > 1) {
				...
				gdImageLine(map_image, (int)x_coord[0] + x_offset, (int)y_coord[0] + y_offset,
							(int)x_coord[1] + x_offset, (int)y_coord[1] + y_offset,
							color_lightgrey);

				/* draw "rightmost" divider */
				gdImageLine(map_image, (int)x_coord[2] + x_offset, (int)y_coord[2] + y_offset,
							(int)x_coord[3] + x_offset, (int)y_coord[3] + y_offset,
							color_lightgrey);
			}
			...
			/* draw inner arc */
			gdImageArc(map_image, x_offset, y_offset,
					   (radius - (CIRCULAR_DRAWING_RADIUS / 2)) * 2,
					   (radius - (CIRCULAR_DRAWING_RADIUS / 2)) * 2, floor(arc_start_angle),
					   ceil(arc_end_angle), color_lightgrey);
			...
			center_x =
				-(sin(-(current_drawing_angle + (available_angle / 2.0)) * (M_PI / 180.0)) *
				  (radius));
			center_y =
				-(sin((90 + current_drawing_angle + (available_angle / 2.0)) * (M_PI / 180.0))
				  * (radius));
			...
			if (translated_x > 0 && translated_y > 0 && translated_x < canvas_width
				&& translated_y < canvas_height)
				gdImageFillToBorder(map_image, translated_x, translated_y, color_lightgrey,
									bgcolor);

			/* recurse into child host ... */
			draw_circular_layer_markup(temp_host,
									   current_drawing_angle +
									   ((available_angle - clipped_available_angle) / 2),
									   clipped_available_angle, layer + 1,
									   radius + CIRCULAR_DRAWING_RADIUS);
