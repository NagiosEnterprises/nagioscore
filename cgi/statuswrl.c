...
void display_world(void)
{
	...
	if (coordinates_were_specified == FALSE) {
		...
		printf
			("string [ \"Error: You have not supplied any 3-D drawing coordinates.\", \"Read the documentation for more information on supplying\", \"3-D drawing coordinates by defining\", \"extended host information entries in your config files.\" ]\n");
...
void write_global_vrml_data(void)
{
	...
	if (custom_viewpoint == TRUE) {
		printf("\n");
		printf("Viewpoint{\n");
		printf("position %2.2f %2.2f %2.2f\n", custom_viewpoint_x, custom_viewpoint_y,
			   custom_viewpoint_z);
	...
	printf("\n");
	printf("Viewpoint{\n");
	printf("position %2.2f %2.2f %2.2f\n", min_x_coord + ((max_x_coord - min_x_coord) / 2.0),
		   min_y_coord + ((max_y_coord - min_y_coord) / 2.0), viewpoint_z);
	printf("fieldOfView 0.78\n");
	...
	for (child_host = host_list; child_host != NULL; child_host = child_host->next) {
		...
		for (parent_host = host_list; parent_host != NULL; parent_host = parent_host->next) {
			...
			if (is_host_immediate_child_of_host(child_host, parent_host) == TRUE) {
				...
				draw_host_link(parent_host, parent_host->x_3d, parent_host->y_3d,
							   parent_host->z_3d, child_host->x_3d, child_host->y_3d,
							   child_host->z_3d);
			}
		}
	}
...
void draw_process_icon(void)
{
	...
	for (child_host = host_list; child_host != NULL; child_host = child_host->next) {
		...
		if (is_host_immediate_child_of_host(NULL, child_host) == TRUE)
			draw_host_link(NULL, nagios_icon_x, nagios_icon_y, 0.0, child_host->x_3d,
						   child_host->y_3d, child_host->z_3d);
	}
