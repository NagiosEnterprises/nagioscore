	...
	*output_buffer = (char *)strdup("");
	...
	while (buf_ptr) {
		...
		/* looks like we're in a macro, so process it... */
			...
			free_macro = FALSE;
			result =
				grab_macro_value_r(mac, temp_buffer, &selected_macro, &macro_options,
								   &free_macro);
			...
			if (result == OK) ; /* do nothing special if things worked out ok */
			...
			/* a non-macro, just some user-defined string between two $s */
				...
				*output_buffer =
					(char *)realloc(*output_buffer,
									strlen(*output_buffer) + strlen(temp_buffer) + 3);
				strcat(*output_buffer, "$");
				strcat(*output_buffer, temp_buffer);
				strcat(*output_buffer, "$");
			}
			...
			if (selected_macro != NULL) {
				log_debug_info(DEBUGL_MACROS, 2,
							   "  Processed '%s', Free: %d,  Cleaning options: %d\n",
				...
				if (macro_options & options & (STRIP_ILLEGAL_MACRO_CHARS | ESCAPE_MACRO_CHARS)) {
					...
					if (selected_macro != NULL
						&& (cleaned_macro =
							clean_macro_chars(selected_macro, options)) != NULL) {
						*output_buffer =
							(char *)realloc(*output_buffer,
											strlen(*output_buffer) + strlen(cleaned_macro) +
											1);
				...
				/* others are not cleaned */
					...
					if (selected_macro != NULL) {
						*output_buffer =
							(char *)realloc(*output_buffer,
											strlen(*output_buffer) + strlen(selected_macro) +
											1);
...
/* this is the big one */
int grab_macro_value_r(nagios_macros * mac, char *macro_buffer, char **output,
					   int *clean_options, int *free_macro)
{
	char	   *buf = NULL;
	char	   *ptr = NULL;
				...
				for (temp_contactsmember = temp_contactgroup->members;
					...
					if (*output == NULL)
						*output = (char *)strdup(temp_buffer);
						...
						if ((*output =
							 (char *)realloc(*output,
											 strlen(*output) + strlen(temp_buffer) +
											 delimiter_len + 1)) == NULL)
							continue;
						strcat(*output, arg[1]);
...
int grab_macrox_value_r(nagios_macros * mac, int macro_type, char *arg1, char *arg2,
						char **output, int *free_macro)
{
	host	   *temp_host = NULL;
	...
	*free_macro = FALSE;
	...
	case MACRO_HOSTANDSERVICESIMPORTANCE:
		...
		if (arg2 == NULL) {
			...
			result =
				grab_standard_host_macro_r(mac, macro_type, temp_host, output, free_macro);
		}
		...
		/* a host macro with a hostgroup name and delimiter */
			...
			for (temp_hostsmember = temp_hostgroup->members; temp_hostsmember != NULL;
				...
				grab_standard_host_macro_r(mac, macro_type, temp_host, &temp_buffer,
										   &free_sub_macro);
	...
	case MACRO_SERVICEIMPORTANCE:
		...
		if (arg1 == NULL && arg2 == NULL) {
			...
			result =
				grab_standard_service_macro_r(mac, macro_type, temp_service, output,
											  free_macro);
		}
			...
			/* else we have a service macro with a servicegroup name and a delimiter... */
				...
				*free_macro = TRUE;
				...
				for (temp_servicesmember = temp_servicegroup->members;
					...
					grab_standard_service_macro_r(mac, macro_type, temp_service, &temp_buffer,
												  &free_sub_macro);
	...
	case MACRO_TOTALSERVICEPROBLEMSUNHANDLED:
		...
		if (mac->x[MACRO_TOTALHOSTSUP] == NULL) {
			...
			service_problems_unhandled =
				services_warning_unhandled + services_critical_unhandled +
				services_unknown_unhandled;
			...
			asprintf(&mac->x[MACRO_TOTALHOSTSUNREACHABLEUNHANDLED], "%d",
			...
			asprintf(&mac->x[MACRO_TOTALSERVICESWARNINGUNHANDLED], "%d",
					 services_warning_unhandled);
			asprintf(&mac->x[MACRO_TOTALSERVICESCRITICALUNHANDLED], "%d",
					 services_critical_unhandled);
			asprintf(&mac->x[MACRO_TOTALSERVICESUNKNOWNUNHANDLED], "%d",
					 services_unknown_unhandled);
			asprintf(&mac->x[MACRO_TOTALSERVICEPROBLEMS], "%d", service_problems);
			asprintf(&mac->x[MACRO_TOTALSERVICEPROBLEMSUNHANDLED], "%d",
					 service_problems_unhandled);
		}
...
int grab_custom_macro_value_r(nagios_macros * mac, char *macro_name, char *arg1, char *arg2, char **output)
{
	...
	if (strstr(macro_name, "_HOST") == macro_name) {
		...
		if (arg2 == NULL) {
			...
			/* get the host macro value */
			result =
				grab_custom_object_macro_r(mac, macro_name + 5, temp_host->custom_variables,
										   output);
		}
		...
		/* a host macro with a hostgroup name and delimiter */
			...
			for (temp_hostsmember = temp_hostgroup->members; temp_hostsmember != NULL;
				...
				/* get the macro value for this host */
				grab_custom_macro_value_r(mac, macro_name, temp_host->name, NULL,
										  &temp_buffer);
	...
	/***** CUSTOM SERVICE MACRO *****/
		...
		if (arg1 == NULL && arg2 == NULL) {
			...
			/* get the service macro value */
			result =
				grab_custom_object_macro_r(mac, macro_name + 8, temp_service->custom_variables,
										   output);
		}
		...
		/* else and ondemand macro... */
			...
			if (mac->host_ptr == NULL)
				return ERROR;
			if ((temp_service =
				 find_service((mac->host_ptr) ? mac->host_ptr->name : NULL, arg2))) {

				/* get the service macro value */
				result =
					grab_custom_object_macro_r(mac, macro_name + 8,
											   temp_service->custom_variables, output);
			}
			...
			/* else we have a service macro with a servicegroup name and a delimiter... */
				...
				for (temp_servicesmember = temp_servicegroup->members;
					...
					/* get the macro value for this service */
					grab_custom_macro_value_r(mac, macro_name, temp_service->host_name,
	...
	/***** CUSTOM CONTACT VARIABLE *****/
		...
		if (arg2 == NULL) {
			...
			/* get the contact macro value */
			result =
				grab_custom_object_macro_r(mac, macro_name + 8, temp_contact->custom_variables,
										   output);
		}
		...
		/* a contact macro with a contactgroup name and delimiter */
			...
			for (temp_contactsmember = temp_contactgroup->members; temp_contactsmember != NULL;
				...
				/* get the macro value for this contact */
				grab_custom_macro_value_r(mac, macro_name, temp_contact->name, NULL,
										  &temp_buffer);
...
int grab_standard_host_macro_r(nagios_macros * mac, int macro_type, host * temp_host,
							   char **output, int *free_macro)
{
	char	   *temp_buffer = NULL;
#ifdef NSCORE
	...
	case MACRO_HOSTGROUPNAMES:
		...
		for (temp_objectlist = temp_host->hostgroups_ptr; temp_objectlist != NULL;
...
{
	return grab_standard_host_macro_r(&global_macros, macro_type, temp_host, output,
									  free_macro);
}


/* computes a hostgroup macro */
int grab_standard_hostgroup_macro_r(nagios_macros * mac, int macro_type,
									hostgroup * temp_hostgroup, char **output)
{
	hostsmember *temp_hostsmember = NULL;
	char	   *temp_buffer = NULL;
...
int grab_standard_service_macro_r(nagios_macros * mac, int macro_type, service * temp_service,
								  char **output, int *free_macro)
{
	char	   *temp_buffer = NULL;
#ifdef NSCORE
...
{
	return grab_standard_service_macro_r(&global_macros, macro_type, temp_service, output,
										 free_macro);
}


/* computes a servicegroup macro */
int grab_standard_servicegroup_macro_r(nagios_macros * mac, int macro_type,
									   servicegroup * temp_servicegroup, char **output)
{
	servicesmember *temp_servicesmember = NULL;
	char	   *temp_buffer = NULL;
	unsigned int temp_len = 0;
	...
	case MACRO_SERVICEGROUPMEMBERS:
		...
		for (temp_servicesmember = temp_servicegroup->members; temp_servicesmember != NULL;
			...
			if (init_len == 0) {	/* If our buffer didn't contain anything, we just need to write "%s,%s" */
				init_len +=
					sprintf(temp_buffer, "%s,%s", temp_servicesmember->host_name,
...
{
	return grab_standard_servicegroup_macro_r(&global_macros, macro_type, temp_servicegroup,
											  output);
}
...
int grab_standard_contact_macro_r(nagios_macros * mac, int macro_type, contact * temp_contact, char **output)
{
	...
	case MACRO_CONTACTGROUPNAMES:
		...
		for (temp_objectlist = temp_contact->contactgroups_ptr; temp_objectlist != NULL;
			 temp_objectlist = temp_objectlist->next) {
		...
		for (temp_contactsmember = temp_contactgroup->members; temp_contactsmember != NULL;
			...
			if (*output == NULL)
				*output = (char *)strdup(temp_contactsmember->contact_name);
					  ...
					  (char *)realloc(*output,
									  strlen(*output) +
									  strlen(temp_contactsmember->contact_name) + 2))) {
				strcat(*output, ",");
...
int grab_custom_object_macro_r(nagios_macros * mac, char *macro_name,
							   customvariablesmember * vars, char **output)
{
	...
	for (temp_customvariablesmember = vars; temp_customvariablesmember != NULL;
...
char	   *get_url_encoded_string(char *input)
{
	...
	for (x = 0, y = 0; input[x]; x++) {
		...
		if (((char)input[x] >= '0' && (char)input[x] <= '9') ||
			...
			((char)input[x] >= 'a' && (char)input[x] <= 'z') ||
			(char)input[x] == '.' ||
			(char)input[x] == '-' || (char)input[x] == '_' || (char)input[x] == '~') {
...
int set_custom_macro_environment_vars_r(nagios_macros * mac, int set)
{
	...
	if ((temp_contact = mac->contact_ptr) && set == TRUE) {
		for (temp_customvariablesmember = temp_contact->custom_variables;
			...
			add_custom_variable_to_object(&mac->custom_contact_vars, customvarname,
										  temp_customvariablesmember->variable_value);
			my_free(customvarname);
		}
	}
...
static int	add_contact_address_environment_vars_r(nagios_macros *, struct kvvec *);
...
struct kvvec *macros_to_kvv(nagios_macros * mac)
{
	...
	/* If we're not supposed to export macros as environment variables,
	   just return */
	if (FALSE == enable_environment_macros)
		return NULL;

	/* Create the kvvec to hold the macros */
	if ((kvvp = calloc(1, sizeof(struct kvvec))) == NULL)
		return NULL;
	if (!kvvec_init(kvvp, MACRO_X_COUNT + MAX_COMMAND_ARGUMENTS + MAX_CONTACT_ADDRESSES + 4))
		return NULL;
...
static int add_argv_macro_environment_vars_r(nagios_macros * mac, struct kvvec *kvvp)
{
	char	   *macro_name = NULL;
	register int x = 0;
...
static int add_custom_macro_environment_vars_r(nagios_macros * mac, struct kvvec *kvvp)
{
...
static int add_contact_address_environment_vars_r(nagios_macros * mac, struct kvvec *kvvp)
{
