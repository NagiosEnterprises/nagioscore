...
static void post_process_object_config(void)
{
	...
	if (servicedependency_ary)
		qsort(servicedependency_ary, num_objects.servicedependencies,
			  sizeof(servicedependency *), cmp_sdep);
	if (hostdependency_ary)
		qsort(hostdependency_ary, num_objects.hostdependencies, sizeof(hostdependency *),
			  cmp_hdep);
	if (hostescalation_ary)
		qsort(hostescalation_ary, num_objects.hostescalations, sizeof(hostescalation *),
			  cmp_hostesc);
	if (serviceescalation_ary)
		qsort(serviceescalation_ary, num_objects.serviceescalations,
			  sizeof(serviceescalation *), cmp_serviceesc);
...
static int create_object_table(const char *name, unsigned int elems, unsigned int size,
							   void **ptr)
{
	void	   *ret;
	if (!elems) {
...
timeperiod *add_timeperiod(char *name, char *alias)
{
	...
	if (result == OK) {
		result =
			dkhash_insert(object_hash_tables[TIMEPERIOD_SKIPLIST], new_timeperiod->name, NULL,
						  new_timeperiod);
		switch (result) {
		case DKHASH_EDUPE:
			logit(NSLOG_CONFIG_ERROR, TRUE,
...
timeperiodexclusion *add_exclusion_to_timeperiod(timeperiod * period, char *name)
{
	...
	if ((new_timeperiodexclusion =
		 (timeperiodexclusion *) malloc(sizeof(timeperiodexclusion))) == NULL)
		return NULL;
...
timerange  *add_timerange_to_timeperiod(timeperiod * period, int day, unsigned long start_time,
...
daterange  *add_exception_to_timeperiod(timeperiod * period, int type, int syear, int smon,
										int smday, int swday, int swday_offset, int eyear,
										int emon, int emday, int ewday, int ewday_offset,
										int skip_interval)
{
...
timerange  *add_timerange_to_daterange(daterange * drange, unsigned long start_time,
									   unsigned long end_time)
{
...
host	   *add_host(char *name, char *display_name, char *alias, char *address,
					 char *check_period, int initial_state, double check_interval,
					 double retry_interval, int max_attempts, int notification_options,
					 double notification_interval, double first_notification_delay,
					 char *notification_period, int notifications_enabled, char *check_command,
					 int checks_enabled, int accept_passive_checks, char *event_handler,
					 int event_handler_enabled, int flap_detection_enabled,
					 double low_flap_threshold, double high_flap_threshold,
					 int flap_detection_options, int stalking_options, int process_perfdata,
					 int check_freshness, int freshness_threshold, char *notes,
					 char *notes_url, char *action_url, char *icon_image, char *icon_image_alt,
					 char *vrml_image, char *statusmap_image, int x_2d, int y_2d,
					 int have_2d_coords, double x_3d, double y_3d, double z_3d,
					 int have_3d_coords, int should_be_drawn, int retain_status_information,
					 int retain_nonstatus_information, int obsess, unsigned int hourly_value)
{
	host	   *new_host = NULL;
	...
	if (result == OK) {
		result =
			dkhash_insert(object_hash_tables[HOST_SKIPLIST], new_host->name, NULL, new_host);
		switch (result) {
		case DKHASH_EDUPE:
...
servicesmember *add_parent_service_to_service(service * svc, char *host_name,
											  char *description)
{
	servicesmember *sm;
	...
	if ((sm->host_name = strdup(host_name)) == NULL
		|| (sm->service_description = strdup(description)) == NULL) {
...
static contactgroupsmember *add_contactgroup_to_object(contactgroupsmember ** cg_list,
...
hostgroup  *add_hostgroup(char *name, char *alias, char *notes, char *notes_url,
						  char *action_url)
{
	...
	if (result == OK) {
		result =
			dkhash_insert(object_hash_tables[HOSTGROUP_SKIPLIST], new_hostgroup->group_name,
						  NULL, new_hostgroup);
		switch (result) {
		case DKHASH_EDUPE:
...
servicegroup *add_servicegroup(char *name, char *alias, char *notes, char *notes_url,
							   char *action_url)
{
	...
	if (result == OK) {
		result =
			dkhash_insert(object_hash_tables[SERVICEGROUP_SKIPLIST],
						  new_servicegroup->group_name, NULL, new_servicegroup);
		switch (result) {
		case DKHASH_EDUPE:
			logit(NSLOG_CONFIG_ERROR, TRUE,
...
servicesmember *add_service_to_servicegroup(servicegroup * temp_servicegroup, char *host_name,
											char *svc_description)
{
	...
	/* make sure we have the data we need */
	if (temp_servicegroup == NULL || (host_name == NULL || !strcmp(host_name, ""))
	...
	for (temp_member = temp_servicegroup->members; temp_member != NULL;
		...
		if (strcmp(new_member->host_name, temp_member->host_name) < 0) {
			...
			break;
				 ...
				 && strcmp(new_member->service_description,
...
contact    *add_contact(char *name, char *alias, char *email, char *pager, char **addresses,
						char *svc_notification_period, char *host_notification_period,
						int service_notification_options, int host_notification_options,
						int host_notifications_enabled, int service_notifications_enabled,
						int can_submit_commands, int retain_status_information,
						int retain_nonstatus_information, unsigned int minimum_value)
{
	...
	if (result == OK) {
		result =
			dkhash_insert(object_hash_tables[CONTACT_SKIPLIST], new_contact->name, NULL,
						  new_contact);
		switch (result) {
		case DKHASH_EDUPE:
...
customvariablesmember *add_custom_variable_to_contact(contact * cntct, char *varname,
													  char *varvalue)
{
...
contactgroup *add_contactgroup(char *name, char *alias)
{
	...
	if (result == OK) {
		result =
			dkhash_insert(object_hash_tables[CONTACTGROUP_SKIPLIST],
						  new_contactgroup->group_name, NULL, new_contactgroup);
		switch (result) {
		case DKHASH_EDUPE:
			logit(NSLOG_CONFIG_ERROR, TRUE,
...
service    *add_service(char *host_name, char *description, char *display_name,
						char *check_period, int initial_state, int max_attempts,
						int parallelize, int accept_passive_checks, double check_interval,
						double retry_interval, double notification_interval,
						double first_notification_delay, char *notification_period,
						int notification_options, int notifications_enabled, int is_volatile,
						char *event_handler, int event_handler_enabled, char *check_command,
						int checks_enabled, int flap_detection_enabled,
						double low_flap_threshold, double high_flap_threshold,
						int flap_detection_options, int stalking_options, int process_perfdata,
						int check_freshness, int freshness_threshold, char *notes,
						char *notes_url, char *action_url, char *icon_image,
						char *icon_image_alt, int retain_status_information,
						int retain_nonstatus_information, int obsess,
						unsigned int hourly_value)
{
	...
	if (host_name == NULL || description == NULL || !*description || check_command == NULL
		|| !*check_command) {
		logit(NSLOG_CONFIG_ERROR, TRUE,
	...
	if (result == OK) {
		result =
			dkhash_insert(object_hash_tables[SERVICE_SKIPLIST], new_service->host_name,
						  new_service->description, new_service);
		switch (result) {
		case DKHASH_EDUPE:
			logit(NSLOG_CONFIG_ERROR, TRUE,
...
customvariablesmember *add_custom_variable_to_service(service * svc, char *varname,
													  char *varvalue)
{
...
command    *add_command(char *name, char *value)
{
	...
	if (result == OK) {
		result =
			dkhash_insert(object_hash_tables[COMMAND_SKIPLIST], new_command->name, NULL,
						  new_command);
		switch (result) {
		case DKHASH_EDUPE:
...
serviceescalation *add_serviceescalation(char *host_name, char *description,
										 int first_notification, int last_notification,
										 double notification_interval, char *escalation_period,
...
servicedependency *add_service_dependency(char *dependent_host_name,
										  char *dependent_service_description, char *host_name,
										  char *service_description, int dependency_type,
										  int inherits_parent, int failure_options,
										  char *dependency_period)
{
	...
	if (dependency_period && !(tp = find_timeperiod(dependency_period))) {
		logit(NSLOG_CONFIG_ERROR, TRUE,
			  ...
			  dependency_period, dependent_service_description, dependent_host_name,
			  service_description, host_name);
		return NULL;
	}
	...
	new_servicedependency->dependency_type =
		(dependency_type ==
		 EXECUTION_DEPENDENCY) ? EXECUTION_DEPENDENCY : NOTIFICATION_DEPENDENCY;
	...
	if (dependency_type == NOTIFICATION_DEPENDENCY)
		result =
			prepend_unique_object_to_objectlist(&child->notify_deps, new_servicedependency,
												sizeof(*new_servicedependency));
	else
		result =
			prepend_unique_object_to_objectlist(&child->exec_deps, new_servicedependency,
												sizeof(*new_servicedependency));
...
hostdependency *add_host_dependency(char *dependent_host_name, char *host_name,
									int dependency_type, int inherits_parent,
									int failure_options, char *dependency_period)
{
	...
	if (dependency_type == NOTIFICATION_DEPENDENCY)
		result =
			prepend_unique_object_to_objectlist(&child->notify_deps, new_hostdependency,
												sizeof(*new_hostdependency));
	else
		result =
			prepend_unique_object_to_objectlist(&child->exec_deps, new_hostdependency,
												sizeof(*new_hostdependency));
...
hostescalation *add_hostescalation(char *host_name, int first_notification,
								   int last_notification, double notification_interval,
								   char *escalation_period, int escalation_options)
{
...
int free_objectlist(objectlist ** temp_list)
{
	...
	for (this_objectlist = *temp_list; this_objectlist != NULL;
...
int is_host_immediate_child_of_host(host * parent_host, host * child_host)
{
	...
	/* mid-level/bottom hosts */
		...
		for (temp_hostsmember = child_host->parent_hosts; temp_hostsmember != NULL;
...
int is_host_member_of_hostgroup(hostgroup * group, host * hst)
{
	...
	for (temp_hostsmember = group->members; temp_hostsmember != NULL;
		 temp_hostsmember = temp_hostsmember->next) {
...
int is_host_member_of_servicegroup(servicegroup * group, host * hst)
{
	...
	for (temp_servicesmember = group->members; temp_servicesmember != NULL;
...
int is_contact_for_host(host * hst, contact * cntct)
{
	...
	for (temp_contactsmember = hst->contacts; temp_contactsmember != NULL;
	...
	for (temp_contactgroupsmember = hst->contact_groups; temp_contactgroupsmember != NULL;
...
/*	tests whether a contactgroup is a contactgroup for a particular
	host escalation */
int is_contactgroup_for_host_escalation(hostescalation * escalation, contactgroup * group)
{
	contactgroupsmember *temp_contactgroupsmember = NULL;
...
int is_contact_for_service(service * svc, contact * cntct)
{
	...
	for (temp_contactsmember = svc->contacts; temp_contactsmember != NULL;
		 temp_contactsmember = temp_contactsmember->next) {
		if (temp_contactsmember->contact_ptr == cntct)
			return TRUE;
	}
	...
	for (temp_contactgroupsmember = svc->contact_groups; temp_contactgroupsmember != NULL;
		 temp_contactgroupsmember = temp_contactgroupsmember->next) {
		temp_contactgroup = temp_contactgroupsmember->group_ptr;
		if (is_contact_member_of_contactgroup(temp_contactgroup, cntct))
			return TRUE;
...
int is_escalated_contact_for_service(service * svc, contact * cntct)
{
	...
	for (list = svc->escalation_list; list; list = list->next) {
		...
		if (is_contact_for_service_escalation(temp_serviceescalation, cntct) == TRUE) {
			return TRUE;
		}
	}
...
int is_contact_for_service_escalation(serviceescalation * escalation, contact * cntct)
{
	contactsmember *temp_contactsmember = NULL;
	contactgroupsmember *temp_contactgroupsmember = NULL;
	contactgroup *temp_contactgroup = NULL;
...
/*	tests whether a contactgroup is a contactgroup for a particular
	service escalation */
int is_contactgroup_for_service_escalation(serviceescalation * escalation, contactgroup * group)
{
	contactgroupsmember *temp_contactgroupsmember = NULL;
	...
	for (i = 0; i < num_objects.timeperiods; i++) {
		...
		for (x = 0; x < DATERANGE_TYPES; x++) {
			...
			for (this_daterange = this_timeperiod->exceptions[x]; this_daterange != NULL;
				...
				for (this_timerange = this_daterange->times; this_timerange != NULL;
		...
		for (x = 0; x < 7; x++) {
			...
			for (this_timerange = this_timeperiod->days[x]; this_timerange != NULL;
				 this_timerange = next_timerange) {
				next_timerange = this_timerange->next;
				my_free(this_timerange);
			}
...
void fcache_customvars(FILE * fp, customvariablesmember * cvlist)
{
	if (cvlist) {
		...
		for (l = cvlist; l; l = l->next)
			fprintf(fp, "\t_%s\t%s\n", l->variable_name,
					(l->variable_value == NULL) ? XODTEMPLATE_NULL : l->variable_value);
	}
}
	...
	for (x = 0; x < DATERANGE_TYPES; x++) {
		for (temp_daterange = temp_timeperiod->exceptions[x]; temp_daterange != NULL;
			...
			case DATERANGE_CALENDAR_DATE:
				fprintf(fp, "\t%d-%02d-%02d", temp_daterange->syear, temp_daterange->smon + 1,
...
void fcache_service(FILE * fp, service * temp_service)
{
	...
	if (temp_service->parents) {
		...
		if (!temp_service->parents->next
			&& temp_service->parents->service_ptr->host_ptr == temp_service->host_ptr)
			fprintf(fp, "%s\n", temp_service->parents->service_ptr->description);
		else {
			servicesmember *sm;
			for (sm = temp_service->parents; sm; sm = sm->next) {
				fprintf(fp, "%s,%s%c", sm->host_name, sm->service_description,
						sm->next ? ',' : '\n');
			}
		}
	}
...
void fcache_servicedependency(FILE * fp, servicedependency * temp_servicedependency)
{
	...
	fprintf(fp, "\t%s_failure_options\t%s\n",
			...
			NOTIFICATION_DEPENDENCY ? "notification" : "execution",
