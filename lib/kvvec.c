...
int kvvec_destroy(struct kvvec *kvv, int flags)
{
	...
	if (flags & (KVVEC_FREE_KEYS | KVVEC_FREE_VALUES)) {
		for (i = 0; i < kvv->kv_pairs; i++) {
			...
			/*
			 * kv->value is always set, since we strdup("") on NULL
			 * ones, so free it regardless of length
			 */
			if (flags & KVVEC_FREE_VALUES && kv->value) {
				free(kv->value);
			}
		}
	}
