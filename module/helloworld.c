...
int nebmodule_init(int flags, char *args, nebmodule * handle)
{
	...
	neb_set_module_info(helloworld_module_handle, NEBMODULE_MODINFO_TITLE,
	...
	neb_set_module_info(helloworld_module_handle, NEBMODULE_MODINFO_DESC,
						"A simple example to get you started with Nagios Event Broker (NEB) modules.");
	...
	time(&current_time);
	interval = 900;
	schedule_new_event(EVENT_USER_FUNCTION, TRUE, current_time + interval, TRUE, interval,
					   NULL, TRUE, (void *)helloworld_reminder_message, "How about you?", 0);

	/* register to be notified of certain events... */
	neb_register_callback(NEBCALLBACK_AGGREGATED_STATUS_DATA, helloworld_module_handle, 0,
	...
	case NEBCALLBACK_AGGREGATED_STATUS_DATA:
		...
		if ((agsdata = (nebstruct_aggregated_status_data *) data)) {
			...
			snprintf(temp_buffer, sizeof(temp_buffer) - 1,
					 "helloworld: An aggregated status update just %s.",
					 (agsdata->type ==
					  NEBTYPE_AGGREGATEDSTATUS_STARTDUMP) ? "started" : "finished");
			temp_buffer[sizeof(temp_buffer) - 1] = '\x0';
			write_to_all_logs(temp_buffer, NSLOG_INFO_MESSAGE);
		}
