...
void broker_system_command(int type, int flags, int attr,
						   ...
						   int timeout, int early_timeout, int retcode, char *cmd,
...
timed_event *schedule_new_event(int event_type, int high_priority,
								...
								void *timing_func, int compensate_for_time_change,
								void *event_data, void *event_args, int event_options)
{
	return NULL;
}
...
void test_escaping(nagios_macros * mac)
{
	...
	RUN_MACRO_TEST("$HOSTNAME$ '&%", "name%27%26%25 '&%", URL_ENCODE_MACRO_CHARS);
