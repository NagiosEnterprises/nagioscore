...
void parse_worker_command_line(int, char **, char **, int *, char **, char **, char **);
...
void parse_worker_command_line(int argc, char **argv, char **env, int *daemon_mode,
							   char **worker_socket, char **worker_user, char **worker_group)
{
	int 		c = 0;
...
static int worker(const char *path)
{
	...
	ret =
		nsock_printf_nul(sd,
						 "@wproc register name=Core Ping Worker %d;pid=%d;plugin=check_ping",
						 getpid(), getpid());
	if (ret < 0) {
	...
	if (NULL != group) {
		...
		if (strspn(group, "0123456789") < strlen(group)) {
			...
			if (NULL != grp) {
				...
				fprintf(stderr, "Warning: Could not get group entry for '%s'\n", group);
			}
		}
	...
	if (NULL != user) {
		...
		if (strspn(user, "0123456789") < strlen(user)) {
			...
			if (NULL != pw) {
				...
				fprintf(stderr, "Warning: Could not get passwd entry for '%s'\n", user);
			}
		}
