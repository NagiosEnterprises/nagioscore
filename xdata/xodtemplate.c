...
int xodtemplate_process_config_dir(char *dirname, int options){
	...
	dirp=opendir(dirname);
        if(dirp==NULL){
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Could not open config directory '%s' for reading.\n",dirname);
#endif
		return ERROR;
	        }
	...
	while((dirfile=readdir(dirp))!=NULL){
		...
		if(dirfile->d_type==DT_UNKNOWN || dirfile->d_type==DT_DIR || dirfile->d_type==DT_LNK){
			...
			if(dirfile->d_type==DT_LNK || (dirfile->d_type==DT_UNKNOWN && S_ISLNK(stat_buf.st_mode))){
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Cannot follow symlink '%s' - Too big!\n",file);
#endif
					return ERROR;
				        }
				...
				if(readlink_count==-1){
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Cannot read symlink '%s': %s!\n",file, strerror(errno));
#endif
					return ERROR;
				        }
				...
				if(x!=0){
					...
					if(errno==ENOENT){
#ifdef NSCORE
						logit(NSLOG_CONFIG_ERROR, "Error: symlink '%s' points to non-existent '%s'!\n",file,link_buffer);
#endif
						return ERROR;
					        }

#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Cannot stat symlinked from '%s' to %s!\n",file,link_buffer);
#endif
					return ERROR;
				        }
...
int xodtemplate_process_config_file(char *filename, int options){
...
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Cannot open config file '%s' for reading: %s\n",filename,strerror(errno));
#endif
		return ERROR;
	        }
	...
	while(1){
		...
		if(strstr(input,"define")==input){
			...
			if(input[0]=='\x0'){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: No object type specified in file '%s' on line %d.\n",filename,current_line);
#endif
				result=ERROR;
				break;
			        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Invalid object definition type '%s' in file '%s' on line %d.\n",input,filename,current_line);
#endif
				result=ERROR;
				break;
			        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Unexpected start of object definition in file '%s' on line %d.  Make sure you close preceding objects before starting a new one.\n",filename,current_line);
#endif
				result=ERROR;
				break;
			        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not add object definition in file '%s' on line %d.\n",filename,current_line);
#endif
				result=ERROR;
				break;
			        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not complete object definition in file '%s' on line %d.\n",filename,current_line);
#endif
				result=ERROR;
				break;
			        }
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Could not add object property in file '%s' on line %d.\n",filename,current_line);
#endif
					result=ERROR;
					break;
				        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Unexpected token or statement in file '%s' on line %d.\n",filename,current_line);
#endif
			result=ERROR;
			break;
		        }
...
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Unexpected EOF in file '%s' on line %d - check for a missing closing bracket.\n",filename,current_line);
#endif
		result=ERROR;
	        }
...
int xodtemplate_add_object_property(char *input, int options){
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_WARNING, "Warning: Duplicate definition found for timeperiod '%s' (config file '%s', starting on line %d)\n",value,xodtemplate_config_file_name(temp_timeperiod->_config_file),temp_timeperiod->_start_line);
#endif
			        }
#endif
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Invalid timeperiod object directive '%s'.\n",variable);
#endif
			return ERROR;
		        }
		break;
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_WARNING, "Warning: Duplicate definition found for command '%s' (config file '%s', starting on line %d)\n",value,xodtemplate_config_file_name(temp_command->_config_file),temp_command->_start_line);
#endif
			        }
#endif
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Invalid command object directive '%s'.\n",variable);
#endif
			return ERROR;
		        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_WARNING, "Warning: Duplicate definition found for contactgroup '%s' (config file '%s', starting on line %d)\n",value,xodtemplate_config_file_name(temp_contactgroup->_config_file),temp_contactgroup->_start_line);
#endif
			        }
#endif
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Invalid contactgroup object directive '%s'.\n",variable);
#endif
			return ERROR;
		        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_WARNING, "Warning: Duplicate definition found for hostgroup '%s' (config file '%s', starting on line %d)\n",value,xodtemplate_config_file_name(temp_hostgroup->_config_file),temp_hostgroup->_start_line);
#endif
			        }
#endif
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Invalid hostgroup object directive '%s'.\n",variable);
#endif
			return ERROR;
		        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_WARNING, "Warning: Duplicate definition found for servicegroup '%s' (config file '%s', starting on line %d)\n",value,xodtemplate_config_file_name(temp_servicegroup->_config_file),temp_servicegroup->_start_line);
#endif
			        }
#endif
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Invalid servicegroup object directive '%s'.\n",variable);
#endif
			return ERROR;
		        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_WARNING, "Warning: Duplicate definition found for service dependency '%s' (config file '%s', starting on line %d)\n",value,xodtemplate_config_file_name(temp_servicedependency->_config_file),temp_servicedependency->_start_line);
#endif
			        }
#endif
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Invalid execution dependency option '%s' in servicedependency definition.\n",temp_ptr);
#endif
					return ERROR;
				        }
			        }
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Invalid notification dependency option '%s' in servicedependency definition.\n",temp_ptr);
#endif
					return ERROR;
				        }
			        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Invalid servicedependency object directive '%s'.\n",variable);
#endif
			return ERROR;
		        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_WARNING, "Warning: Duplicate definition found for service escalation '%s' (config file '%s', starting on line %d)\n",value,xodtemplate_config_file_name(temp_serviceescalation->_config_file),temp_serviceescalation->_start_line);
#endif
			        }
#endif
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Invalid escalation option '%s' in serviceescalation definition.\n",temp_ptr);
#endif
					return ERROR;
				        }
			        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Invalid serviceescalation object directive '%s'.\n",variable);
#endif
			return ERROR;
		        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_WARNING, "Warning: Duplicate definition found for contact '%s' (config file '%s', starting on line %d)\n",value,xodtemplate_config_file_name(temp_contact->_config_file),temp_contact->_start_line);
#endif
			        }
#endif
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Invalid host notification option '%s' in contact definition.\n",temp_ptr);
#endif
					return ERROR;
				        }
			        }
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Invalid service notification option '%s' in contact definition.\n",temp_ptr);
#endif
					return ERROR;
				        }
			        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Null custom variable name.\n");
#endif
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Invalid contact object directive '%s'.\n",variable);
#endif
			return ERROR;
		        }
	...
	case XODTEMPLATE_HOST:
		...
		if(!strcmp(variable,"use")){
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_WARNING, "Warning: Duplicate definition found for host '%s' (config file '%s', starting on line %d)\n",value,xodtemplate_config_file_name(temp_host->_config_file),temp_host->_start_line);
#endif
			        }
#endif
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Invalid initial state '%s' in host definition.\n",value);
#endif
				result=ERROR;
				}
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Invalid flap detection option '%s' in host definition.\n",temp_ptr);
#endif
					result=ERROR;
				        }
			        }
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Invalid notification option '%s' in host definition.\n",temp_ptr);
#endif
					result=ERROR;
				        }
			        }
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Invalid stalking option '%s' in host definition.\n",temp_ptr);
#endif
					result=ERROR;
				        }
			        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Invalid 2d_coords value '%s' in host definition.\n",temp_ptr);
#endif
				return ERROR;
			        }
			...
			if((temp_ptr=strtok(value,", "))==NULL){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Invalid 3d_coords value '%s' in host definition.\n",temp_ptr);
#endif
				return ERROR;
			        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Invalid host object directive '%s'.\n",variable);
#endif
			return ERROR;
		        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_WARNING, "Warning: Duplicate definition found for service '%s' (config file '%s', starting on line %d)\n",value,xodtemplate_config_file_name(temp_service->_config_file),temp_service->_start_line);
#endif
			        }
#endif
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Invalid initial state '%s' in service definition.\n",value);
#endif
				result=ERROR;
				}
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Invalid flap detection option '%s' in service definition.\n",temp_ptr);
#endif
					return ERROR;
				        }
			        }
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Invalid notification option '%s' in service definition.\n",temp_ptr);
#endif
					return ERROR;
				        }
			        }
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Invalid stalking option '%s' in service definition.\n",temp_ptr);
#endif
					return ERROR;
				        }
			        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Invalid service object directive '%s'.\n",variable);
#endif
			return ERROR;
		        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_WARNING, "Warning: Duplicate definition found for host dependency '%s' (config file '%s', starting on line %d)\n",value,xodtemplate_config_file_name(temp_hostdependency->_config_file),temp_hostdependency->_start_line);
#endif
			        }
#endif
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Invalid notification dependency option '%s' in hostdependency definition.\n",temp_ptr);
#endif
					return ERROR;
				        }
			        }
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Invalid execution dependency option '%s' in hostdependency definition.\n",temp_ptr);
#endif
					return ERROR;
				        }
			        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Invalid hostdependency object directive '%s'.\n",variable);
#endif
			return ERROR;
		        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_WARNING, "Warning: Duplicate definition found for host escalation '%s' (config file '%s', starting on line %d)\n",value,xodtemplate_config_file_name(temp_hostescalation->_config_file),temp_hostescalation->_start_line);
#endif
			        }
#endif
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Invalid escalation option '%s' in hostescalation definition.\n",temp_ptr);
#endif
					return ERROR;
				        }
			        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Invalid hostescalation object directive '%s'.\n",variable);
#endif
			return ERROR;
		        }
	...
	case XODTEMPLATE_HOSTEXTINFO:
		...
		if(!strcmp(variable,"use")){
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_WARNING, "Warning: Duplicate definition found for extended host info '%s' (config file '%s', starting on line %d)\n",value,xodtemplate_config_file_name(temp_hostextinfo->_config_file),temp_hostextinfo->_start_line);
#endif
			        }
#endif
			...
			if(temp_ptr==NULL){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Invalid 2d_coords value '%s' in extended host info definition.\n",temp_ptr);
#endif
				return ERROR;
			        }
			...
			if(temp_ptr==NULL){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Invalid 3d_coords value '%s' in extended host info definition.\n",temp_ptr);
#endif
				return ERROR;
			        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Invalid hostextinfo object directive '%s'.\n",variable);
#endif
			return ERROR;
		        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_WARNING, "Warning: Duplicate definition found for extended service info '%s' (config file '%s', starting on line %d)\n",value,xodtemplate_config_file_name(temp_serviceextinfo->_config_file),temp_serviceextinfo->_start_line);
#endif
			        }
#endif
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Invalid serviceextinfo object directive '%s'.\n",variable);
#endif
			return ERROR;
		        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand hostgroups and/or hosts specified in service (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_service->_config_file),temp_service->_start_line);
#endif
			return ERROR;
		        }
...
int xodtemplate_duplicate_objects(void){
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand hostgroups and/or hosts specified in host escalation (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_hostescalation->_config_file),temp_hostescalation->_start_line);
#endif
			return ERROR;
		        }
	...
	for(temp_serviceescalation=xodtemplate_serviceescalation_list;temp_serviceescalation!=NULL;temp_serviceescalation=temp_serviceescalation->next){
		...
		if(master_hostlist==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand hostgroups and/or hosts specified in service escalation (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_serviceescalation->_config_file),temp_serviceescalation->_start_line);
#endif
			return ERROR;
		        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand services specified in service escalation (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_serviceescalation->_config_file),temp_serviceescalation->_start_line);
#endif
			return ERROR;
		        }
	...
	for(temp_serviceescalation=xodtemplate_serviceescalation_list;temp_serviceescalation!=NULL;temp_serviceescalation=temp_serviceescalation->next){
		...
		if(master_servicelist==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand servicegroups specified in service escalation (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_serviceescalation->_config_file),temp_serviceescalation->_start_line);
#endif
			return ERROR;
		        }
	...
	for(temp_hostdependency=xodtemplate_hostdependency_list;temp_hostdependency!=NULL;temp_hostdependency=temp_hostdependency->next){
		...
		if(master_hostlist==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand master hostgroups and/or hosts specified in host dependency (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_hostdependency->_config_file),temp_hostdependency->_start_line);
#endif
			return ERROR;
		        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand dependent hostgroups and/or hosts specified in host dependency (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_hostdependency->_config_file),temp_hostdependency->_start_line);
#endif
			xodtemplate_free_memberlist(&master_hostlist);
			return ERROR;
		        }
	...
	for(temp_servicedependency=xodtemplate_servicedependency_list;temp_servicedependency!=NULL;temp_servicedependency=temp_servicedependency->next){
		...
		if(temp_servicedependency->servicegroup_name){
			...
			if(master_servicelist==NULL){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not expand master servicegroups specified in service dependency (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_servicedependency->_config_file),temp_servicedependency->_start_line);
#endif
				return ERROR;
				}
			...
			if(temp_servicedependency->host_name!=NULL || temp_servicedependency->hostgroup_name!=NULL || temp_servicedependency->service_description!=NULL){
				...
				my_free((void **)&temp_servicedependency->hostgroup_name);
				}
	...
	for(temp_servicedependency=xodtemplate_servicedependency_list;temp_servicedependency!=NULL;temp_servicedependency=temp_servicedependency->next){
		...
		if(temp_servicedependency->host_name!=NULL || temp_servicedependency->hostgroup_name!=NULL){
			...
			if(master_hostlist==NULL){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not expand master hostgroups and/or hosts specified in service dependency (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_servicedependency->_config_file),temp_servicedependency->_start_line);
#endif
				return ERROR;
				}
			...
			for(temp_masterhost=master_hostlist;temp_masterhost!=NULL;temp_masterhost=temp_masterhost->next){
				...
				if(master_servicelist==NULL){
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Could not expand master services specified in service dependency (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_servicedependency->_config_file),temp_servicedependency->_start_line);
#endif
					return ERROR;
					}
	...
	for(temp_servicedependency=xodtemplate_servicedependency_list;temp_servicedependency!=NULL;temp_servicedependency=temp_servicedependency->next){
		...
		if(temp_servicedependency->dependent_servicegroup_name){
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not expand dependent servicegroups specified in service dependency (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_servicedependency->_config_file),temp_servicedependency->_start_line);
#endif
				return ERROR;
				}
			...
			if(temp_servicedependency->dependent_host_name!=NULL || temp_servicedependency->dependent_hostgroup_name!=NULL || temp_servicedependency->dependent_service_description!=NULL){
				...
				my_free((void **)&temp_servicedependency->dependent_hostgroup_name);
				}
	...
	for(temp_servicedependency=xodtemplate_servicedependency_list;temp_servicedependency!=NULL;temp_servicedependency=temp_servicedependency->next){
		...
		if(temp_servicedependency->dependent_host_name!=NULL || temp_servicedependency->dependent_hostgroup_name!=NULL){
			...
			if(dependent_hostlist==NULL){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not expand dependent hostgroups and/or hosts specified in service dependency (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_servicedependency->_config_file),temp_servicedependency->_start_line);
#endif
				return ERROR;
				}
			...
			for(temp_dependenthost=dependent_hostlist;temp_dependenthost!=NULL;temp_dependenthost=temp_dependenthost->next){
				...
				if(dependent_servicelist==NULL){
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Could not expand dependent services specified in service dependency (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_servicedependency->_config_file),temp_servicedependency->_start_line);
#endif
					return ERROR;
					}
	...
	for(temp_hostextinfo=xodtemplate_hostextinfo_list;temp_hostextinfo!=NULL;temp_hostextinfo=temp_hostextinfo->next){
		...
		if(master_hostlist==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand hostgroups and/or hosts specified in extended host info (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_hostextinfo->_config_file),temp_hostextinfo->_start_line);
#endif
			return ERROR;
		        }
	...
	for(temp_serviceextinfo=xodtemplate_serviceextinfo_list;temp_serviceextinfo!=NULL;temp_serviceextinfo=temp_serviceextinfo->next){
		...
		if(master_hostlist==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand hostgroups and/or hosts specified in extended service info (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_serviceextinfo->_config_file),temp_serviceextinfo->_start_line);
#endif
			return ERROR;
		        }
...
int xodtemplate_duplicate_service(xodtemplate_service *temp_service, char *host_name){
	...
	if(error==TRUE){
		...
		my_free((void **)&new_service->contacts);
...
int xodtemplate_duplicate_hostescalation(xodtemplate_hostescalation *temp_hostescalation, char *host_name){
	...
	if(error==TRUE){
		...
		my_free((void **)&new_hostescalation->contacts);
...
int xodtemplate_duplicate_serviceescalation(xodtemplate_serviceescalation *temp_serviceescalation, char *host_name, char *svc_description){
	...
	if(error==TRUE){
		...
		my_free((void **)&new_serviceescalation->contacts);
...
int xodtemplate_duplicate_servicedependency(xodtemplate_servicedependency *temp_servicedependency, char *master_host_name, char *master_service_description, char *master_hostgroup_name, char *master_servicegroup_name, char *dependent_host_name, char *dependent_service_description, char *dependent_hostgroup_name, char *dependent_servicegroup_name){
	...
	if(error==TRUE){
		...
		my_free((void **)&new_servicedependency->hostgroup_name);
		my_free((void **)&new_servicedependency->servicegroup_name);
		my_free((void **)&new_servicedependency->dependent_host_name);
		my_free((void **)&new_servicedependency->dependent_service_description);
		my_free((void **)&new_servicedependency->dependent_hostgroup_name);
		my_free((void **)&new_servicedependency->dependent_servicegroup_name);
		my_free((void **)&new_servicedependency->dependency_period);
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Template '%s' specified in timeperiod definition could not be not found (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(this_timeperiod->_config_file),this_timeperiod->_start_line);
#endif
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Template '%s' specified in command definition could not be not found (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(this_command->_config_file),this_command->_start_line);
#endif
			my_free((void **)&template_names);
			return ERROR;
	                }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Template '%s' specified in contactgroup definition could not be not found (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(this_contactgroup->_config_file),this_contactgroup->_start_line);
#endif
			my_free((void **)&template_names);
			return ERROR;
	                }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Template '%s' specified in hostgroup definition could not be not found (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(this_hostgroup->_config_file),this_hostgroup->_start_line);
#endif
			my_free((void **)&template_names);
			return ERROR;
	                }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Template '%s' specified in servicegroup definition could not be not found (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(this_servicegroup->_config_file),this_servicegroup->_start_line);
#endif
			my_free((void **)&template_names);
			return ERROR;
	                }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Template '%s' specified in service dependency definition could not be not found (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(this_servicedependency->_config_file),this_servicedependency->_start_line);
#endif
			my_free((void **)&template_names);
			return ERROR;
	                }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Template '%s' specified in service escalation definition could not be not found (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(this_serviceescalation->_config_file),this_serviceescalation->_start_line);
#endif
			my_free((void **)&template_names);
			return ERROR;
	                }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Template '%s' specified in contact definition could not be not found (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(this_contact->_config_file),this_contact->_start_line);
#endif
			my_free((void **)&template_names);
			return ERROR;
	                }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Template '%s' specified in host definition could not be not found (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(this_host->_config_file),this_host->_start_line);
#endif
			my_free((void **)&template_names);
			return ERROR;
	                }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Template '%s' specified in service definition could not be not found (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(this_service->_config_file),this_service->_start_line);
#endif
			my_free((void **)&template_names);
			return ERROR;
	                }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Template '%s' specified in host dependency definition could not be not found (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(this_hostdependency->_config_file),this_hostdependency->_start_line);
#endif
			my_free((void **)&template_names);
			return ERROR;
	                }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Template '%s' specified in host escalation definition could not be not found (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(this_hostescalation->_config_file),this_hostescalation->_start_line);
#endif
			my_free((void **)&template_names);
			return ERROR;
	                }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Template '%s' specified in extended host info definition could not be not found (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(this_hostextinfo->_config_file),this_hostextinfo->_start_line);
#endif
			my_free((void **)&template_names);
			return ERROR;
	                }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Template '%s' specified in extended service info definition could not be not found (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(this_serviceextinfo->_config_file),this_serviceextinfo->_start_line);
#endif
			my_free((void **)&template_names);
			return ERROR;
	                }
...
int xodtemplate_recombobulate_contactgroups(void){
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not find contactgroup '%s' specified in contact '%s' definition (config file '%s', starting on line %d)\n",temp_ptr,temp_contact->contact_name,xodtemplate_config_file_name(temp_contact->_config_file),temp_contact->_start_line);
#endif
	...
	for(temp_contactgroup=xodtemplate_contactgroup_list;temp_contactgroup;temp_contactgroup=temp_contactgroup->next){
		...
		if(temp_memberlist==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand member contacts specified in contactgroup (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_contactgroup->_config_file),temp_contactgroup->_start_line);
#endif
			return ERROR;
	                }
...
/*#endif*/
	...
	for(temp_host=xodtemplate_host_list;temp_host;temp_host=temp_host->next){
		...
		if(temp_memberlist==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand contacts specified in host (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_host->_config_file),temp_host->_start_line);
#endif
			return ERROR;
	                }

		/* add all contacts */
		my_free((void **)&temp_host->contacts);
		for(this_memberlist=temp_memberlist;this_memberlist;this_memberlist=this_memberlist->next){
		...
		my_free((void **)&temp_host->contact_groups);
	        }
	...
	for(temp_service=xodtemplate_service_list;temp_service;temp_service=temp_service->next){
		...
		if(temp_memberlist==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand contacts specified in service (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_service->_config_file),temp_service->_start_line);
#endif
			return ERROR;
	                }

		/* add all contacts */
		my_free((void **)&temp_service->contacts);
		for(this_memberlist=temp_memberlist;this_memberlist;this_memberlist=this_memberlist->next){
		...
		/* null out contactgroups member - we don't use it from here on out */
		my_free((void **)&temp_service->contact_groups);
	        }
	...
	for(temp_hostescalation=xodtemplate_hostescalation_list;temp_hostescalation;temp_hostescalation=temp_hostescalation->next){
		...
		if(temp_memberlist==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand contacts specified in host escalation (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_hostescalation->_config_file),temp_hostescalation->_start_line);
#endif
			return ERROR;
	                }

		/* add all contacts */
		my_free((void **)&temp_hostescalation->contacts);
		for(this_memberlist=temp_memberlist;this_memberlist;this_memberlist=this_memberlist->next){
		...
		/* null out contactgroups member - we don't use it from here on out */
		my_free((void **)&temp_hostescalation->contact_groups);
	        }
	...
	for(temp_serviceescalation=xodtemplate_serviceescalation_list;temp_serviceescalation;temp_serviceescalation=temp_serviceescalation->next){
		...
		if(temp_memberlist==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand contacts specified in service escalation (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_serviceescalation->_config_file),temp_serviceescalation->_start_line);
#endif
			return ERROR;
	                }

		/* add all contacts */
		my_free((void **)&temp_serviceescalation->contacts);
		for(this_memberlist=temp_memberlist;this_memberlist;this_memberlist=this_memberlist->next){
		...
		/* null out contactgroups member - we don't use it from here on out */
		my_free((void **)&temp_serviceescalation->contact_groups);
	        }
...
int xodtemplate_recombobulate_hostgroups(void){
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not find hostgroup '%s' specified in host '%s' definition (config file '%s', starting on line %d)\n",temp_ptr,temp_host->host_name,xodtemplate_config_file_name(temp_host->_config_file),temp_host->_start_line);
#endif
	...
	for(temp_hostgroup=xodtemplate_hostgroup_list;temp_hostgroup;temp_hostgroup=temp_hostgroup->next){
		...
		if(temp_memberlist==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand member hosts specified in hostgroup (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_hostgroup->_config_file),temp_hostgroup->_start_line);
#endif
			return ERROR;
	                }
...
int xodtemplate_recombobulate_servicegroups(void){
	...
	for(temp_servicegroup=xodtemplate_servicegroup_list;temp_servicegroup;temp_servicegroup=temp_servicegroup->next){
		...
		for(temp_ptr=member_names;temp_ptr;temp_ptr=strchr(temp_ptr+1,',')){
			...
			/* this is the service description */
				...
				if(temp_memberlist==NULL){
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Could not expand member services specified in servicegroup (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_servicegroup->_config_file),temp_servicegroup->_start_line);
#endif
		...
		if(host_name!=NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Servicegroup members must be specified in <host_name>,<service_description> pairs (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_servicegroup->_config_file),temp_servicegroup->_start_line);
#endif
			my_free((void **)&host_name);
			return ERROR;
		        }
	...
	for(temp_servicegroup=xodtemplate_servicegroup_list;temp_servicegroup;temp_servicegroup=temp_servicegroup->next){
		...
		if(temp_memberlist==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not expand member servicegroups specified in servicegroup (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(temp_servicegroup->_config_file),temp_servicegroup->_start_line);
#endif
			return ERROR;
		        }
...
int xodtemplate_register_timeperiod(xodtemplate_timeperiod *this_timeperiod){
	...
	if(new_timeperiod==NULL){
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Could not register timeperiod (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_timeperiod->_config_file),this_timeperiod->_start_line);
#endif
		return ERROR;
	        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not add date exception to timeperiod (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_timeperiod->_config_file),this_timeperiod->_start_line);
#endif
				return ERROR;
			        }
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Could not parse timerange #%d of timeperiod (config file '%s', starting on line %d)\n",range,xodtemplate_config_file_name(this_timeperiod->_config_file),this_timeperiod->_start_line);
#endif
					return ERROR;
					}
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR, "Error: Could not add timerange #%d to timeperiod (config file '%s', starting on line %d)\n",range,xodtemplate_config_file_name(this_timeperiod->_config_file),this_timeperiod->_start_line);
#endif
					return ERROR;
					}
				}
	...
	for(day=0;day<7;day++){
		...
		for(day_range_start_buffer=my_strsep(&day_range_ptr,", ");day_range_start_buffer!=NULL;day_range_start_buffer=my_strsep(&day_range_ptr,", ")){
			...
			if(xodtemplate_get_time_ranges(day_range_start_buffer,&range_start_time,&range_end_time)==ERROR){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not parse timerange #%d for day %d of timeperiod (config file '%s', starting on line %d)\n",range,day,xodtemplate_config_file_name(this_timeperiod->_config_file),this_timeperiod->_start_line);
#endif
				return ERROR;
				}
			...
			if(new_timerange==NULL){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not add timerange #%d for day %d to timeperiod (config file '%s', starting on line %d)\n",range,day,xodtemplate_config_file_name(this_timeperiod->_config_file),this_timeperiod->_start_line);
#endif
				return ERROR;
			        }
		        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not add excluded timeperiod '%s' to timeperiod (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(this_timeperiod->_config_file),this_timeperiod->_start_line);
#endif
				return ERROR;
				}
			}
...
int xodtemplate_register_command(xodtemplate_command *this_command){
	...
	if(new_command==NULL){
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Could not register command (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_command->_config_file),this_command->_start_line);
#endif
		return ERROR;
	        }
...
int xodtemplate_register_contactgroup(xodtemplate_contactgroup *this_contactgroup){
	...
	if(new_contactgroup==NULL){
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Could not register contactgroup (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_contactgroup->_config_file),this_contactgroup->_start_line);
#endif
		return ERROR;
	        }
...
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Contactgroup has no members (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_contactgroup->_config_file),this_contactgroup->_start_line);
#endif
		return ERROR;
	        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not add contact '%s' to contactgroup (config file '%s', starting on line %d)\n",contact_name,xodtemplate_config_file_name(this_contactgroup->_config_file),this_contactgroup->_start_line);
#endif
			return ERROR;
		        }
	        }
...
int xodtemplate_register_hostgroup(xodtemplate_hostgroup *this_hostgroup){
	...
	if(new_hostgroup==NULL){
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Could not register hostgroup (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_hostgroup->_config_file),this_hostgroup->_start_line);
#endif
		return ERROR;
	        }
...
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Hostgroup has no members (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_hostgroup->_config_file),this_hostgroup->_start_line);
#endif
		return ERROR;
	        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not add host '%s' to hostgroup (config file '%s', starting on line %d)\n",host_name,xodtemplate_config_file_name(this_hostgroup->_config_file),this_hostgroup->_start_line);
#endif
			return ERROR;
		        }
	        }
...
int xodtemplate_register_servicegroup(xodtemplate_servicegroup *this_servicegroup){
	...
	if(new_servicegroup==NULL){
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Could not register servicegroup (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_servicegroup->_config_file),this_servicegroup->_start_line);
#endif
		return ERROR;
	        }
...
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Servicegroup has no members (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_servicegroup->_config_file),this_servicegroup->_start_line);
#endif
		return ERROR;
	        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Missing service name in servicegroup definition (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_servicegroup->_config_file),this_servicegroup->_start_line);
#endif
			return ERROR;
	                }
...
int xodtemplate_register_servicedependency(xodtemplate_servicedependency *this_servicedependency){
	...
	if(this_servicedependency->have_execution_dependency_options==TRUE){
		...
		if(new_servicedependency==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not register service execution dependency (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_servicedependency->_config_file),this_servicedependency->_start_line);
#endif
			return ERROR;
		        }
	        }
	if(this_servicedependency->have_notification_dependency_options==TRUE){
		...
		if(new_servicedependency==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not register service notification dependency (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_servicedependency->_config_file),this_servicedependency->_start_line);
#endif
			return ERROR;
		        }
	        }
...
int xodtemplate_register_serviceescalation(xodtemplate_serviceescalation *this_serviceescalation){
	...
	if(new_serviceescalation==NULL){
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Could not register service escalation (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_serviceescalation->_config_file),this_serviceescalation->_start_line);
#endif
		return ERROR;
	        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not add contactgroup '%s' to service escalation (config file '%s', starting on line %d)\n",contact_group,xodtemplate_config_file_name(this_serviceescalation->_config_file),this_serviceescalation->_start_line);
#endif
				return ERROR;
				}
			}
...
int xodtemplate_register_contact(xodtemplate_contact *this_contact){
	...
	if(new_contact==NULL){
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Could not register contact (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_contact->_config_file),this_contact->_start_line);
#endif
		return ERROR;
	        }
...
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not add host notification command '%s' to contact (config file '%s', starting on line %d)\n",command_name,xodtemplate_config_file_name(this_contact->_config_file),this_contact->_start_line);
#endif
				return ERROR;
			        }
		        }
	...
	if(this_contact->service_notification_commands!=NULL){
		...
		for(command_name=strtok(this_contact->service_notification_commands,", ");command_name!=NULL;command_name=strtok(NULL,", ")){
			...
			if(new_commandsmember==NULL){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not add service notification command '%s' to contact (config file '%s', starting on line %d)\n",command_name,xodtemplate_config_file_name(this_contact->_config_file),this_contact->_start_line);
#endif
				return ERROR;
			        }
		        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not custom variable to contact (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_contact->_config_file),this_contact->_start_line);
#endif
			return ERROR;
		        }
	        }
...
int xodtemplate_register_host(xodtemplate_host *this_host){
	...
	if(new_host==NULL){
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Could not register host (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_host->_config_file),this_host->_start_line);
#endif
		return ERROR;
	        }
	...
	if(this_host->parents!=NULL){
		...
		for(parent_host=strtok(this_host->parents,",");parent_host!=NULL;parent_host=strtok(NULL,",")){
			...
			if(new_hostsmember==NULL){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not add parent host '%s' to host (config file '%s', starting on line %d)\n",parent_host,xodtemplate_config_file_name(this_host->_config_file),this_host->_start_line);
#endif
				return ERROR;
			        }
		        }
	...
	if(this_host->contact_groups!=NULL){
		...
		for(contact_group=strtok(this_host->contact_groups,",");contact_group!=NULL;contact_group=strtok(NULL,",")){
			...
			if(new_contactgroupsmember==NULL){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not add contactgroup '%s' to host (config file '%s', starting on line %d)\n",contact_group,xodtemplate_config_file_name(this_host->_config_file),this_host->_start_line);
#endif
				return ERROR;
			        }
	                }
	...
	if(this_host->contacts!=NULL){
		...
		for(contact_name=strtok(this_host->contacts,",");contact_name!=NULL;contact_name=strtok(NULL,",")){
			...
			if(new_contactsmember==NULL){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not add contact '%s' to host (config file '%s', starting on line %d)\n",contact_name,xodtemplate_config_file_name(this_host->_config_file),this_host->_start_line);
#endif
				return ERROR;
			        }
	                }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not custom variable to host (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_host->_config_file),this_host->_start_line);
#endif
			return ERROR;
		        }
	        }
...
int xodtemplate_register_service(xodtemplate_service *this_service){
	...
	if(new_service==NULL){
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Could not register service (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_service->_config_file),this_service->_start_line);
#endif
		return ERROR;
	        }
	...
	if(this_service->contact_groups!=NULL){
		...
		for(contact_group=strtok(this_service->contact_groups,",");contact_group!=NULL;contact_group=strtok(NULL,",")){
			...
			if(new_contactgroupsmember==NULL){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not add contactgroup '%s' to service (config file '%s', starting on line %d)\n",contact_group,xodtemplate_config_file_name(this_service->_config_file),this_service->_start_line);
#endif
				return ERROR;
			        }
	                }
	...
	if(this_service->contacts!=NULL){
		...
		for(contact_name=strtok(this_service->contacts,",");contact_name!=NULL;contact_name=strtok(NULL,",")){
			...
			if(new_contactsmember==NULL){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not add contact '%s' to service (config file '%s', starting on line %d)\n",contact_name,xodtemplate_config_file_name(this_service->_config_file),this_service->_start_line);
#endif
				return ERROR;
		                }
		        }
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not custom variable to service (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_service->_config_file),this_service->_start_line);
#endif
			return ERROR;
		        }
	        }
...
int xodtemplate_register_hostdependency(xodtemplate_hostdependency *this_hostdependency){
	...
	if(this_hostdependency->have_execution_dependency_options==TRUE){
		...
		if(new_hostdependency==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not register host execution dependency (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_hostdependency->_config_file),this_hostdependency->_start_line);
#endif
			return ERROR;
                        }
	        }
	...
	if(this_hostdependency->have_notification_dependency_options==TRUE){
		...
		if(new_hostdependency==NULL){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not register host notification dependency (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_hostdependency->_config_file),this_hostdependency->_start_line);
#endif
			return ERROR;
                        }
	        }
...
int xodtemplate_register_hostescalation(xodtemplate_hostescalation *this_hostescalation){
	...
	if(new_hostescalation==NULL){
#ifdef NSCORE
		logit(NSLOG_CONFIG_ERROR, "Error: Could not register host escalation (config file '%s', starting on line %d)\n",xodtemplate_config_file_name(this_hostescalation->_config_file),this_hostescalation->_start_line);
#endif
		return ERROR;
	        }
	...
	if(this_hostescalation->contact_groups!=NULL){
		...
		for(contact_group=strtok(this_hostescalation->contact_groups,",");contact_group!=NULL;contact_group=strtok(NULL,",")){
			...
			if(new_contactgroupsmember==NULL){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not add contactgroup '%s' to host escalation (config file '%s', starting on line %d)\n",contact_group,xodtemplate_config_file_name(this_hostescalation->_config_file),this_hostescalation->_start_line);
#endif
				return ERROR;
			        }
	                }
	...
	if(this_hostescalation->contacts!=NULL){
		...
		for(contact_name=strtok(this_hostescalation->contacts,", ");contact_name!=NULL;contact_name=strtok(NULL,", ")){
			...
			if(new_contactsmember==NULL){
#ifdef NSCORE
				logit(NSLOG_CONFIG_ERROR, "Error: Could not add contact '%s' to host escalation (config file '%s', starting on line %d)\n",contact_name,xodtemplate_config_file_name(this_hostescalation->_config_file),this_hostescalation->_start_line);
#endif
				return ERROR;
				}
			}
...
#ifdef NSCORE
		logit(NSLOG_CONFIG_WARNING, "Warning: Extended service information (serviceextinfo) definitions are deprecated in Nagios 3.x and will not be supported in future versions.  Please merge variables in these definitions with your service definitions.");
#endif
	        }
	*/
#endif
	...
	if(xodtemplate_serviceextinfo_list!=NULL){
#ifdef NSCORE
		logit(NSLOG_CONFIG_WARNING, "Warning: Extended host information (hostextinfo) definitions are deprecated in Nagios 3.x and will not be supported in future versions.  Please merge variables in these definitions with your host definitions.");
#endif
	        }
	*/
#endif
...
int xodtemplate_cache_objects(char *cache_file){
	...
	if(fp==NULL){
#ifdef NSCORE
		logit(NSLOG_CONFIG_WARNING, "Warning: Could not open object cache file '%s' for writing!\n",cache_file);
#endif
		return ERROR;
	        }
...
int xodtemplate_free_memory(void){
	...
	for(this_serviceescalation=xodtemplate_serviceescalation_list;this_serviceescalation!=NULL;this_serviceescalation=next_serviceescalation){
		...
		my_free((void **)&this_serviceescalation->contacts);
	...
	for(this_host=xodtemplate_host_list;this_host!=NULL;this_host=next_host){
		...
		my_free((void **)&this_host->contacts);
	...
	for(this_service=xodtemplate_service_list;this_service!=NULL;this_service=next_service){
		...
		my_free((void **)&this_service->contacts);
	...
	for(this_hostescalation=xodtemplate_hostescalation_list;this_hostescalation!=NULL;this_hostescalation=next_hostescalation){
		...
		my_free((void **)&this_hostescalation->contacts);
...
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not find any contactgroup matching '%s' (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(_config_file),_start_line);
#endif
			break;
	                }
	        }
...
int xodtemplate_expand_contacts(xodtemplate_memberlist **list, xodtemplate_memberlist **reject_list, char *contacts, int _config_file, int _start_line){
	...
	for(temp_ptr=strtok(contact_names,",");temp_ptr;temp_ptr=strtok(NULL,",")){
		...
		if(found_match==FALSE){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not find any contact matching '%s' (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(_config_file),_start_line);
#endif
			break;
	                }
	        }
...
int xodtemplate_expand_hostgroups(xodtemplate_memberlist **list, xodtemplate_memberlist **reject_list, char *hostgroups, int _config_file, int _start_line){
	...
	for(temp_ptr=strtok(hostgroup_names,",");temp_ptr;temp_ptr=strtok(NULL,",")){
		...
		if(found_match==FALSE){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not find any hostgroup matching '%s' (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(_config_file),_start_line);
#endif
			break;
	                }
	        }
...
int xodtemplate_expand_hosts(xodtemplate_memberlist **list, xodtemplate_memberlist **reject_list, char *hosts, int _config_file, int _start_line){
	...
	for(temp_ptr=strtok(host_names,",");temp_ptr;temp_ptr=strtok(NULL,",")){
		...
		if(found_match==FALSE){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not find any host matching '%s' (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(_config_file),_start_line);
#endif
			break;
	                }
	        }
...
int xodtemplate_expand_servicegroups(xodtemplate_memberlist **list, xodtemplate_memberlist **reject_list, char *servicegroups, int _config_file, int _start_line){
	...
	for(temp_ptr=strtok(servicegroup_names,",");temp_ptr;temp_ptr=strtok(NULL,",")){
		...
		if(found_match==FALSE){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not find any servicegroup matching '%s' (config file '%s', starting on line %d)\n",temp_ptr,xodtemplate_config_file_name(_config_file),_start_line);
#endif
			break;
		        }
	        }
...
int xodtemplate_expand_services(xodtemplate_memberlist **list, xodtemplate_memberlist **reject_list, char *host_name, char *services, int _config_file, int _start_line){
	...
	for(temp_ptr=strtok(service_names,",");temp_ptr;temp_ptr=strtok(NULL,",")){
		...
		if(found_match==FALSE){
#ifdef NSCORE
			logit(NSLOG_CONFIG_ERROR, "Error: Could not find a service matching host name '%s' and description '%s' (config file '%s', starting on line %d)\n",host_name,temp_ptr,xodtemplate_config_file_name(_config_file),_start_line);
#endif
			break;
	                }
	        }
