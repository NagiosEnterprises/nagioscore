	...
	while((dirfile=readdir(dirp))!=NULL){
		...
		if(dirfile->d_type==DT_UNKNOWN || dirfile->d_type==DT_DIR || dirfile->d_type==DT_LNK){
			...
			if(dirfile->d_type==DT_LNK || (dirfile->d_type==DT_UNKNOWN && S_ISLNK(stat_buf.st_mode))){
...
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR,TRUE,"Error: Cannot follow symlink '%s' - Too big!\n",file);
#endif
					return ERROR;
				        }
				...
				if(readlink_count==-1){
#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR,TRUE,"Error: Cannot read symlink '%s': %s!\n",file, strerror(errno));
#endif
					return ERROR;
				        }
				...
				if(x!=0){
					...
					if(errno==ENOENT){
#ifdef NSCORE
						logit(NSLOG_CONFIG_ERROR,TRUE,"Error: symlink '%s' points to non-existent '%s'!\n",file,link_buffer);
#endif
						return ERROR;
					        }

#ifdef NSCORE
					logit(NSLOG_CONFIG_ERROR,TRUE,"Error: Cannot stat symlinked from '%s' to %s!\n",file,link_buffer);
#endif
					return ERROR;
				        }
