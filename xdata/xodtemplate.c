...
/*	if(result == OK) */
	...
	if (test_scheduling == TRUE) {
		...
		if (use_precached_objects == FALSE) {
			...
			runtime[8] =
				(double)((double)(tv[10].tv_sec - tv[9].tv_sec) +
						 (double)((tv[10].tv_usec - tv[9].tv_usec) / 1000.0) / 1000.0);
		} else {
			runtime[1] = 0.0;
			runtime[2] = 0.0;
			runtime[3] = 0.0;
			...
			runtime[7] = 0.0;
			runtime[8] =
				(double)((double)(tv[10].tv_sec - tv[1].tv_sec) +
						 (double)((tv[10].tv_usec - tv[1].tv_usec) / 1000.0) / 1000.0);
		}
		runtime[9] =
			(double)((double)(tv[11].tv_sec - tv[10].tv_sec) +
					 (double)((tv[11].tv_usec - tv[10].tv_usec) / 1000.0) / 1000.0);
		runtime[10] =
			(double)((double)(tv[11].tv_sec - tv[0].tv_sec) +
					 (double)((tv[11].tv_usec - tv[0].tv_usec) / 1000.0) / 1000.0);
		...
		printf("Object Config Source: %s\n\n",
			   (use_precached_objects ==
				TRUE) ? "Pre-cached config file" : "Config files (uncached)");
		...
		if (use_precached_objects == FALSE)
			printf("* = %.6lf sec (%.2f%%) estimated savings",
				   (runtime[10] - runtime[9] - runtime[8] - runtime[0]) / 4,
				   ((runtime[10] - runtime[9] - runtime[8] -
					 runtime[0]) / runtime[10]) * 25.0);
		printf("\n");
		printf("\n\n");
	}
#endif
...
int xodtemplate_process_config_file(char *filename, int options)
{
	...
	if (!(xodtemplate_current_config_file % 256)) {
		xodtemplate_config_files =
			(char **)realloc(xodtemplate_config_files,
							 (xodtemplate_current_config_file + 256) * sizeof(char *));
		if (xodtemplate_config_files == NULL)
			return ERROR;
	}
	...
	while (1) {
		...
		if (strstr(input, "define") == input) {
			...
			if (xodtemplate_begin_object_definition
				(input, options, xodtemplate_current_config_file, current_line) == ERROR) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
	...
	case XODTEMPLATE_TIMEPERIOD:
		...
		if (!strcmp(variable, "use")) {
			...
			if (result == OK) {
				...
				result =
					skiplist_insert(xobject_template_skiplists[TIMEPERIOD_SKIPLIST],
				...
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
						  ...
						  value, xodtemplate_config_file_name(temp_timeperiod->_config_file),
			...
			if (result == OK) {
				/* add timeperiod to template skiplist for fast searches */
				result =
					skiplist_insert(xobject_skiplists[TIMEPERIOD_SKIPLIST],
									(void *)temp_timeperiod);
				switch (result) {
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
		...
		else if (xodtemplate_parse_timeperiod_directive(temp_timeperiod, variable, value) ==
				 OK)
			result = OK;
		else {
			logit(NSLOG_CONFIG_ERROR, TRUE,
	...
	case XODTEMPLATE_COMMAND:
		...
		if (!strcmp(variable, "use")) {
			...
			if (result == OK) {
				...
				result =
					skiplist_insert(xobject_template_skiplists[COMMAND_SKIPLIST],
									(void *)temp_command);
				switch (result) {
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
						  "Warning: Duplicate definition found for command '%s' (config file '%s', starting on line %d)\n",
						  value, xodtemplate_config_file_name(temp_command->_config_file),
			...
			if (result == OK) {
				/* add command to template skiplist for fast searches */
				result =
					skiplist_insert(xobject_skiplists[COMMAND_SKIPLIST], (void *)temp_command);
				switch (result) {
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
						  "Warning: Duplicate definition found for command '%s' (config file '%s', starting on line %d)\n",
	...
	case XODTEMPLATE_CONTACTGROUP:
		...
		if (!strcmp(variable, "use")) {
			...
			if (result == OK) {
				...
				result =
					skiplist_insert(xobject_template_skiplists[CONTACTGROUP_SKIPLIST],
									(void *)temp_contactgroup);
				switch (result) {
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
						  "Warning: Duplicate definition found for contactgroup '%s' (config file '%s', starting on line %d)\n",
						  value, xodtemplate_config_file_name(temp_contactgroup->_config_file),
			...
			if (result == OK) {
				/* add contactgroup to template skiplist for fast searches */
				result =
					skiplist_insert(xobject_skiplists[CONTACTGROUP_SKIPLIST],
									(void *)temp_contactgroup);
				switch (result) {
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
			...
			if (strcmp(value, XODTEMPLATE_NULL)) {
				if (temp_contactgroup->members == NULL)
					...
					temp_contactgroup->members =
						(char *)realloc(temp_contactgroup->members,
										strlen(temp_contactgroup->members) + strlen(value) +
										2);
					if (temp_contactgroup->members != NULL) {
	...
	case XODTEMPLATE_HOSTGROUP:
		...
		if (!strcmp(variable, "use")) {
			...
			if (result == OK) {
				...
				result =
					skiplist_insert(xobject_template_skiplists[HOSTGROUP_SKIPLIST],
									(void *)temp_hostgroup);
				switch (result) {
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
						  "Warning: Duplicate definition found for hostgroup '%s' (config file '%s', starting on line %d)\n",
						  value, xodtemplate_config_file_name(temp_hostgroup->_config_file),
			...
			if (result == OK) {
				/* add hostgroup to template skiplist for fast searches */
				result =
					skiplist_insert(xobject_skiplists[HOSTGROUP_SKIPLIST],
									(void *)temp_hostgroup);
				switch (result) {
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
			...
			if (strcmp(value, XODTEMPLATE_NULL)) {
				if (temp_hostgroup->members == NULL)
					...
					temp_hostgroup->members =
						(char *)realloc(temp_hostgroup->members,
			...
			if (strcmp(value, XODTEMPLATE_NULL)) {
				if (temp_hostgroup->hostgroup_members == NULL)
					...
					temp_hostgroup->hostgroup_members =
						(char *)realloc(temp_hostgroup->hostgroup_members,
										strlen(temp_hostgroup->hostgroup_members) +
										strlen(value) + 2);
					if (temp_hostgroup->hostgroup_members != NULL) {
	...
	case XODTEMPLATE_SERVICEGROUP:
		...
		if (!strcmp(variable, "use")) {
			...
			if (result == OK) {
				...
				result =
					skiplist_insert(xobject_template_skiplists[SERVICEGROUP_SKIPLIST],
									(void *)temp_servicegroup);
				switch (result) {
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
						  "Warning: Duplicate definition found for servicegroup '%s' (config file '%s', starting on line %d)\n",
						  value, xodtemplate_config_file_name(temp_servicegroup->_config_file),
			...
			if (result == OK) {
				/* add servicegroup to template skiplist for fast searches */
				result =
					skiplist_insert(xobject_skiplists[SERVICEGROUP_SKIPLIST],
									(void *)temp_servicegroup);
				switch (result) {
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
				...
				if (temp_servicegroup->members == NULL)
					...
					temp_servicegroup->members =
						(char *)realloc(temp_servicegroup->members,
										strlen(temp_servicegroup->members) + strlen(value) +
										2);
					if (temp_servicegroup->members != NULL) {
			...
			if (strcmp(value, XODTEMPLATE_NULL)) {
				if (temp_servicegroup->servicegroup_members == NULL)
					...
					temp_servicegroup->servicegroup_members =
						(char *)realloc(temp_servicegroup->servicegroup_members,
	...
	case XODTEMPLATE_SERVICEDEPENDENCY:
		...
		if (!strcmp(variable, "use")) {
			...
			if (result == OK) {
				...
				result =
					skiplist_insert(xobject_template_skiplists[SERVICEDEPENDENCY_SKIPLIST],
		...
		} else if (!strcmp(variable, "servicegroup") || !strcmp(variable, "servicegroups")
				   || !strcmp(variable, "servicegroup_name")) {
			if (strcmp(value, XODTEMPLATE_NULL)) {
				if ((temp_servicedependency->servicegroup_name =
					 (char *)strdup(value)) == NULL)
					result = ERROR;
			}
			...
			if (strcmp(value, XODTEMPLATE_NULL)) {
				if ((temp_servicedependency->dependent_hostgroup_name =
					 (char *)strdup(value)) == NULL)
					result = ERROR;
			}
			...
			for (temp_ptr = strtok(value, ", "); temp_ptr; temp_ptr = strtok(NULL, ", ")) {
				if (!strcmp(temp_ptr, "o") || !strcmp(temp_ptr, "ok"))
					...
					flag_set(temp_servicedependency->notification_failure_options,
							 OPT_UNKNOWN);
				else if (!strcmp(temp_ptr, "w") || !strcmp(temp_ptr, "warning"))
					flag_set(temp_servicedependency->notification_failure_options,
							 OPT_WARNING);
	...
	case XODTEMPLATE_SERVICEESCALATION:
		...
		if (!strcmp(variable, "use")) {
			...
			if (result == OK) {
				...
				result =
					skiplist_insert(xobject_template_skiplists[SERVICEESCALATION_SKIPLIST],
			...
			temp_serviceescalation->have_service_description = TRUE;
				   ...
				   || !strcmp(variable, "servicegroup_name")) {
			if (strcmp(value, XODTEMPLATE_NULL)) {
				if ((temp_serviceescalation->servicegroup_name =
					 (char *)strdup(value)) == NULL)
					result = ERROR;
			}
			...
			if (*value == '\0') {
				logit(NSLOG_CONFIG_WARNING, TRUE,
					  "Error: Empty value for contact_groups (config file '%s', starting on line %d)\n",
					  xodtemplate_config_file_name(temp_serviceescalation->_config_file),
					  temp_serviceescalation->_start_line);
				result = ERROR;
				break;
			...
			if (*value == '\0') {
				logit(NSLOG_CONFIG_WARNING, TRUE,
					  "Error: Empty value for contacts (config file '%s', starting on line %d)\n",
					  xodtemplate_config_file_name(temp_serviceescalation->_config_file),
					  temp_serviceescalation->_start_line);
				result = ERROR;
				break;
	...
	case XODTEMPLATE_CONTACT:
		...
		if (!strcmp(variable, "use")) {
			...
			if (result == OK) {
				...
				result =
					skiplist_insert(xobject_template_skiplists[CONTACT_SKIPLIST],
									(void *)temp_contact);
				switch (result) {
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
						  "Warning: Duplicate definition found for contact '%s' (config file '%s', starting on line %d)\n",
						  value, xodtemplate_config_file_name(temp_contact->_config_file),
			...
			if (result == OK) {
				/* add contact to template skiplist for fast searches */
				result =
					skiplist_insert(xobject_skiplists[CONTACT_SKIPLIST], (void *)temp_contact);
				switch (result) {
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
						  "Warning: Duplicate definition found for contact '%s' (config file '%s', starting on line %d)\n",
			...
			if (!strcmp(variable, "minimum_value")) {
				logit(NSLOG_CONFIG_WARNING, TRUE,
					  "WARNING: The minimum_value attribute is deprecated and will be removed in future versions. Please use minimum_importance instead.\n");
			}
	...
	case XODTEMPLATE_HOST:
		...
		if (!strcmp(variable, "use")) {
			...
			if (result == OK) {
				...
				result =
					skiplist_insert(xobject_template_skiplists[HOST_SKIPLIST],
									(void *)temp_host);
				switch (result) {
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
						  "Warning: Duplicate definition found for host '%s' (config file '%s', starting on line %d)\n",
						  value, xodtemplate_config_file_name(temp_host->_config_file),
			...
			if (!strcmp(variable, "hourly_value")) {
				logit(NSLOG_CONFIG_WARNING, TRUE,
					  "WARNING: The hourly_value attribute is deprecated and will be removed in future versions. Please use importance instead.\n");
			}
			...
			if (xodtemplate_add_custom_variable_to_host
				(temp_host, customvarname, customvarvalue) == NULL) {
				my_free(customvarname);
				my_free(customvarvalue);
				return ERROR;
			}
	...
	case XODTEMPLATE_SERVICE:
		...
		if (!strcmp(variable, "use")) {
			...
			if (result == OK) {
				...
				result =
					skiplist_insert(xobject_template_skiplists[SERVICE_SKIPLIST],
									(void *)temp_service);
				switch (result) {
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
						  "Warning: Duplicate definition found for service '%s' (config file '%s', starting on line %d)\n",
						  value, xodtemplate_config_file_name(temp_service->_config_file),
			...
			if (result == OK && force_skiplists == TRUE && temp_service->host_name != NULL
				&& temp_service->service_description != NULL) {
				/* add service to template skiplist for fast searches */
				result =
					skiplist_insert(xobject_skiplists[SERVICE_SKIPLIST], (void *)temp_service);
				switch (result) {
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
						  "Warning: Duplicate definition found for service '%s' (config file '%s', starting on line %d)\n",
	...
	case XODTEMPLATE_HOSTDEPENDENCY:
		...
		if (!strcmp(variable, "use")) {
			...
			if (result == OK) {
				/* add dependency to template skiplist for fast searches */
				result =
					skiplist_insert(xobject_template_skiplists[HOSTDEPENDENCY_SKIPLIST],
			...
			temp_hostdependency->have_host_name = TRUE;
				   ...
				   || !strcmp(variable, "dependent_hostgroup_name")) {
			if (strcmp(value, XODTEMPLATE_NULL)) {
				if ((temp_hostdependency->dependent_hostgroup_name =
					 (char *)strdup(value)) == NULL)
					result = ERROR;
			}
			...
			for (temp_ptr = strtok(value, ", "); temp_ptr; temp_ptr = strtok(NULL, ", ")) {
				if (!strcmp(temp_ptr, "o") || !strcmp(temp_ptr, "up"))
					...
					flag_set(temp_hostdependency->notification_failure_options,
							 OPT_UNREACHABLE);
				else if (!strcmp(temp_ptr, "p") || !strcmp(temp_ptr, "pending"))
	...
	case XODTEMPLATE_HOSTESCALATION:
		...
		if (!strcmp(variable, "use")) {
			...
			if (result == OK) {
				/* add escalation to template skiplist for fast searches */
				result =
					skiplist_insert(xobject_template_skiplists[HOSTESCALATION_SKIPLIST],
	...
	case XODTEMPLATE_HOSTEXTINFO:
		...
		if (!strcmp(variable, "use")) {
			...
			if (result == OK) {
				...
				result =
					skiplist_insert(xobject_template_skiplists[HOSTEXTINFO_SKIPLIST],
									(void *)temp_hostextinfo);
				switch (result) {
				case SKIPLIST_ERROR_DUPLICATE:
					logit(NSLOG_CONFIG_WARNING, TRUE,
						  "Warning: Duplicate definition found for extended host info '%s' (config file '%s', starting on line %d)\n",
						  value, xodtemplate_config_file_name(temp_hostextinfo->_config_file),
	...
	case XODTEMPLATE_SERVICEEXTINFO:
		...
		if (!strcmp(variable, "use")) {
			...
			if (result == OK) {
				/* add to template skiplist for fast searches */
				result =
					skiplist_insert(xobject_template_skiplists[SERVICEEXTINFO_SKIPLIST],
																		   ...
																		   char *varvalue)
{
	...
	return xodtemplate_add_custom_variable_to_object(&hst->custom_variables, varname,
													 varvalue);
}
...
xodtemplate_customvariablesmember
	*xodtemplate_add_custom_variable_to_service(xodtemplate_service * svc, char *varname,
												char *varvalue)
{

	return xodtemplate_add_custom_variable_to_object(&svc->custom_variables, varname,
													 varvalue);
}
...
xodtemplate_customvariablesmember
	*xodtemplate_add_custom_variable_to_contact(xodtemplate_contact * cntct, char *varname,
												char *varvalue)
{

	return xodtemplate_add_custom_variable_to_object(&cntct->custom_variables, varname,
													 varvalue);
}
...
xodtemplate_customvariablesmember
	*xodtemplate_add_custom_variable_to_object(xodtemplate_customvariablesmember ** object_ptr,
											   char *varname, char *varvalue)
{
	...
	if ((new_customvariablesmember =
		 malloc(sizeof(xodtemplate_customvariablesmember))) == NULL)
		return NULL;
...
int xodtemplate_parse_timeperiod_directive(xodtemplate_timeperiod * tperiod, char *var, char *val)
{
	...
	/* calendar dates */
			  ...
			  sscanf(input, "%4d-%2d-%2d - %4d-%2d-%2d / %d %[0-9:, -]", &syear, &smon, &smday,
					 &eyear, &emon, &emday, &skip_interval, temp_buffer[0])) == 8) {
		/* add timerange exception */
		if (xodtemplate_add_exception_to_timeperiod
			(tperiod, DATERANGE_CALENDAR_DATE, syear, smon - 1, smday, 0, 0, eyear, emon - 1,
			 emday, 0, 0, skip_interval, temp_buffer[0]) == NULL)
			result = ERROR;
	}

	else if ((items =
			  sscanf(input, "%4d-%2d-%2d / %d %[0-9:, -]", &syear, &smon, &smday,
		...
		if (xodtemplate_add_exception_to_timeperiod
			...
			result = ERROR;
			  ...
			  sscanf(input, "%4d-%2d-%2d - %4d-%2d-%2d %[0-9:, -]", &syear, &smon, &smday,
					 &eyear, &emon, &emday, temp_buffer[0])) == 7) {
		/* add timerange exception */
		if (xodtemplate_add_exception_to_timeperiod
			(tperiod, DATERANGE_CALENDAR_DATE, syear, smon - 1, smday, 0, 0, eyear, emon - 1,
			 emday, 0, 0, 0, temp_buffer[0]) == NULL)
			result = ERROR;
	}

	else if ((items =
			  sscanf(input, "%4d-%2d-%2d %[0-9:, -]", &syear, &smon, &smday,
					 temp_buffer[0])) == 4) {
		eyear = syear;
		emon = smon;
		emday = smday;
	...
	/* other types... */
			  ...
			  sscanf(input, "%[a-z] %d %[a-z] - %[a-z] %d %[a-z] / %d %[0-9:, -]",
					 temp_buffer[0], &swday_offset, temp_buffer[1], temp_buffer[2],
					 &ewday_offset, temp_buffer[3], &skip_interval, temp_buffer[4])) == 8) {
		...
		if ((result = xodtemplate_get_weekday_from_string(temp_buffer[0], &swday)) == OK
			...
			if (xodtemplate_add_exception_to_timeperiod
				(tperiod, DATERANGE_MONTH_WEEK_DAY, 0, smon, 0, swday, swday_offset, 0, emon,
				 0, ewday, ewday_offset, skip_interval, temp_buffer[4]) == NULL)
				result = ERROR;
		}
	}

	else if ((items =
			  sscanf(input, "%[a-z] %d - %[a-z] %d / %d %[0-9:, -]", temp_buffer[0], &smday,
					 temp_buffer[1], &emday, &skip_interval, temp_buffer[2])) == 6) {
		...
		if ((result = xodtemplate_get_weekday_from_string(temp_buffer[0], &swday)) == OK
			...
			/* add timeperiod exception */
			if (xodtemplate_add_exception_to_timeperiod
				(tperiod, DATERANGE_WEEK_DAY, 0, 0, 0, swday, swday_offset, 0, 0, 0, ewday,
				 ewday_offset, skip_interval, temp_buffer[2]) == NULL)
				result = ERROR;
		} else if ((result = xodtemplate_get_month_from_string(temp_buffer[0], &smon)) == OK
				   && (result =
					   xodtemplate_get_month_from_string(temp_buffer[1], &emon)) == OK) {
			/* february 1 - march 15 / 3 */
			/* add timeperiod exception */
			if (xodtemplate_add_exception_to_timeperiod
				(tperiod, DATERANGE_MONTH_DATE, 0, smon, smday, 0, 0, 0, emon, emday, 0, 0,
				 skip_interval, temp_buffer[2]) == NULL)
				result = ERROR;
			...
			result = OK;
			if (xodtemplate_add_exception_to_timeperiod
				(tperiod, DATERANGE_MONTH_DAY, 0, 0, smday, 0, 0, 0, 0, emday, 0, 0,
				 skip_interval, temp_buffer[2]) == NULL)
				result = ERROR;
		}
	}
			  ...
			  sscanf(input, "%[a-z] %d - %d / %d %[0-9:, -]", temp_buffer[0], &smday, &emday,
					 &skip_interval, temp_buffer[1])) == 5) {
		/* february 1 - 15 / 3 */
		/* monday 2 - 3 / 2 */
		/* day 1 - 25 / 4 */
		if ((result = xodtemplate_get_weekday_from_string(temp_buffer[0], &swday)) == OK) {
			...
			if (xodtemplate_add_exception_to_timeperiod
				(tperiod, DATERANGE_WEEK_DAY, 0, 0, 0, swday, swday_offset, 0, 0, 0, ewday,
				 ewday_offset, skip_interval, temp_buffer[1]) == NULL)
				result = ERROR;
		} else if ((result = xodtemplate_get_month_from_string(temp_buffer[0], &smon)) == OK) {
			/* february 3 - 5 */
			emon = smon;
			...
			if (xodtemplate_add_exception_to_timeperiod
				(tperiod, DATERANGE_MONTH_DATE, 0, smon, smday, 0, 0, 0, emon, emday, 0, 0,
				 skip_interval, temp_buffer[1]) == NULL)
				result = ERROR;
			...
			if (xodtemplate_add_exception_to_timeperiod
			  ...
			  sscanf(input, "%[a-z] %d %[a-z] - %[a-z] %d %[a-z] %[0-9:, -]", temp_buffer[0],
					 &swday_offset, temp_buffer[1], temp_buffer[2], &ewday_offset,
					 temp_buffer[3], temp_buffer[4])) == 7) {
		...
		if ((result = xodtemplate_get_weekday_from_string(temp_buffer[0], &swday)) == OK
			...
			if (xodtemplate_add_exception_to_timeperiod
				(tperiod, DATERANGE_MONTH_WEEK_DAY, 0, smon, 0, swday, swday_offset, 0, emon,
				 0, ewday, ewday_offset, 0, temp_buffer[4]) == NULL)
				result = ERROR;
		}
	}

	else if ((items =
			  sscanf(input, "%[a-z] %d - %d %[0-9:, -]", temp_buffer[0], &smday, &emday,
					 temp_buffer[1])) == 4) {
		/* february 3 - 5 */
		/* thursday 2 - 4 */
		/* day 1 - 4 */
		if ((result = xodtemplate_get_weekday_from_string(temp_buffer[0], &swday)) == OK) {
			...
			if (xodtemplate_add_exception_to_timeperiod
				(tperiod, DATERANGE_WEEK_DAY, 0, 0, 0, swday, swday_offset, 0, 0, 0, ewday,
				 ewday_offset, 0, temp_buffer[1]) == NULL)
				result = ERROR;
		} else if ((result = xodtemplate_get_month_from_string(temp_buffer[0], &smon)) == OK) {
			/* february 3 - 5 */
			emon = smon;
			/* add timeperiod exception */
			if (xodtemplate_add_exception_to_timeperiod
				(tperiod, DATERANGE_MONTH_DATE, 0, smon, smday, 0, 0, 0, emon, emday, 0, 0, 0,
				 temp_buffer[1]) == NULL)
				result = ERROR;
		} else if (!strcmp(temp_buffer[0], "day")) {
			/* day 1 - 4 */
			...
			result = OK;
			if (xodtemplate_add_exception_to_timeperiod
				(tperiod, DATERANGE_MONTH_DAY, 0, 0, smday, 0, 0, 0, 0, emday, 0, 0, 0,
				 temp_buffer[1]) == NULL)
				result = ERROR;
		}
	}
			  ...
			  sscanf(input, "%[a-z] %d - %[a-z] %d %[0-9:, -]", temp_buffer[0], &smday,
					 temp_buffer[1], &emday, temp_buffer[2])) == 5) {
		/* february 1 - march 15 */
		/* monday 2 - thursday 3 */
		/* day 1 - day 5 */
		if ((result = xodtemplate_get_weekday_from_string(temp_buffer[0], &swday)) == OK
			...
			if (xodtemplate_add_exception_to_timeperiod
				(tperiod, DATERANGE_WEEK_DAY, 0, 0, 0, swday, swday_offset, 0, 0, 0, ewday,
				 ewday_offset, 0, temp_buffer[2]) == NULL)
				result = ERROR;
		} else if ((result = xodtemplate_get_month_from_string(temp_buffer[0], &smon)) == OK
				   && (result =
					   xodtemplate_get_month_from_string(temp_buffer[1], &emon)) == OK) {
			...
			if (xodtemplate_add_exception_to_timeperiod
			  ...
			  sscanf(input, "%[a-z] %d%*[ \t]%[0-9:, -]", temp_buffer[0], &smday,
					 temp_buffer[1])) == 3) {
		/* february 3 */
		/* thursday 2 */
		/* day 1 */
		if ((result = xodtemplate_get_weekday_from_string(temp_buffer[0], &swday)) == OK) {
			...
			if (xodtemplate_add_exception_to_timeperiod
			  ...
			  sscanf(input, "%[a-z] %d %[a-z] %[0-9:, -]", temp_buffer[0], &swday_offset,
					 temp_buffer[1], temp_buffer[2])) == 4) {
		/* thursday 3 february */
		if ((result = xodtemplate_get_weekday_from_string(temp_buffer[0], &swday)) == OK
			&& (result = xodtemplate_get_month_from_string(temp_buffer[1], &smon)) == OK) {
			emon = smon;
...
xodtemplate_daterange *xodtemplate_add_exception_to_timeperiod(xodtemplate_timeperiod * period,
															   int type, int syear, int smon,
															   int smday, int swday,
															   int swday_offset, int eyear,
															   int emon, int emday, int ewday,
															   int ewday_offset,
															   int skip_interval,
															   char *timeranges)
{
	...
	for (temp_service = xodtemplate_service_list; temp_service != NULL;
		...
		if (temp_service->hostgroup_name != NULL) {
			if (xodtemplate_expand_hostgroups
				(&glist, host_map, temp_service->hostgroup_name, temp_service->_config_file,
		...
		if (temp_service->host_name) {
			if (xodtemplate_expand_hosts
				(&hlist, host_map, temp_service->host_name, temp_service->_config_file,
				 temp_service->_start_line) != OK) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
		...
		for (list = glist; list; list = next) {
			...
			for (hlist = hg->member_list; hlist; hlist = hlist->next) {
				...
				if (!next && !hlist->next) {
					...
					result =
						xodtemplate_duplicate_service(temp_service, h->host_name,
													  hg != &fake_hg);
				}
			}
	...
	for (temp_service = xodtemplate_service_list; temp_service != NULL;
		...
		case SKIPLIST_ERROR_DUPLICATE:
			logit(NSLOG_CONFIG_WARNING, TRUE,
				  ...
				  temp_service->service_description, temp_service->host_name,
				  xodtemplate_config_file_name(temp_service->_config_file),
				  temp_service->_start_line);
			result = ERROR;
			break;
 ...
 */
static int xodtemplate_create_service_list(objectlist ** ret, bitmap * reject_map,
										   char *host_name, char *hostgroup_name,
										   char *servicegroup_name, char *service_description,
										   int _config_file, int _start_line)
{
	...
	if (servicegroup_name
		&& xodtemplate_expand_servicegroups(&sglist, reject_map, servicegroup_name,
											_config_file, _start_line) != OK)
		return ERROR;
	 ...
	 */
	if (host_name
		&& xodtemplate_expand_hosts(&hlist, host_map, host_name, _config_file,
									_start_line) != OK)
		return ERROR;
	if (hostgroup_name
		&& xodtemplate_expand_hostgroups(&hglist, host_map, hostgroup_name, _config_file,
										 _start_line) != OK) {
	...
	for (glist = hglist; glist; glist = gnext) {
		...
		for (hlist = hg->member_list; hlist; hlist = hlist->next) {
			...
			if (xodtemplate_expand_services
				(&slist, reject_map, h->host_name, service_description, _config_file,
				 _start_line) != OK) {
				free_objectlist(&gnext);
				bitmap_destroy(in);
				return ERROR;
...
int xodtemplate_duplicate_objects(void)
{
	...
	for (temp_hostescalation = xodtemplate_hostescalation_list; temp_hostescalation != NULL;
		...
		/* get list of hosts */
		master_hostlist =
			xodtemplate_expand_hostgroups_and_hosts(temp_hostescalation->hostgroup_name,
	...
	for (temp_serviceescalation = xodtemplate_serviceescalation_list;
		...
		if (temp_serviceescalation->servicegroup_name == NULL
			...
			&& (temp_serviceescalation->host_name == NULL
				|| temp_serviceescalation->hostgroup_name == NULL))
			continue;
		...
		if (xodtemplate_create_service_list
			(&master_servicelist, service_map, temp_serviceescalation->host_name,
			 temp_serviceescalation->hostgroup_name, temp_serviceescalation->servicegroup_name,
			 temp_serviceescalation->service_description, temp_serviceescalation->_config_file,
			 temp_serviceescalation->_start_line) != OK)
			return ERROR;
		...
		for (list = master_servicelist; list; list = next) {
			...
			result =
				xodtemplate_duplicate_serviceescalation(temp_serviceescalation, s->host_name,
	...
	for (temp_hostextinfo = xodtemplate_hostextinfo_list; temp_hostextinfo != NULL;
		 temp_hostextinfo = next_he) {
		...
		/* get list of hosts */
		master_hostlist =
			xodtemplate_expand_hostgroups_and_hosts(temp_hostextinfo->hostgroup_name,
	...
	for (temp_serviceextinfo = xodtemplate_serviceextinfo_list; temp_serviceextinfo != NULL;
		 temp_serviceextinfo = next_se) {
		next_se = temp_serviceextinfo->next;

		/* skip definitions without enough data */
		if (temp_serviceextinfo->service_description == NULL
			|| (temp_serviceextinfo->hostgroup_name == NULL
		...
		/* get list of services */
		if (xodtemplate_create_service_list
			(&master_servicelist, service_map, temp_serviceextinfo->host_name,
			 temp_serviceextinfo->hostgroup_name, NULL,
			 temp_serviceextinfo->service_description, temp_serviceextinfo->_config_file,
			 temp_serviceextinfo->_start_line) != OK) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
...
int xodtemplate_duplicate_service(xodtemplate_service * temp_service, char *host_name, int from_hg)
{
	...
	if (temp_service->service_groups != NULL
		&& (new_service->service_groups =
			(char *)strdup(temp_service->service_groups)) == NULL)
		error = TRUE;
	if (temp_service->contact_groups != NULL
		&& (new_service->contact_groups =
			(char *)strdup(temp_service->contact_groups)) == NULL)
		error = TRUE;
	if (temp_service->contacts != NULL
		&& (new_service->contacts = (char *)strdup(temp_service->contacts)) == NULL)
		error = TRUE;
...
int xodtemplate_duplicate_hostescalation(xodtemplate_hostescalation * temp_hostescalation, char *host_name)
{
	...
	new_hostescalation =
		(xodtemplate_hostescalation *) calloc(1, sizeof(xodtemplate_hostescalation));
	...
	if (temp_hostescalation->contact_groups != NULL
		&& (new_hostescalation->contact_groups =
			(char *)strdup(temp_hostescalation->contact_groups)) == NULL)
		error = TRUE;
	if (temp_hostescalation->contacts != NULL
		&& (new_hostescalation->contacts =
			(char *)strdup(temp_hostescalation->contacts)) == NULL)
		error = TRUE;
...
int xodtemplate_duplicate_serviceescalation(xodtemplate_serviceescalation *
											temp_serviceescalation, char *host_name,
											char *svc_description)
{
	...
	new_serviceescalation =
		(xodtemplate_serviceescalation *) calloc(1, sizeof(xodtemplate_serviceescalation));
	...
	if (temp_serviceescalation->contact_groups != NULL
		&& (new_serviceescalation->contact_groups =
			(char *)strdup(temp_serviceescalation->contact_groups)) == NULL)
		error = TRUE;
	if (temp_serviceescalation->contacts != NULL
		&& (new_serviceescalation->contacts =
			(char *)strdup(temp_serviceescalation->contacts)) == NULL)
		error = TRUE;
...
int xodtemplate_inherit_object_properties(void)
{
	...
	for (temp_serviceescalation = xodtemplate_serviceescalation_list;
		...
		/* find the service */
		if ((temp_service =
			 xodtemplate_find_real_service(temp_serviceescalation->host_name,
		...
		if (temp_serviceescalation->have_contact_groups == FALSE
			&& temp_serviceescalation->have_contacts == FALSE) {
		...
		if (temp_serviceescalation->have_escalation_period == FALSE
			...
			temp_serviceescalation->escalation_period =
				(char *)strdup(temp_service->notification_period);
			temp_serviceescalation->have_escalation_period = TRUE;
		}
	...
	for (temp_hostescalation = xodtemplate_hostescalation_list; temp_hostescalation != NULL;
		...
		if (temp_hostescalation->contacts != NULL && temp_hostescalation->contacts[0] == '+')
			xodtemplate_get_inherited_string(&temp_host->have_contacts, &temp_host->contacts,
		...
		if (temp_hostescalation->have_contact_groups == FALSE
			&& temp_hostescalation->have_contacts == FALSE) {
		...
		if (temp_hostescalation->have_escalation_period == FALSE
			...
			temp_hostescalation->escalation_period =
				(char *)strdup(temp_host->notification_period);
			temp_hostescalation->have_escalation_period = TRUE;
		}
...
static int xodtemplate_register_and_destroy_hostdependency(void *hd_)
{
	...
	master_hostlist =
		xodtemplate_expand_hostgroups_and_hosts(temp_hostdependency->hostgroup_name,
	...
	dependent_hostlist =
		xodtemplate_expand_hostgroups_and_hosts(temp_hostdependency->dependent_hostgroup_name,
...
static int xodtemplate_register_and_destroy_servicedependency(void *sd_)
{
	...
	xodtemplate_servicedependency *temp_servicedependency =
		(xodtemplate_servicedependency *) sd_;
	...
	if (!children_first) {
		pret =
			xodtemplate_create_service_list(&parents, parent_map,
		...
		if (!same_host)
			cret =
				xodtemplate_create_service_list(&children, service_map,
	...
	for (plist = parents; plist; plist = pnext) {
		...
		if (same_host) {
			children = NULL;
			if (xodtemplate_expand_services
				(&children, service_map, p->host_name, dsdesc,
				 temp_servicedependency->_config_file,
				 temp_servicedependency->_start_line) != OK) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
		...
		for (clist = children; clist; clist = clist->next) {
			...
			if (xodtemplate_register_servicedependency(temp_servicedependency) != OK) {
				logit(NSLOG_VERIFICATION_WARNING, TRUE,
					  ...
					  p->host_name, p->service_description, c->host_name,
...
int xodtemplate_resolve_objects(void)
{
	...
	for (temp_contactgroup = xodtemplate_contactgroup_list; temp_contactgroup != NULL;
	...
	for (temp_servicegroup = xodtemplate_servicegroup_list; temp_servicegroup != NULL;
	...
	for (temp_hostdependency = xodtemplate_hostdependency_list; temp_hostdependency != NULL;
	...
	for (temp_ptr = my_strsep(&template_name_ptr, ","); temp_ptr != NULL;
		...
		if (template_timeperiod == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  temp_ptr, xodtemplate_config_file_name(this_timeperiod->_config_file),
		...
		for (x = 0; x < DATERANGE_TYPES; x++) {
			for (template_daterange = template_timeperiod->exceptions[x];
				 template_daterange != NULL; template_daterange = template_daterange->next) {

				/* see if this same daterange already exists in the timeperiod */
				for (this_daterange = this_timeperiod->exceptions[x]; this_daterange != NULL;
				...
				if ((new_daterange =
					 (xodtemplate_daterange *) malloc(sizeof(xodtemplate_daterange))) == NULL)
					continue;
	...
	for (temp_ptr = my_strsep(&template_name_ptr, ","); temp_ptr != NULL;
		...
		if (template_command == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  temp_ptr, xodtemplate_config_file_name(this_command->_config_file),
	...
	for (temp_ptr = my_strsep(&template_name_ptr, ","); temp_ptr != NULL;
		...
		if (template_contactgroup == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  temp_ptr, xodtemplate_config_file_name(this_contactgroup->_config_file),
	...
	for (temp_ptr = my_strsep(&template_name_ptr, ","); temp_ptr != NULL;
		...
		if (template_hostgroup == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  temp_ptr, xodtemplate_config_file_name(this_hostgroup->_config_file),
	...
	for (temp_ptr = my_strsep(&template_name_ptr, ","); temp_ptr != NULL;
		...
		if (template_servicegroup == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  temp_ptr, xodtemplate_config_file_name(this_servicegroup->_config_file),
	...
	for (temp_ptr = my_strsep(&template_name_ptr, ","); temp_ptr != NULL;
		...
		if (template_servicedependency == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  temp_ptr, xodtemplate_config_file_name(this_servicedependency->_config_file),
		...
		xodtemplate_get_inherited_string(&template_servicedependency->
										 have_dependent_servicegroup_name,
										 &template_servicedependency->
										 dependent_servicegroup_name,
		...
		xodtemplate_get_inherited_string(&template_servicedependency->
										 have_dependent_service_description,
										 &template_servicedependency->
										 dependent_service_description,
										 &this_servicedependency->
										 have_dependent_service_description,
		...
		if (this_servicedependency->have_dependency_period == FALSE
			...
			if (this_servicedependency->dependency_period == NULL
				...
				this_servicedependency->dependency_period =
					(char *)strdup(template_servicedependency->dependency_period);
		...
		if (this_servicedependency->have_execution_failure_options == FALSE
			&& template_servicedependency->have_execution_failure_options == TRUE) {
			this_servicedependency->execution_failure_options =
				template_servicedependency->execution_failure_options;
			this_servicedependency->have_execution_failure_options = TRUE;
		}
		if (this_servicedependency->have_notification_failure_options == FALSE
			&& template_servicedependency->have_notification_failure_options == TRUE) {
			this_servicedependency->notification_failure_options =
				template_servicedependency->notification_failure_options;
	...
	for (temp_ptr = my_strsep(&template_name_ptr, ","); temp_ptr != NULL;
		...
		if (template_serviceescalation == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  temp_ptr, xodtemplate_config_file_name(this_serviceescalation->_config_file),
		...
		if (this_serviceescalation->have_escalation_period == FALSE
			...
			if (this_serviceescalation->escalation_period == NULL
				...
				this_serviceescalation->escalation_period =
					(char *)strdup(template_serviceescalation->escalation_period);
		...
		if (this_serviceescalation->have_escalation_options == FALSE
			...
			this_serviceescalation->escalation_options =
				template_serviceescalation->escalation_options;
	...
	for (temp_ptr = my_strsep(&template_name_ptr, ","); temp_ptr != NULL;
		...
		if (template_contact == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  temp_ptr, xodtemplate_config_file_name(this_contact->_config_file),
	...
	for (temp_ptr = my_strsep(&template_name_ptr, ","); temp_ptr != NULL;
		...
		if (template_host == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  temp_ptr, xodtemplate_config_file_name(this_host->_config_file),
		...
		xodtemplate_get_inherited_string(&template_host->have_parents, &template_host->parents,
		...
		xodtemplate_get_inherited_string(&template_host->have_contacts,
										 &template_host->contacts, &this_host->have_contacts,
										 &this_host->contacts);
	...
	for (temp_ptr = my_strsep(&template_name_ptr, ","); temp_ptr != NULL;
		...
		if (template_service == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  temp_ptr, xodtemplate_config_file_name(this_service->_config_file),
		...
		xodtemplate_get_inherited_string(&template_service->have_parents,
										 &template_service->parents,
										 &this_service->have_parents, &this_service->parents);
	...
	for (temp_ptr = my_strsep(&template_name_ptr, ","); temp_ptr != NULL;
		...
		if (template_hostdependency == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  temp_ptr, xodtemplate_config_file_name(this_hostdependency->_config_file),
		...
		xodtemplate_get_inherited_string(&template_hostdependency->
										 have_dependent_hostgroup_name,
		...
		xod_inherit(this_hostdependency, template_hostdependency,
	...
	for (temp_ptr = my_strsep(&template_name_ptr, ","); temp_ptr != NULL;
		...
		if (template_hostescalation == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  temp_ptr, xodtemplate_config_file_name(this_hostescalation->_config_file),
	...
	for (temp_ptr = my_strsep(&template_name_ptr, ","); temp_ptr != NULL;
		...
		if (template_hostextinfo == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  temp_ptr, xodtemplate_config_file_name(this_hostextinfo->_config_file),
	...
	for (temp_ptr = my_strsep(&template_name_ptr, ","); temp_ptr != NULL;
		...
		if (template_serviceextinfo == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  temp_ptr, xodtemplate_config_file_name(this_serviceextinfo->_config_file),
 ...
 */
static int _xodtemplate_add_group_member(objectlist ** list, bitmap * in, bitmap * reject,
										 void *obj)
{
...
static int xodtemplate_recombobulate_contactgroup_subgroups(xodtemplate_contactgroup *
															temp_contactgroup)
{
		...
		for (mlist = inc->member_list; mlist; mlist = mlist->next) {
			...
			if (xodtemplate_add_contactgroup_member(temp_contactgroup, c) != OK) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
					  ...
					  c->contact_name, temp_contactgroup->contactgroup_name,
...
int xodtemplate_recombobulate_contactgroups(void)
{
	...
	for (temp_contactgroup = xodtemplate_contactgroup_list; temp_contactgroup;
			...
			for (next_ptr = ptr = temp_contactgroup->contactgroup_members; next_ptr;
				...
				if (!(cg = xodtemplate_find_real_contactgroup(ptr))) {
					logit(NSLOG_CONFIG_ERROR, TRUE,
						  ...
						  ptr, temp_contactgroup->contactgroup_name,
		...
		if (!use_precached_objects
			&& !(temp_contactgroup->reject_map = bitmap_create(xodcount.contacts))) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  "Error: Could not create reject map for contactgroup '%s'",
				  temp_contactgroup->contactgroup_name);
			return ERROR;
		}

		/* get list of contacts in the contactgroup */
		if (xodtemplate_expand_contacts
			(&accept, temp_contactgroup->reject_map, temp_contactgroup->members,
			 temp_contactgroup->_config_file, temp_contactgroup->_start_line) != OK) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
	...
	for (temp_contact = xodtemplate_contact_list; temp_contact != NULL;
		...
		if ((contactgroup_names =
			 xodtemplate_process_contactgroup_names(temp_contact->contact_groups,
		...
		for (temp_ptr = strtok(contactgroup_names, ","); temp_ptr;
			 temp_ptr = strtok(NULL, ",")) {
			...
			if (temp_contactgroup == NULL) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
					  ...
					  temp_ptr, temp_contact->contact_name,
					  xodtemplate_config_file_name(temp_contact->_config_file),
					  temp_contact->_start_line);
			...
			if (!temp_contactgroup->member_map
				&& !(temp_contactgroup->member_map = bitmap_create(xodcount.contacts))) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
...
static int xodtemplate_recombobulate_hostgroup_subgroups(xodtemplate_hostgroup *
														 temp_hostgroup)
{
	objectlist *mlist, *glist;
...
int xodtemplate_recombobulate_hostgroups(void)
{
	...
	for (temp_hostgroup = xodtemplate_hostgroup_list; temp_hostgroup;
		...
		/* we might need this */
		if (!use_precached_objects
			&& !(temp_hostgroup->reject_map = bitmap_create(xodcount.hosts))) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
	...
	for (temp_host = xodtemplate_host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		if ((hostgroup_names =
			 xodtemplate_process_hostgroup_names(temp_host->host_groups,
			...
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  temp_host->host_name, xodtemplate_config_file_name(temp_host->_config_file),
				  temp_host->_start_line);
			return ERROR;
		}
		...
		for (temp_ptr = strtok(hostgroup_names, ","); temp_ptr; temp_ptr = strtok(NULL, ",")) {
			...
			if (temp_hostgroup == NULL) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
					  ...
					  temp_ptr, temp_host->host_name,
					  xodtemplate_config_file_name(temp_host->_config_file),
					  temp_host->_start_line);
				my_free(hostgroup_names);
				return ERROR;
			}
			if (!temp_hostgroup->member_map
				&& !(temp_hostgroup->member_map = bitmap_create(xodcount.hosts))) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
...
static int xodtemplate_recombobulate_servicegroup_subgroups(xodtemplate_servicegroup *
															temp_servicegroup)
{
	objectlist *mlist, *glist;
...
int xodtemplate_recombobulate_servicegroups(void)
{
	 ...
	 */
	for (temp_servicegroup = xodtemplate_servicegroup_list; temp_servicegroup;
		 temp_servicegroup = temp_servicegroup->next) {
			...
			for (ptr = temp_servicegroup->servicegroup_members; ptr; ptr = next_ptr + 1) {
				...
				if (!(sg = xodtemplate_find_real_servicegroup(ptr))) {
					logit(NSLOG_CONFIG_ERROR, TRUE,
						  ...
						  ptr, temp_servicegroup->servicegroup_name,
		...
		/* we might need this */
		if (!use_precached_objects
			&& !(temp_servicegroup->reject_map = bitmap_create(xodcount.services))) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  "Error: Could not create reject map for hostgroup '%s'\n",
				  temp_servicegroup->servicegroup_name);
			return ERROR;
		...
		xodtemplate_expand_services(&accept, temp_servicegroup->reject_map, NULL,
									temp_servicegroup->members,
	...
	for (temp_service = xodtemplate_service_list; temp_service != NULL;
		...
		if (temp_service->service_groups == NULL || temp_service->host_name == NULL
			|| temp_service->service_description == NULL)
			continue;
		...
		if ((servicegroup_names =
			 xodtemplate_process_servicegroup_names(temp_service->service_groups,
		...
		for (temp_ptr = strtok(servicegroup_names, ","); temp_ptr;
			 temp_ptr = strtok(NULL, ",")) {
			...
			if (temp_servicegroup == NULL) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
					  ...
					  temp_ptr, temp_service->service_description, temp_service->host_name,
					  xodtemplate_config_file_name(temp_service->_config_file),
					  temp_service->_start_line);
			...
			if (!temp_servicegroup->member_map
				&& !(temp_servicegroup->member_map = bitmap_create(xodcount.services))) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
...
xodtemplate_timeperiod *xodtemplate_find_timeperiod(char *name)
{
	...
	return skiplist_find_first(xobject_template_skiplists[TIMEPERIOD_SKIPLIST],
...
xodtemplate_command *xodtemplate_find_command(char *name)
{
	...
	return skiplist_find_first(xobject_template_skiplists[COMMAND_SKIPLIST], &temp_command,
							   NULL);
}
...
xodtemplate_contactgroup *xodtemplate_find_contactgroup(char *name)
{
	...
	return skiplist_find_first(xobject_template_skiplists[CONTACTGROUP_SKIPLIST],
...
xodtemplate_contactgroup *xodtemplate_find_real_contactgroup(char *name)
{
	...
	return skiplist_find_first(xobject_skiplists[CONTACTGROUP_SKIPLIST], &temp_contactgroup,
							   NULL);
}
...
xodtemplate_hostgroup *xodtemplate_find_hostgroup(char *name)
{
	...
	return skiplist_find_first(xobject_template_skiplists[HOSTGROUP_SKIPLIST], &temp_hostgroup,
							   NULL);
}
...
xodtemplate_servicegroup *xodtemplate_find_servicegroup(char *name)
{
	...
	return skiplist_find_first(xobject_template_skiplists[SERVICEGROUP_SKIPLIST],
...
xodtemplate_servicegroup *xodtemplate_find_real_servicegroup(char *name)
{
	...
	return skiplist_find_first(xobject_skiplists[SERVICEGROUP_SKIPLIST], &temp_servicegroup,
							   NULL);
}
...
xodtemplate_servicedependency *xodtemplate_find_servicedependency(char *name)
{
	...
	return skiplist_find_first(xobject_template_skiplists[SERVICEDEPENDENCY_SKIPLIST],
							   &temp_servicedependency, NULL);
}
...
xodtemplate_serviceescalation *xodtemplate_find_serviceescalation(char *name)
{
	...
	return skiplist_find_first(xobject_template_skiplists[SERVICEESCALATION_SKIPLIST],
							   &temp_serviceescalation, NULL);
}
...
xodtemplate_contact *xodtemplate_find_contact(char *name)
{
	...
	return skiplist_find_first(xobject_template_skiplists[CONTACT_SKIPLIST], &temp_contact,
							   NULL);
}
...
xodtemplate_hostdependency *xodtemplate_find_hostdependency(char *name)
{
	...
	return skiplist_find_first(xobject_template_skiplists[HOSTDEPENDENCY_SKIPLIST],
...
xodtemplate_hostescalation *xodtemplate_find_hostescalation(char *name)
{
	...
	return skiplist_find_first(xobject_template_skiplists[HOSTESCALATION_SKIPLIST],
...
xodtemplate_hostextinfo *xodtemplate_find_hostextinfo(char *name)
{
	...
	return skiplist_find_first(xobject_template_skiplists[HOSTEXTINFO_SKIPLIST],
...
xodtemplate_serviceextinfo *xodtemplate_find_serviceextinfo(char *name)
{
	...
	return skiplist_find_first(xobject_template_skiplists[SERVICEEXTINFO_SKIPLIST],
...
xodtemplate_service *xodtemplate_find_service(char *name)
{
	...
	return skiplist_find_first(xobject_template_skiplists[SERVICE_SKIPLIST], &temp_service,
							   NULL);
}
...
static int xodtemplate_register_contactgroup_members(xodtemplate_contactgroup *
													 this_contactgroup)
{
	objectlist *list;
...
static int xodtemplate_register_hostgroup_members(xodtemplate_hostgroup * this_hostgroup)
{
	...
	for (list = this_hostgroup->member_list; list; list = list->next) {
		...
		if (!add_host_to_hostgroup(hg, h->host_name)) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  "Error: Bad member of hostgroup '%s' (config file '%s', starting on line %d)\n",
				  hg->group_name, xodtemplate_config_file_name(this_hostgroup->_config_file),
				  this_hostgroup->_start_line);
			return -1;
		}
		num_regs++;
...
static int xodtemplate_register_servicegroup_members(xodtemplate_servicegroup *
													 this_servicegroup)
{
	objectlist *list, *next;
...
int xodtemplate_register_objects(void)
{
	...
	ptr = NULL;
	for (temp_timeperiod =
		 (xodtemplate_timeperiod *) skiplist_get_first(xobject_skiplists[TIMEPERIOD_SKIPLIST],
													   &ptr); temp_timeperiod != NULL;
	...
	ptr = NULL;
	for (temp_command =
		 (xodtemplate_command *) skiplist_get_first(xobject_skiplists[COMMAND_SKIPLIST], &ptr);
		 temp_command != NULL;
	...
	ptr = NULL;
	for (temp_contactgroup =
		 (xodtemplate_contactgroup *)
		 skiplist_get_first(xobject_skiplists[CONTACTGROUP_SKIPLIST], &ptr);
		 temp_contactgroup != NULL;
	...
	ptr = NULL;
	for (temp_hostgroup =
		 (xodtemplate_hostgroup *) skiplist_get_first(xobject_skiplists[HOSTGROUP_SKIPLIST],
													  &ptr); temp_hostgroup != NULL;
	...
	ptr = NULL;
	for (temp_servicegroup =
		 (xodtemplate_servicegroup *)
		 skiplist_get_first(xobject_skiplists[SERVICEGROUP_SKIPLIST], &ptr);
		 temp_servicegroup != NULL;
	...
	ptr = NULL;
	for (temp_contact =
		 (xodtemplate_contact *) skiplist_get_first(xobject_skiplists[CONTACT_SKIPLIST], &ptr);
		 temp_contact != NULL;
	...
	ptr = NULL;
	for (temp_host =
		 (xodtemplate_host *) skiplist_get_first(xobject_skiplists[HOST_SKIPLIST], &ptr);
		 temp_host != NULL; temp_host = (xodtemplate_host *) skiplist_get_next(&ptr)) {
	...
	ptr = NULL;
	for (temp_service =
		 (xodtemplate_service *) skiplist_get_first(xobject_skiplists[SERVICE_SKIPLIST], &ptr);
		 temp_service != NULL;
...
int xodtemplate_register_timeperiod(xodtemplate_timeperiod * this_timeperiod)
{
	...
	for (x = 0; x < DATERANGE_TYPES; x++) {
		for (temp_daterange = this_timeperiod->exceptions[x]; temp_daterange != NULL;
			...
			new_daterange =
				add_exception_to_timeperiod(new_timeperiod, temp_daterange->type,
											temp_daterange->syear, temp_daterange->smon,
											temp_daterange->smday, temp_daterange->swday,
											temp_daterange->swday_offset,
											temp_daterange->eyear, temp_daterange->emon,
											temp_daterange->emday, temp_daterange->ewday,
			...
			for (day_range_start_buffer = my_strsep(&day_range_ptr, ", ");
				...
				if (xodtemplate_get_time_ranges
					(day_range_start_buffer, &range_start_time, &range_end_time) == ERROR) {
					logit(NSLOG_CONFIG_ERROR, TRUE,
						  "Error: Could not parse timerange #%d of timeperiod (config file '%s', starting on line %d)\n",
						  range, xodtemplate_config_file_name(this_timeperiod->_config_file),
						  this_timeperiod->_start_line);
					return ERROR;
				}
				...
				new_timerange =
					add_timerange_to_daterange(new_daterange, range_start_time,
											   range_end_time);
		...
		for (day_range_start_buffer = my_strsep(&day_range_ptr, ", ");
			...
			if (xodtemplate_get_time_ranges
				...
				logit(NSLOG_CONFIG_ERROR, TRUE,
					  ...
					  range, day, xodtemplate_config_file_name(this_timeperiod->_config_file),
					  this_timeperiod->_start_line);
				return ERROR;
			}
			...
			new_timerange =
				add_timerange_to_timeperiod(new_timeperiod, day, range_start_time,
											range_end_time);
			if (new_timerange == NULL) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
...
int xodtemplate_register_contactgroup(xodtemplate_contactgroup * this_contactgroup)
{
	...
	new_contactgroup =
		add_contactgroup(this_contactgroup->contactgroup_name, this_contactgroup->alias);
...
int xodtemplate_register_servicegroup(xodtemplate_servicegroup * this_servicegroup)
{
	...
	new_servicegroup =
		add_servicegroup(this_servicegroup->servicegroup_name, this_servicegroup->alias,
...
int xodtemplate_register_servicedependency(xodtemplate_servicedependency *this_servicedependency)
{
	...
	if (this_servicedependency->have_notification_failure_options == FALSE
		&& this_servicedependency->have_execution_failure_options == FALSE) {
		logit(NSLOG_CONFIG_WARNING, TRUE,
	...
	if (this_servicedependency->have_execution_failure_options == TRUE) {
		...
		new_servicedependency =
			add_service_dependency(this_servicedependency->dependent_host_name,
...
int xodtemplate_register_serviceescalation(xodtemplate_serviceescalation *this_serviceescalation)
{
	...
	new_serviceescalation =
		add_serviceescalation(this_serviceescalation->host_name,
							  this_serviceescalation->service_description,
							  this_serviceescalation->first_notification,
							  this_serviceescalation->last_notification,
							  this_serviceescalation->notification_interval,
	...
	if (this_serviceescalation->contact_groups != NULL) {
		...
		for (contact_group = strtok(this_serviceescalation->contact_groups, ",");
			...
			new_contactgroupsmember =
				add_contactgroup_to_serviceescalation(new_serviceescalation, contact_group);
...
int xodtemplate_register_contact(xodtemplate_contact * this_contact)
{
	...
	/* add the contact */
	new_contact =
		add_contact(this_contact->contact_name, this_contact->alias, this_contact->email,
					this_contact->pager, this_contact->address,
					this_contact->service_notification_period,
					this_contact->host_notification_period,
					this_contact->service_notification_options,
					this_contact->host_notification_options,
					this_contact->host_notifications_enabled,
					this_contact->service_notifications_enabled,
					this_contact->can_submit_commands, this_contact->retain_status_information,
					this_contact->retain_nonstatus_information, this_contact->minimum_value);
		...
		for (command_name = strtok(this_contact->host_notification_commands, ", ");
			...
			if (new_commandsmember == NULL) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
					  ...
					  command_name, xodtemplate_config_file_name(this_contact->_config_file),
					  this_contact->_start_line);
				return ERROR;
			}
		}
	...
	if (this_contact->service_notification_commands != NULL) {
		...
		for (command_name = strtok(this_contact->service_notification_commands, ", ");
			 command_name != NULL; command_name = strtok(NULL, ", ")) {
			new_commandsmember =
				add_service_notification_command_to_contact(new_contact, command_name);
			if (new_commandsmember == NULL) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
...
int xodtemplate_register_host(xodtemplate_host * this_host)
{
	...
	new_host =
		add_host(this_host->host_name, this_host->display_name, this_host->alias,
				 this_host->address, this_host->check_period, this_host->initial_state,
				 this_host->check_interval, this_host->retry_interval,
				 this_host->max_check_attempts, this_host->notification_options,
				 ...
				 this_host->flap_detection_options, this_host->stalking_options,
				 ...
				 this_host->y_2d, this_host->have_2d_coords, this_host->x_3d, this_host->y_3d,
				 this_host->z_3d, this_host->have_3d_coords, TRUE,
				 this_host->retain_status_information, this_host->retain_nonstatus_information,
				 this_host->obsess, this_host->hourly_value);
		...
		for (parent_host = strtok(this_host->parents, ","); parent_host != NULL;
			...
			if (new_hostsmember == NULL) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
					  ...
					  parent_host, xodtemplate_config_file_name(this_host->_config_file),
					  this_host->_start_line);
				return ERROR;
			}
		}
		...
		for (contact_group = strtok(this_host->contact_groups, ","); contact_group != NULL;
			...
			if (new_contactgroupsmember == NULL) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
					  ...
					  contact_group, xodtemplate_config_file_name(this_host->_config_file),
					  this_host->_start_line);
				return ERROR;
			}
		}
		...
		for (contact_name = strtok(this_host->contacts, ","); contact_name != NULL;
			...
			if (new_contactsmember == NULL) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
					  ...
					  contact_name, xodtemplate_config_file_name(this_host->_config_file),
					  this_host->_start_line);
				return ERROR;
			}
		}
...
int xodtemplate_register_service(xodtemplate_service * this_service)
{
	...
	/* add the service */
	new_service =
		add_service(this_service->host_name, this_service->service_description,
					...
					this_service->notification_options, this_service->notifications_enabled,
					this_service->is_volatile, this_service->event_handler,
					this_service->event_handler_enabled, this_service->check_command,
					this_service->active_checks_enabled, this_service->flap_detection_enabled,
					this_service->low_flap_threshold, this_service->high_flap_threshold,
					this_service->flap_detection_options, this_service->stalking_options,
					this_service->process_perf_data, this_service->check_freshness,
					...
					this_service->retain_nonstatus_information, this_service->obsess,
	...
	if (this_service->parents != NULL) {
		...
		if (!comma) {			/* same-host single-service parent */
			...
			bitmap_clear(service_map);
			if (xodtemplate_expand_services
				(&list, service_map, NULL, this_service->parents, this_service->_config_file,
				 this_service->_start_line) != OK) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
			...
			for (; list; list = next) {
				...
				free(list);
				new_servicesmember =
					add_parent_service_to_service(new_service, parent->host_name,
												  parent->service_description);
				if (new_servicesmember == NULL) {
					logit(NSLOG_CONFIG_ERROR, TRUE,
		...
		for (contact_group = strtok(this_service->contact_groups, ","); contact_group != NULL;
			...
			if (new_contactgroupsmember == NULL) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
					  ...
					  contact_group, xodtemplate_config_file_name(this_service->_config_file),
					  this_service->_start_line);
				return ERROR;
			}
		}
		...
		for (contact_name = strtok(this_service->contacts, ","); contact_name != NULL;
			...
			if (new_contactsmember == NULL) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
					  ...
					  contact_name, xodtemplate_config_file_name(this_service->_config_file),
					  this_service->_start_line);
				return ERROR;
			}
		}
...
int xodtemplate_register_hostdependency(xodtemplate_hostdependency * this_hostdependency)
{
	...
	if (this_hostdependency->have_execution_failure_options == TRUE) {
		...
		new_hostdependency =
			add_host_dependency(this_hostdependency->dependent_host_name,
								this_hostdependency->host_name, EXECUTION_DEPENDENCY,
								this_hostdependency->inherits_parent,
								this_hostdependency->execution_failure_options,
	...
	if (this_hostdependency->have_notification_failure_options == TRUE) {
		...
		new_hostdependency =
			add_host_dependency(this_hostdependency->dependent_host_name,
								...
								this_hostdependency->inherits_parent,
								this_hostdependency->notification_failure_options,
								this_hostdependency->dependency_period);
...
int xodtemplate_register_hostescalation(xodtemplate_hostescalation * this_hostescalation)
{
	...
	new_hostescalation =
		add_hostescalation(this_hostescalation->host_name,
						   ...
						   this_hostescalation->escalation_options);
	...
	if (this_hostescalation->contact_groups != NULL) {
		...
		for (contact_group = strtok(this_hostescalation->contact_groups, ",");
			...
			strip(contact_group);
			new_contactgroupsmember =
				add_contactgroup_to_hostescalation(new_hostescalation, contact_group);
			if (new_contactgroupsmember == NULL) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
	...
	if (this_hostescalation->contacts != NULL) {
		...
		for (contact_name = strtok(this_hostescalation->contacts, ","); contact_name != NULL;
			 contact_name = strtok(NULL, ",")) {

			strip(contact_name);
			new_contactsmember =
				add_contact_to_hostescalation(new_hostescalation, contact_name);
			if (new_contactsmember == NULL) {
				logit(NSLOG_CONFIG_ERROR, TRUE,
...
int xodtemplate_init_xobject_skiplists(void)
{
	...
	xobject_template_skiplists[HOST_SKIPLIST] =
		skiplist_new(16, 0.5, FALSE, FALSE, xodtemplate_skiplist_compare_host_template);
	xobject_template_skiplists[SERVICE_SKIPLIST] =
		skiplist_new(16, 0.5, FALSE, FALSE, xodtemplate_skiplist_compare_service_template);
	xobject_template_skiplists[COMMAND_SKIPLIST] =
		skiplist_new(10, 0.5, FALSE, FALSE, xodtemplate_skiplist_compare_command_template);
	xobject_template_skiplists[TIMEPERIOD_SKIPLIST] =
		skiplist_new(10, 0.5, FALSE, FALSE, xodtemplate_skiplist_compare_timeperiod_template);
	xobject_template_skiplists[CONTACT_SKIPLIST] =
		skiplist_new(10, 0.5, FALSE, FALSE, xodtemplate_skiplist_compare_contact_template);
	xobject_template_skiplists[CONTACTGROUP_SKIPLIST] =
		skiplist_new(10, 0.5, FALSE, FALSE,
					 xodtemplate_skiplist_compare_contactgroup_template);
	xobject_template_skiplists[HOSTGROUP_SKIPLIST] =
		skiplist_new(10, 0.5, FALSE, FALSE, xodtemplate_skiplist_compare_hostgroup_template);
	xobject_template_skiplists[SERVICEGROUP_SKIPLIST] =
		skiplist_new(10, 0.5, FALSE, FALSE,
					 xodtemplate_skiplist_compare_servicegroup_template);
	xobject_template_skiplists[HOSTDEPENDENCY_SKIPLIST] =
		skiplist_new(16, 0.5, FALSE, FALSE,
					 xodtemplate_skiplist_compare_hostdependency_template);
	xobject_template_skiplists[SERVICEDEPENDENCY_SKIPLIST] =
		skiplist_new(16, 0.5, FALSE, FALSE,
					 xodtemplate_skiplist_compare_servicedependency_template);
	xobject_template_skiplists[HOSTESCALATION_SKIPLIST] =
		skiplist_new(16, 0.5, FALSE, FALSE,
					 xodtemplate_skiplist_compare_hostescalation_template);
	xobject_template_skiplists[SERVICEESCALATION_SKIPLIST] =
		skiplist_new(16, 0.5, FALSE, FALSE,
					 xodtemplate_skiplist_compare_serviceescalation_template);
	xobject_template_skiplists[HOSTEXTINFO_SKIPLIST] =
		skiplist_new(16, 0.5, FALSE, FALSE, xodtemplate_skiplist_compare_hostextinfo_template);
	xobject_template_skiplists[SERVICEEXTINFO_SKIPLIST] =
		skiplist_new(16, 0.5, FALSE, FALSE,
					 xodtemplate_skiplist_compare_serviceextinfo_template);

	xobject_skiplists[HOST_SKIPLIST] =
		skiplist_new(16, 0.5, FALSE, FALSE, xodtemplate_skiplist_compare_host);
	xobject_skiplists[SERVICE_SKIPLIST] =
		skiplist_new(16, 0.5, FALSE, FALSE, xodtemplate_skiplist_compare_service);
	xobject_skiplists[COMMAND_SKIPLIST] =
		skiplist_new(16, 0.5, FALSE, FALSE, xodtemplate_skiplist_compare_command);
	xobject_skiplists[TIMEPERIOD_SKIPLIST] =
		skiplist_new(16, 0.5, FALSE, FALSE, xodtemplate_skiplist_compare_timeperiod);
	xobject_skiplists[CONTACT_SKIPLIST] =
		skiplist_new(10, 0.5, FALSE, FALSE, xodtemplate_skiplist_compare_contact);
	xobject_skiplists[CONTACTGROUP_SKIPLIST] =
		skiplist_new(10, 0.5, FALSE, FALSE, xodtemplate_skiplist_compare_contactgroup);
	xobject_skiplists[HOSTGROUP_SKIPLIST] =
		skiplist_new(10, 0.5, FALSE, FALSE, xodtemplate_skiplist_compare_hostgroup);
	xobject_skiplists[SERVICEGROUP_SKIPLIST] =
		skiplist_new(10, 0.5, FALSE, FALSE, xodtemplate_skiplist_compare_servicegroup);
	/*
...
int xodtemplate_skiplist_compare_service(void *a, void *b)
{
	...
	return skiplist_compare_text(oa->host_name, oa->service_description, ob->host_name,
...
int xodtemplate_skiplist_compare_servicedependency(void *a, void *b)
{
	...
	return skiplist_compare_text(oa->dependent_host_name, oa->dependent_service_description,
...
int xodtemplate_free_memory(void)
{
	...
	for (this_contactgroup = xodtemplate_contactgroup_list; this_contactgroup != NULL;
	...
	for (this_servicegroup = xodtemplate_servicegroup_list; this_servicegroup != NULL;
...
int xodtemplate_add_member_to_memberlist(xodtemplate_memberlist ** list, char *name1, char *name2)
{
	...
	if ((new_item =
		 (xodtemplate_memberlist *) calloc(1, sizeof(xodtemplate_memberlist))) == NULL)
		return ERROR;
...
/* expands contacts */
int xodtemplate_expand_contacts(objectlist ** ret, bitmap * reject_map, char *contacts,
								int _config_file, int _start_line)
{
	...
	*ret = NULL;
	...
	for (temp_ptr = strtok(contact_names, ","); temp_ptr; temp_ptr = strtok(NULL, ",")) {
		...
		if (use_regexp_matches == TRUE
			&& (use_true_regexp_matching == TRUE || strstr(temp_ptr, "*")
				|| strstr(temp_ptr, "?") || strstr(temp_ptr, "+") || strstr(temp_ptr, "\\.")))
			use_regexp = TRUE;
 ...
 */
objectlist *xodtemplate_expand_hostgroups_and_hosts(char *hostgroups, char *hosts,
													int _config_file, int _start_line)
{
	...
	if (hostgroups != NULL) {
		/* expand host */
		result =
			xodtemplate_expand_hostgroups(&glist, reject, hostgroups, _config_file,
										  _start_line);
		if (result != OK) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
 ...
 */
int xodtemplate_expand_hostgroups(objectlist ** list, bitmap * reject_map, char *hostgroups,
								  int _config_file, int _start_line)
{
	char	   *hostgroup_names = NULL;
	char	   *temp_ptr = NULL;
...
/* expands hosts */
int xodtemplate_expand_hosts(objectlist ** list, bitmap * reject_map, char *hosts,
							 int _config_file, int _start_line)
{
	char	   *temp_ptr = NULL;
	xodtemplate_host *temp_host = NULL;
 ...
 */
int xodtemplate_expand_servicegroups(objectlist ** list, bitmap * reject, char *servicegroups,
									 int _config_file, int _start_line)
{
	xodtemplate_servicegroup *temp_servicegroup = NULL;
	regex_t 	preg;
...
int xodtemplate_expand_services(objectlist ** list, bitmap * reject_map, char *host_name,
								char *services, int _config_file, int _start_line)
{
	...
	if (host_name == NULL && services != NULL) {
		...
		for (next_p = p1 = scopy; next_p; p1 = next_p + 1) {
			...
			*p2 = 0;
			...
			if (xodtemplate_expand_services
				(list, reject_map, p1, p2, _config_file, _start_line) != OK) {
				free(scopy);
				return ERROR;
			}
		}
	...
	if (use_regexp_matches == TRUE
		&& (use_true_regexp_matching == TRUE || strstr(host_name, "*")
			|| strstr(host_name, "?") || strstr(host_name, "+") || strstr(host_name, "\\.")))
	...
	for (temp_ptr = strtok(service_names, ","); temp_ptr; temp_ptr = strtok(NULL, ",")) {
		...
		if (found_match == FALSE && reject_item == FALSE) {
			logit(NSLOG_CONFIG_ERROR, TRUE,
				  ...
				  host_name, temp_ptr, xodtemplate_config_file_name(_config_file),
				  _start_line);
			break;
		}
	}
...
char	   *xodtemplate_process_hostgroup_names(char *hostgroups, int _config_file,
												int _start_line)
{
	...
	if (hostgroups != NULL) {
		...
		result =
			xodtemplate_get_hostgroup_names(&temp_list, &reject_list, hostgroups, _config_file,
											_start_line);
		if (result != OK) {
...
int xodtemplate_get_hostgroup_names(xodtemplate_memberlist ** list,
									xodtemplate_memberlist ** reject_list, char *hostgroups,
									int _config_file, int _start_line)
{
	char	   *hostgroup_names = NULL;
	char	   *temp_ptr = NULL;
	...
	for (temp_ptr = strtok(hostgroup_names, ","); temp_ptr; temp_ptr = strtok(NULL, ",")) {
		...
		if (use_regexp == TRUE) {
			...
			for (temp_hostgroup = xodtemplate_hostgroup_list; temp_hostgroup != NULL;
				...
				/* add hostgroup to list */
				xodtemplate_add_member_to_memberlist(list, temp_hostgroup->hostgroup_name,
													 NULL);
			}
		...
		/* use standard matching... */
			...
			/* else this is just a single hostgroup... */
				...
				if (temp_hostgroup != NULL) {
					...
					xodtemplate_add_member_to_memberlist((reject_item ==
														  TRUE) ? reject_list : list,
...
char	   *xodtemplate_process_contactgroup_names(char *contactgroups, int _config_file,
												   int _start_line)
{
	xodtemplate_memberlist *temp_list = NULL;
	xodtemplate_memberlist *reject_list = NULL;
	...
	if (contactgroups != NULL) {
		...
		/* split group names into two lists */
		result =
			xodtemplate_get_contactgroup_names(&temp_list, &reject_list, contactgroups,
...
int xodtemplate_get_contactgroup_names(xodtemplate_memberlist ** list,
									   ...
									   char *contactgroups, int _config_file, int _start_line)
{
	char	   *contactgroup_names = NULL;
	char	   *temp_ptr = NULL;
	xodtemplate_contactgroup *temp_contactgroup = NULL;
...
char	   *xodtemplate_process_servicegroup_names(char *servicegroups, int _config_file,
												   int _start_line)
{
	xodtemplate_memberlist *temp_list = NULL;
	xodtemplate_memberlist *reject_list = NULL;
	...
	if (servicegroups != NULL) {
		...
		/* split group names into two lists */
		result =
			xodtemplate_get_servicegroup_names(&temp_list, &reject_list, servicegroups,
											   _config_file, _start_line);
		if (result != OK) {
			xodtemplate_free_memberlist(&temp_list);
			xodtemplate_free_memberlist(&reject_list);
...
int xodtemplate_get_servicegroup_names(xodtemplate_memberlist ** list,
									   xodtemplate_memberlist ** reject_list,
									   char *servicegroups, int _config_file, int _start_line)
{
	char	   *servicegroup_names = NULL;
	char	   *temp_ptr = NULL;
	xodtemplate_servicegroup *temp_servicegroup = NULL;
...
int xodtemplate_get_inherited_string(char *have_template_value, char **template_value,
									 char *have_this_value, char **this_value)
{
	char	   *buf = NULL;
	...
	if (*have_template_value == TRUE) {
		...
		if (*template_value != NULL) {
			...
			/* we already have a value... */
				...
				if (*this_value[0] == '+') {
					if ((buf =
						 (char *)malloc(strlen(*template_value) + strlen(*this_value) + 1))) {
...
int xodtemplate_clean_additive_strings(void)
{
	...
	for (temp_servicedependency = xodtemplate_servicedependency_list;
		...
		xodtemplate_clean_additive_string(&temp_servicedependency->
										  dependent_servicegroup_name);
