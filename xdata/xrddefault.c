...
int xrddefault_save_state_information(void)
{
			...
			if (temp_customvariablesmember->variable_name)
				fprintf(fp, "_%s=%d;%s\n", temp_customvariablesmember->variable_name,
						...
						(temp_customvariablesmember->variable_value ==
						 NULL) ? "" : temp_customvariablesmember->variable_value);
		}
	...
	for (temp_service = service_list; temp_service != NULL; temp_service = temp_service->next) {
		...
		for (x = 0; x < MAX_STATE_HISTORY_ENTRIES; x++)
			fprintf(fp, "%s%d", (x > 0) ? "," : "",
					temp_service->state_history[(x + temp_service->state_history_index) %
												MAX_STATE_HISTORY_ENTRIES]);
		fprintf(fp, "\n");
	...
	for (temp_contact = contact_list; temp_contact != NULL; temp_contact = temp_contact->next) {
		...
		fprintf(fp, "host_notification_period=%s\n",
				(temp_contact->host_notification_period ==
				 NULL) ? "" : temp_contact->host_notification_period);
		fprintf(fp, "service_notification_period=%s\n",
				(temp_contact->service_notification_period ==
				 NULL) ? "" : temp_contact->service_notification_period);
	...
	if (result == 0) {
		...
		if (my_rename(tmp_file, retention_file)) {
			unlink(tmp_file);
			logit(NSLOG_RUNTIME_ERROR, TRUE, "Error: Unable to update retention file '%s': %s",
	...
	/* a problem occurred saving the file */
		...
		unlink(tmp_file);
		logit(NSLOG_RUNTIME_ERROR, TRUE, "Error: Unable to save retention file: %s",
			  strerror(errno));
	}
	...
	while (1) {
		...
		if (!strcmp(input, "service {"))
			...
			case XRDDEFAULT_HOSTSTATUS_DATA:
				...
				if (temp_host != NULL) {
					...
					if (temp_host->current_state != HOST_UP
						&& temp_host->last_notification != (time_t) 0)
						temp_host->next_notification =
							get_next_host_notification_time(temp_host,
				...
				if (temp_service != NULL) {
					...
					if (temp_service->current_state != STATE_OK
						...
						temp_service->next_notification =
							get_next_service_notification_time(temp_service,
															   temp_service->
															   last_notification);
			...
			case XRDDEFAULT_SERVICECOMMENT_DATA:
				...
				/* add the comment */
				add_comment((data_type ==
							 XRDDEFAULT_HOSTCOMMENT_DATA) ? HOST_COMMENT : SERVICE_COMMENT,
							entry_type, host_name, service_description, entry_time, author,
							comment_data, comment_id, persistent, expires, expire_time,
							source);
				...
				/* non-persistent comments don't last past restarts UNLESS they're acks (see above) */
						 ...
						 && (sigrestart == FALSE || entry_type == DOWNTIME_COMMENT))
					remove_comment = TRUE;

				if (remove_comment == TRUE)
					delete_comment((data_type ==
									XRDDEFAULT_HOSTCOMMENT_DATA) ? HOST_COMMENT :
								   SERVICE_COMMENT, comment_id);
			...
			case XRDDEFAULT_SERVICEDOWNTIME_DATA:
				...
				if (data_type == XRDDEFAULT_HOSTDOWNTIME_DATA)
					add_host_downtime(host_name, entry_time, author, comment_data, start_time,
									  flex_downtime_start, end_time, fixed, triggered_by,
									  duration, downtime_id, is_in_effect,
									  start_notification_sent);
				else
					add_service_downtime(host_name, service_description, entry_time, author,
										 comment_data, start_time, flex_downtime_start,
				...
				register_downtime((data_type ==
								   XRDDEFAULT_HOSTDOWNTIME_DATA) ? HOST_DOWNTIME :
								  SERVICE_DOWNTIME, downtime_id);
									...
									for (temp_customvariablesmember =
										...
										if (!strcmp
											...
											if ((x = atoi(val)) > 0 && strlen(val) > 3) {
												my_free(temp_customvariablesmember->
														variable_value);
												temp_customvariablesmember->variable_value =
													(char *)strdup(val + 2);
												temp_customvariablesmember->has_been_modified =
				...
				if (temp_contact == NULL) {
					...
					if (temp_contact->retain_nonstatus_information == TRUE) {
						...
						if (found_directive == TRUE) ;
							...
							if (temp_contact->
								modified_host_attributes & MODATTR_NOTIFICATION_TIMEPERIOD) {
							...
							if (temp_contact->
								modified_service_attributes & MODATTR_NOTIFICATION_TIMEPERIOD)
							{
							...
							if (temp_contact->
								modified_host_attributes & MODATTR_NOTIFICATIONS_ENABLED)
								temp_contact->host_notifications_enabled =
									(atoi(val) > 0) ? TRUE : FALSE;
						} else if (!strcmp(var, "service_notifications_enabled")) {
							if (temp_contact->
								modified_service_attributes & MODATTR_NOTIFICATIONS_ENABLED)
								temp_contact->service_notifications_enabled =
									(atoi(val) > 0) ? TRUE : FALSE;
						}
						/* custom variables */
